    ...
    public class EnvioCandidatoVagaPerfil : InputPlugin
    {
        ...
        protected override IPluginResult DoExecuteTask(ParametroExecucaoCollection objParametros, Dictionary<string, byte[]> objAnexos)
        {
            ...
            try
            {
                ...
                if (objPlanoAdquiridoDetalhes != null)
                {
                    ...
                    List<BNE.BLL.DTO.PessoaFisicaEnvioSMSTanque> listaCVSEmail = BNE.BLL.Curriculo.RecuperarListaCvsEnvio(objVaga.Cidade.IdCidade, objVaga.Funcao.IdFuncao, VagaTipoVinculo.PossuiVinculo(objVaga, TipoVinculo.Estágio));
                    string mensagem;
        ...
        public static void CriarMensagens(BLL.Vaga objVaga, out MensagemPlugin.MensagemSMSTanque LoteSMSTanque, out List<MensagemPlugin.MensagemEmail> listaEmail, BLL.PlanoAdquiridoDetalhes objPlanoAdquiridoDetalhes = null)
        {
            try
            {
                ...
                var templateSMS = new CampanhaTanque().GetTextoCampanha(BLL.Enumeradores.CampanhaTanque.EnvioCandVagaPerfil);
                ...
                LoteSMSTanque.idCampanha = (int)BNE.BLL.Enumeradores.CampanhaTanque.EnvioCandVagaPerfil;
                ...
                foreach (var objCurriculo in listaCvs)
                {
                    ...
                    try
                    {
                        ...
                        if (objVaga.Deficiencia == null || objVaga.Deficiencia.IdDeficiencia.Equals((int)BLL.Enumeradores.Deficiencia.Nenhuma))
                        {
                            ...
                            sms = sms.Replace("{Link}", BLL.Vaga.MontarLinkVagaSMS(objVaga.IdVaga, BLL.Curriculo.LoadObject(objCurriculo.idDestinatario), "?utm_source=candidatarvaga&utm_medium=SMS&utm_campaign=candidatovagaperfilSMS"));
                            sms = sms.Replace("{Nome}", PessoaFisica.RetornarPrimeiroNome(objCurriculo.nomePessoa));
                    ...
                    try
                    {
                        ...
                        if (BLL.Custom.Validacao.ValidarEmail(objCurriculo.emailPessoa))
                        {
                            ...
                            BNE.BLL.Curriculo.RetornarHashLogarCurriculo(objCurriculo.numeroCPF, objCurriculo.dataNascimento, out listaVagas, out salaVip, out vip, out quemMeViu, out cadastroCurriculo, out pesquisaVagas, out loginCandidato, out cadastroExperiencias);

                            string linkVaga = LoginAutomatico.GerarHashAcessoLogin(objCurriculo.numeroCPF, objCurriculo.dataNascimento, objVaga.URL());
                            #endregion
