    ...
    public class NotificacoesVip
    {
        ...
        private static void NotificarVip(DTO.Curriculo vip)
        {
            try
            {
                ...
                DateTime dataInicioPlano = vip.dataInicioPlano;
                DateTime hoje = DateTime.Now;

                //Definir Ciclo Do Mes
                TimeSpan diferencaDatas = hoje - dataInicioPlano;
                int diasCadastro = diferencaDatas.Days;
                int cicloCV = 0;
                ...
                //GravaLog(null, "Ciclo " + cicloCV + " do CV -> " + vip.idCurriculo);

                NotificacaoVip notificacaoParaExecutar = _notificacoes.Where(x => x.NumeroDia == cicloCV && x.FlagInativo == false).FirstOrDefault();
