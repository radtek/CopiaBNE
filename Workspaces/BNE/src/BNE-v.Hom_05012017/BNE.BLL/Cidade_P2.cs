...
namespace BNE.BLL
{
    public partial class Cidade // Tabela: TAB_Cidade
    {
        ...
        private static List<CidadeCache> ListarCidadesCACHE()
        {
            ...
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, spselecttodascidades, null))
            {
                while (dr.Read())
                {
                    ...
                    if (dr["Geo_Localizacao"] != DBNull.Value)
                        objCidade.GeoLocalizacao = (SqlGeography)(dr["Geo_Localizacao"]);

                    listaCidades.Add(objCidade);
                }
                if (!dr.IsClosed)
                    dr.Close();
            }
        ...
        private class CidadeCache
        {
            ...
            public SqlGeography GeoLocalizacao { get; set; }
        }
        #endregion
        ...
        private const string Spselectnomeestadocount = @"
        SELECT  COUNT(*)
        FROM    plataforma.TAB_Cidade Cid WITH(NOLOCK) 
        WHERE   Cid.Nme_Cidade like @Nme_Cidade
                AND Cid.Sig_Estado = @Sig_Estado and Flg_Inativo = 0
        ...
        private const string Spselectnomecount = @"
        SELECT  COUNT(*)
        FROM    plataforma.TAB_Cidade Cid WITH(NOLOCK) 
        WHERE   Cid.Nme_Cidade like @Nme_Cidade
                AND Flg_Inativo = 0
        ...
        #region SP_RecuperarCidadesRegiaoMetropolitana
        private const string SP_RecuperarCidadesRegiaoMetropolitana = @"
			SELECT cid.* FROM plataforma.TAB_VW_Cidade_Regiao regiao
				JOIN plataforma.TAB_Cidade cid ON cid.Idf_Cidade = regiao.Idf_Cidade_Regiao
			WHERE regiao.Idf_Cidade =  @Idf_Cidade
            AND cid.Flg_Inativo = 0";
        #endregion

        private const string SpCarregarCidadesProximasPorGeolocalizacao = @"BNE.SP_Carregar_Cidades_Proximas_Por_Geolocalizacao";

        #region [spUpdateGeolocalizacao]
        private const string spUpdateGeolocalizacao = "update plataforma.tab_cidade set Geo_Localizacao = @Geo_Localizacao where idf_Cidade = @Idf_Cidade";
        #endregion
        ...
        public static bool CarregarPorNome(string nomeCidadeSiglaEstado, out Cidade objCidade, SqlTransaction trans = null)
        {
            if (!string.IsNullOrEmpty(nomeCidadeSiglaEstado))
            {
                ...
                if (HabilitaCache)
                {
                    if (string.IsNullOrWhiteSpace(nomeCidade) && string.IsNullOrWhiteSpace(siglaEstado))
                    {
                        ...
                        if (cidade != null)
                        {
                            objCidade = new Cidade { IdCidade = cidade.Identificador, NomeCidade = cidade.NomeCidade, Estado = new Estado(cidade.SiglaEstado), GeoLocalizacao = cidade.GeoLocalizacao };
                            retorno = true;
                        }
                        else
                            objCidade = null;
                ...
                else
                {
                    ...
                    dr = DataAccessLayer.ExecuteReader(trans,CommandType.Text, Spselectnomeestado, parms);
                }
        ...
        #region CarregarPorNomeCidadeSiglaEstado
        /// <summary>
        /// Método utilizado para retornar uma instância de Cidade a partir do banco de dados. Contempla o padrão "Cidade/SIGLAESTADO Ex.: Curitiba/PR"
        /// </summary>
        ...
        /// <param name="objCidade"> </param>
        /// <returns>objCidade</returns>
        /// <remarks>Gustavo Marty</remarks>
        public static bool CarregarPorNomeCidadeSiglaEstado(string nomeCidade, string siglaEstado, out Cidade objCidade)
        {
            if (!string.IsNullOrEmpty(nomeCidade) && !string.IsNullOrEmpty(siglaEstado))
            {
                var retorno = false;
                objCidade = new Cidade();
        ...
        /// <returns>objCidade</returns>
        /// <remarks>Gieyson Stelmak</remarks>
        public static bool Existe(string nomeCidadeSiglaEstado)
        {
            if (!string.IsNullOrEmpty(nomeCidadeSiglaEstado))
            {
                var retorno = false;
                ...
                {
                    parms.Add(new SqlParameter { ParameterName = "@Nme_Cidade", SqlDbType = SqlDbType.VarChar, Size = 80, Value = nomeCidadeSiglaEstado });
                    scalar = DataAccessLayer.ExecuteScalar(CommandType.Text, Spselectnomecount, parms);
                }
                else
                {
                    parms.Add(new SqlParameter { ParameterName = "@Nme_Cidade", SqlDbType = SqlDbType.VarChar, Size = 80, Value = nomeCidade });
                    parms.Add(new SqlParameter { ParameterName = "@Sig_Estado", SqlDbType = SqlDbType.Char, Size = 2, Value = siglaEstado });
                    scalar = DataAccessLayer.ExecuteScalar(CommandType.Text, Spselectnomeestadocount, parms);
                }
            ...
            if (HabilitaCache)
            {
                if (string.IsNullOrWhiteSpace(uf))
                    return Cidades.OrderBy(c => c.NomeCidade).Where(c => c.NomeCidade.NormalizarStringLINQ().StartsWith(nomeParcialCidade.NormalizarStringLINQ())).Take(numeroRegistros)
                       .OrderByDescending(c => c.SiglaEstado).ThenBy(c => c.NomeCidade)
        ...
        private static bool SetInstance(IDataReader dr, Cidade objCidade)
        {
            try
            {
                if (dr.Read())
                {
                    ...
                    if (dr["Geo_Localizacao"] != DBNull.Value)
                        objCidade._GeoLocalizacao = (SqlGeography)(dr["Geo_Localizacao"]);
        ...
        public static Cidade LoadObject(int idCidade)
        {
            ...
            if (HabilitaCache)
            {
                ...
                if (cidade != null)
                    return new Cidade { IdCidade = cidade.Identificador, NomeCidade = cidade.NomeCidade, Estado = new Estado(cidade.SiglaEstado), GeoLocalizacao = cidade.GeoLocalizacao };
        ...
        public bool CompleteObject()
        {
            ...
            if (HabilitaCache)
            {
                ...
                if (cidade != null)
                {
                    ...
                    this.GeoLocalizacao = cidade.GeoLocalizacao;
                    return true;
                }
                return false;
            }
        ...
        #region CarregarCidadesProximasPorGeolocalizacao
        public static DataTable CarregarCidadesProximasPorGeolocalizacao(int qtdeMaxRetorno, int? idCidade = null, string nomeCidade = null, string siglaEstado = null)
        {
            var parms = new List<SqlParameter>
                {
                    ...
                    new SqlParameter{ ParameterName = "@Nme_Cidade", SqlDbType = SqlDbType.VarChar, Size = 80, Value = nomeCidade},
            ...
            return DataAccessLayer.ExecuteReaderDs(CommandType.StoredProcedure, SpCarregarCidadesProximasPorGeolocalizacao, parms).Tables[0];
        }
        #endregion


        #region RecuperarCidadesRegiaoMetropolitana
        /// <summary>
        /// Retorna uma lista com as cidades da região metropolitana
        /// </summary>
        /// <param name="idfCidade"></param>
        /// <returns></returns>
        public static List<Cidade> RecuperarCidadesRegiaoMetropolitana(int idfCidade)
        {
            var parms = new List<SqlParameter>() 
            {
                new SqlParameter{ ParameterName = "@Idf_Cidade", SqlDbType = SqlDbType.Int, Size = 4,SqlValue = idfCidade}
            };

            var cidades = new List<Cidade>();
            using (var dr = DataAccessLayer.ExecuteReader(CommandType.Text, SP_RecuperarCidadesRegiaoMetropolitana, parms))
            {
                ...
                while (dr.Read())
                {
                    ...
                    if(SetInstance_NotDispose(dr,objCidade))
        ...
        #region Save
        /// <summary>
        /// Método utilizado para salvar uma instância de Cidade no banco de dados.
        /// </summary>
        /// <remarks>Gieyson Stelmak</remarks>
        public void Save()
        {
            ...
            Cache.RemoveItem(ObjectKey);
        }
        /// <summary>
        /// Método utilizado para salvar uma instância de Cidade no banco de dados, dentro de uma transação.
        /// </summary>
        /// <param name="trans">Transação existente no banco de dados.</param>
        /// <remarks>Gieyson Stelmak</remarks>
        ...
        #region [SalvarGeolocalizacao]
        public void SalvarGeolocalizacao()
        {
            List<SqlParameter> parms = new List<SqlParameter>(){
                new SqlParameter{ParameterName = "@Geo_Localizacao", UdtTypeName = "Geography", Value = this._GeoLocalizacao},
                new SqlParameter{ParameterName = "@Idf_Cidade", SqlDbType = SqlDbType.Int, Value = this._idCidade}
            };

            DataAccessLayer.ExecuteNonQuery(CommandType.Text, spUpdateGeolocalizacao, parms);
        }
        #endregion
