...
WHERE
        ...
        public void CriarParcelas(TipoPagamento objTipoPagamento, CodigoDesconto objCodigoDesconto, int? prazoBoleto)
        {
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        this.CalcularParcelas(DateTime.Now.AddDays(prazoBoleto ?? 0), DateTime.Now, null, objTipoPagamento, objCodigoDesconto, trans);
        ...
        {
            SalvarPlanoAdquiridoParcelaPagamento(objUsuarioFilialPerfil, objFilial, objPlano, objPlanoAdquiridoDetalhes, null, null, null);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        public void SalvarPlanoAdquiridoParcelaPagamento(UsuarioFilialPerfil objUsuarioFilialPerfil, Filial objFilial, Plano objPlano, PlanoAdquiridoDetalhes objPlanoAdquiridoDetalhes, DateTime? dataEnvioBoleto, DateTime? dataVencimentoBoleto, DateTime? dataEmissaoNFAntecipada)
        {
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                conn.Open();
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (dataEnvioBoleto.HasValue && dataVencimentoBoleto.HasValue)
                            pagamentos = CalcularParcelas(dataVencimentoBoleto.Value, dataEnvioBoleto.Value, dataEmissaoNFAntecipada, new TipoPagamento((int)Enumeradores.TipoPagamento.BoletoBancario), null, trans);
                        else //Ajustando parcelas a vencer
                        {
                            ...
                            foreach (var objPlanoParcela in parcelasEmAberto)
                            {
                                ...
                                if (objPagamentoAberto != null)
                                {
                                    ...
                                    if (dataEmissaoNFAntecipada.HasValue && objPagamentoAberto.DataVencimento.Value.Date > dataEmissaoNFAntecipada.Value.Date)//O vencimento tem que ser maior do que a data 
                                        objPlanoParcela.DataEmissaoNotaAntecipada = new DateTime(objPagamentoAberto.DataVencimento.Value.Year, objPagamentoAberto.DataVencimento.Value.Month, dataEmissaoNFAntecipada.Value.Day);
                                }
                            ...
                            if (pagamentos.Count > 0)
                            {
                                ...
                                if (pagamentos.Count > 1)
                                {
                                    if (objPlanoAdquiridoDetalhes != null && !string.IsNullOrWhiteSpace(objPlanoAdquiridoDetalhes.EmailEnvioBoleto))
                                    {
                                        ...
                                        string conteudo = BoletoBancario.CriarConteudoParaEnviarPorEmail(carta.Conteudo, BoletoBancario.GerarLayoutBoletoHTML(BoletoBancario.CriarBoletos(pagamentos)));
                                        MensagemCS.EnvioDeEmailComValidacao(TipoEnviadorEmail.Fila, carta.Assunto, conteudo, BLL.Enumeradores.CartaEmail.ConteudoBoletoVencimento, emailRemetente, objPlanoAdquiridoDetalhes.EmailEnvioBoleto);
                                    }
                                }
                            }
                        }
        ...
        public List<Pagamento> CalcularParcelas(DateTime dtVencimento, DateTime dtEnvioBoleto, DateTime? dtaEmissaoNotaAntecipada, TipoPagamento objTipoPagamento, CodigoDesconto objCodigoDesconto, SqlTransaction trans)
        {
            var pagamentos = new List<Pagamento>();
            ...
            if (this.Plano != null)
            {
                ...
                if (this.ParaPessoaFisica(trans))
                {
                    ...
                    else
                    {
                        ...
                        //Não liberar vip apra quem comprou o plando de candidatura premium
                        if (!objCurriculo.FlagVIP && this.Plano.IdPlano != Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.PlanoCandidaturaPremium)))
                        {
                            objCurriculo.FlagVIP = true;
                            objCurriculo.SituacaoCurriculo = new SituacaoCurriculo((int)Enumeradores.SituacaoCurriculo.AguardandoRevisaoVIP);
                            objCurriculo.Save(trans);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        #region [Vagas deixarem de ser premium]
                        try
                        {
                            if (this.Filial != null)
                            {
                                if (trans != null)
                                    BLL.ParametroVaga.ApagarParametroVagaPremium(trans, this.Filial.IdFilial);
                                else
                                    BLL.ParametroVaga.ApagarParametroVagaPremium(this.Filial.IdFilial);
                            }

                        }
                        catch (Exception ex)
                        {
                            EL.GerenciadorException.GravarExcecao(ex, "Erro - apagara as vaga premium de empresa que comprou premium -  Libera o pagamento");
                        }
                        #endregion
                    }
                }
        ...
        public static void SalvarNovaDataFimPlano(int idPlanoAdquirido, SqlTransaction trans = null)
        {
            PlanoAdquirido objPlanoAdquirido = PlanoAdquirido.LoadObject(idPlanoAdquirido);
            
            if (objPlanoAdquirido.DataFimPlano < DateTime.Today)
                objPlanoAdquirido.DataFimPlano = objPlanoAdquirido.DataFimPlano.AddMonths(1);
            else
                objPlanoAdquirido.DataFimPlano = DateTime.Today.AddMonths(1);
