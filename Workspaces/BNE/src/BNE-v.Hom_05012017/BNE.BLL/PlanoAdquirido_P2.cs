    ...
    public partial class PlanoAdquirido // Tabela: BNE_Plano_Adquirido
    {
                ...
                AND ( ( Flg_Recorrente = 0
                      ...
                      OR ( Flg_Recorrente = 1 -- Recorrente
                           AND CONVERT(VARCHAR, PA.Dta_Fim_Plano, 112) < CONVERT(VARCHAR, DATEADD(DAY,
                                                                      -7, GETDATE()), 112) -- cancela após a 3 cobrança no 7 dia no caso
        ...
        WHERE   PA.Idf_Plano_Situacao = 1 /* Liberado */
                ...
                AND ( ( Flg_Recorrente = 0
                      ...
                      OR ( Flg_Recorrente = 1
                           AND CONVERT(VARCHAR, PA.Dta_Fim_Plano, 112) < CONVERT(VARCHAR, DATEADD(DAY,-7,GETDATE()), 112)
                         )
                    )
                ";
        #endregion
        ...
        FROM    bne.BNE_Plano_Adquirido PA
                ...
                INNER JOIN BNE.TAB_Usuario_Filial_Perfil UFP ON UFP.Idf_Filial = PA.Idf_Filial
        ...
        FROM    bne.BNE_Plano_Adquirido PA
                ...
                INNER JOIN BNE.TAB_Usuario_Filial_Perfil UFP ON UFP.Idf_Filial = PA.Idf_Filial
		        LEFT JOIN BNE.BNE_Plano_Adquirido_Contrato PAC ON PAC.Idf_Plano_Adquirido = PA.Idf_Plano_Adquirido
                LEFT JOIN BNE.BNE_Plano_Adquirido_Contrato_Usuario PACU ON PACU.Idf_Plano_Adquirido_Contrato = PAC.Idf_Plano_Adquirido_Contrato AND PACU.Idf_Usuario_Filial_Perfil = UFP.Idf_Usuario_Filial_Perfil
        WHERE   Dta_Inicio_Plano > @DataImplantacaoAceiteContrato
                AND pa.Idf_Plano_Situacao = 1
		        AND P.Flg_Enviar_Contrato = 1
        ...
        WHERE   pa.Flg_Recorrente = 1 --1 => Com Recorrência
                AND ( ( ( CAST(Dta_Fim_Plano AS DATE) = CAST(GETDATE() AS DATE) -- Data fim igual
                      ...
                      OR ( DATEDIFF(DAY, Dta_Fim_Plano, GETDATE()) IN ( 0, 3, 6 ) -- Que esteja com 3
        ...
        public string Contrato()
        {
            ...
            {
                numeroParcelas = this.Plano.QuantidadeParcela;
            ...
            if (quantidadSMSTanque > 0)
                descricaoQuantidadaSMS += quantidadSMSTanque + " sms por dia;";
        ...
        #region EncerrarCandidaturaPremium
        /// <summary>
        /// Encerrar plano adquirido do plano de vaga premium 
        /// </summary>
        /// <param name="idPlanoAdquirido"></param>
        public static void EncerrarCandidaturaPremium(int idPlanoAdquirido)
        {
            var parms = new List<SqlParameter> 
                {
                    new SqlParameter{ ParameterName = "@idf_Plano_Situacao", SqlDbType = SqlDbType.Int, Size = 4, Value = (int)Enumeradores.PlanoSituacao.Encerrado},
                    new SqlParameter{ ParameterName = "@idf_Plano_Adquirido", SqlDbType = SqlDbType.Int, Size = 4, Value = idPlanoAdquirido}
                };
            DataAccessLayer.ExecuteNonQuery(CommandType.Text, spEncerrarCandidaturaPremium, parms);
        }
        #endregion
        #endregion
