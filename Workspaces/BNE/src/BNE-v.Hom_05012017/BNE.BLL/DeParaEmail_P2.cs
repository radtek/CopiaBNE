...
namespace BNE.BLL
{
    public partial class DeParaEmail // Tabela: plataforma.TAB_DePara_Email
    {
        ...
        private const string spselectbydomainat = " Select count(idf_Ranking_email) from bne_ranking_email with(nolock) where des_email = '@' + @DomainAt ";
        ...
        private const string spLista = " Select top 10 idf_Ranking_email, replace(Des_Email , '@','') as Des_Email  from bne_ranking_email wiht(nolock)";
        #endregion
        ...
        #region ValidaAutoComplete
        
        #endregion

        #region ExistsDomainWithAt
        /// <summary>
        /// Verifica se exite na tabela TAB_DePara_Email usada para corrigir o email
        /// </summary>
        ...
        #region ListaAt
        public static Dictionary<string, string> ListaAt()
        {
            #region Cache
            ...
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, spLista, null))
            {
                while (dr.Read())
                {
                    Lista.Add(dr["idf_Ranking_email"].ToString(), dr["Des_Email"].ToString());
                }
                if (!dr.IsClosed)
                    dr.Close();
            }
        ...
        #region ExistsDomainAt
        /// <summary>
        /// Veirifica se existe na tabela de bne_ranking_email a que lista no dropdownlist
        /// </summary>
        /// <param name="domain"></param>
        /// <returns></returns>
        public static bool ExistsDomainAt(string domain)
        {
            #region Cache
            ...
            var parms = new List<SqlParameter>
                {
                    new SqlParameter { ParameterName = "@DomainAt", SqlDbType = SqlDbType.VarChar, Size = 100, Value = domain }
                };

            return Convert.ToInt32(DataAccessLayer.ExecuteScalar(CommandType.Text, spselectbydomainat, parms)) > 0;
        }
        #endregion
