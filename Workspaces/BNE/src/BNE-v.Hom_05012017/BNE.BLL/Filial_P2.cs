...
namespace BNE.BLL
{
    public partial class Filial : ICloneable // Tabela: TAB_Filial
    {
        ...
        SET @iSelectPag = @iSelectPag + ' ORDER BY 3 ' 
...
WHERE  fa.Idf_Fato IS NULL
	...
	AND lm.idf_log_Envio_mensagem is null ";
        ...
        {
            return SituacaoFilial.IdSituacaoFilial.Equals((int)Enumeradores.SituacaoFilial.Bloqueado) || SituacaoFilial.IdSituacaoFilial.Equals((int)Enumeradores.SituacaoFilial.Cancelado);
        }
        #endregion
        ...
        public static DTO.AllInFilial CarregaFilialExportacaoAllIn(int idFilial, SqlTransaction trans)
        {
            ...
            try
            {
                ...
                foreach (DataRow item in result.Rows)
                {
                    ...
                    var objUsuarioFilialPerfil = new UsuarioFilialPerfil(idUsuFilialPerf);
                    objUsuarioFilialPerfil.CompleteObject(trans);

                    listPerf.Add(objUsuarioFilialPerfil);

                    if (objUsuarioFilialPerfil.PessoaFisica != null && objUsuarioFilialPerfil.PessoaFisica.IdPessoaFisica > 0)
                    {
                        objUsuarioFilialPerfil.PessoaFisica.CompleteObject(trans);
                    }

                    if (objUsuarioFilialPerfil.Perfil != null && objUsuarioFilialPerfil.Perfil.IdPerfil > 0)
                    {
                        objUsuarioFilialPerfil.Perfil.CompleteObject(trans);
                    }

                    UsuarioFilial objUsuarioFilial;
                    if (UsuarioFilial.CarregarUsuarioFilialPorUsuarioFilialPerfil(objUsuarioFilialPerfil.IdUsuarioFilialPerfil,
                        out objUsuarioFilial, trans))
                    {
                        objFilial.UsuariosFilial[objUsuarioFilialPerfil] = objUsuarioFilial;
                    }

                    var idPf = objUsuarioFilialPerfil.PessoaFisica != null ? objUsuarioFilialPerfil.PessoaFisica.IdPessoaFisica : -1;
                    Usuario usu;
                    if (Usuario.CarregarPorPessoaFisica(idPf, out usu, trans))
                    {
                        objFilial.Interacoes[objUsuarioFilialPerfil] = usu;
                    }

                    PesquisaCurriculo pesq;
                    if (PesquisaCurriculo.RecuperarUltimaPesquisa(idPf, out pesq, trans))
                    {
                        objFilial.Pesquisas[objUsuarioFilialPerfil] = pesq;
                    }
                }
