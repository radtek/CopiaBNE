...
WHERE  Dta_Visualizacao IS NULL
    ...
    AND Idf_Filial = @Idf_Filial ";
        ...
        public static bool VerificarVisualizacaoCV(int idCurriculo, int idFilial, int qtdDiasRevisualizacao, SqlTransaction trans)
        {
            var parms = new List<SqlParameter>
                {
            ...
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        CurriculoVisualizacao objCurriculoVisualizacao;
                        if (CurriculoVisualizacao.CarregarPorCurriculoFilial(objCurriculo, objPlanoAdquirido.Filial, diasRevisualizacao, trans, out objCurriculoVisualizacao))
                            podeVerDados = true;
                        else
                        {
                            if (objPlanoAdquirido.SaldoVisualizacao(trans) > 0)
