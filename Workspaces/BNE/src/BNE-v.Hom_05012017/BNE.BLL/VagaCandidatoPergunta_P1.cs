...
namespace BNE.BLL
{
	public partial class VagaCandidatoPergunta // Tabela: BNE_Vaga_Candidato_Pergunta
	{
		...
		private string _descricaoResposta;
		...
		public string DescricaoResposta
		{
			get
			{
				return this._descricaoResposta;
			}
			set
			{
				this._descricaoResposta = value;
				this._modified = true;
			}
		}
		#endregion 
		...
		#region Consultas
		private const string SPINSERT = "INSERT INTO BNE_Vaga_Candidato_Pergunta (Idf_Vaga_Pergunta, Idf_Vaga_Candidato, Flg_Resposta, Des_Resposta) VALUES (@Idf_Vaga_Pergunta, @Idf_Vaga_Candidato, @Flg_Resposta, @Des_Resposta);SET @Idf_Vaga_Candidato_Pergunta = SCOPE_IDENTITY();";
		private const string SPUPDATE = "UPDATE BNE_Vaga_Candidato_Pergunta SET Idf_Vaga_Pergunta = @Idf_Vaga_Pergunta, Idf_Vaga_Candidato = @Idf_Vaga_Candidato, Flg_Resposta = @Flg_Resposta, Des_Resposta = @Des_Resposta WHERE Idf_Vaga_Candidato_Pergunta = @Idf_Vaga_Candidato_Pergunta";
		...
		private const string SPSELECTID = "SELECT * FROM BNE_Vaga_Candidato_Pergunta WITH(NOLOCK) WHERE Idf_Vaga_Candidato_Pergunta = @Idf_Vaga_Candidato_Pergunta";
		#endregion
		...
		private List<SqlParameter> GetParameters()
		{
			...
			parms.Add(new SqlParameter("@Des_Resposta", SqlDbType.VarChar, 200));
			return(parms);
		}
		#endregion
		...
		private void SetParameters(List<SqlParameter> parms)
		{
			...
			if (!String.IsNullOrEmpty(this._descricaoResposta))
				parms[4].Value = this._descricaoResposta;
			else
		...
		public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
		{
			...
			string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, Vag.Idf_Vaga_Candidato_Pergunta, Vag.Idf_Vaga_Pergunta, Vag.Idf_Vaga_Candidato, Vag.Flg_Resposta, Vag.Des_Resposta FROM BNE_Vaga_Candidato_Pergunta Vag";
		...
		/// <returns>Verdadeiro ou falso informando se a operação foi executada com sucesso.</returns>
		/// <remarks>Gieyson Stelmak</remarks>
		private static bool SetInstance(IDataReader dr, VagaCandidatoPergunta objVagaCandidatoPergunta, bool dispose = true)
		{
			try
			{
				if (dr.Read())
				{
					...
					if (dr["Des_Resposta"] != DBNull.Value)
						objVagaCandidatoPergunta._descricaoResposta = Convert.ToString(dr["Des_Resposta"]);
