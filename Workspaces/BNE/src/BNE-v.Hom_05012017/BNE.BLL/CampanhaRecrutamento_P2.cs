            ...
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        objVagaTipoVinculo.TipoVinculo = TipoVinculo.LoadObject((int)Enumeradores.TipoVinculo.Efetivo);
                        objVagaTipoVinculo.Vaga = objVaga;
                        ...
                        #region Notificar Vendedor

                        try
                        {
                            objFilial.CompleteObject();
                            var vendedor = objFilial.Vendedor();
                            if (vendedor != null && !string.IsNullOrWhiteSpace(vendedor.EmailVendedor))
                            {
                                objUsuarioFilialPerfil.CompleteObject();
                                this.TipoRetornoCampanhaRecrutamento.CompleteObject();

                                string nomePlano = string.Empty;
                                PlanoAdquirido objPlanoAdquirido;
                                if (objFilial.PossuiPlanoAtivo() && PlanoAdquirido.CarregarPlanoAdquiridoPorSituacao(objFilial, (int)Enumeradores.PlanoSituacao.Liberado, out objPlanoAdquirido))
                                    nomePlano = objPlanoAdquirido.Plano.DescricaoPlano;

                                var parametros = new
                                {
                                    Consultor = vendedor.NomeVendedor,
                                    CNPJ = objFilial.CNPJ,
                                    NomeEmpresa = objFilial.RazaoSocial,
                                    Plano = string.IsNullOrWhiteSpace(nomePlano) ? "<br>" : string.Format("Plano: {0}<br><br>", nomePlano),
                                    Funcao = nomeFuncao,
                                    Cidade = nomeCidade,
                                    Salario = valorSalario,
                                    Usuario = objUsuarioFilialPerfil.PessoaFisica.NomeCompleto,
                                    QuantidadeRetorno = this.QuantidadeRetorno,
                                    TipoRetorno = this.TipoRetornoCampanhaRecrutamento.DescricaoTipoRetornoCampanhaRecrutamento,
                                    QuantidadeCurriculo = this.QuantidadeCurriculosNaBase()
                                };

                                string assunto;
                                var carta = CartaEmail.RetornarConteudoBNE(Enumeradores.CartaEmail.NotificacaoCampanhaCriada, out assunto);
                                var mensagem = parametros.ToString(carta);
                                var remetente = Parametro.RecuperaValorParametro(Enumeradores.Parametro.EmailMensagens);

                                EmailSenderFactory.Create(TipoEnviadorEmail.Fila).Enviar(assunto, mensagem, Enumeradores.CartaEmail.NotificacaoCampanhaCriada, remetente, objUsuarioFilial.EmailComercial);
                            }
                        }
                        catch (Exception ex)
                        {
