  ...
  and CHARINDEX('@',tab.Des_Email_Destino) > 0";
        ...
        public static void EnvioSMSTanque(string idUsuarioFilialPerfilRemetente, List<DTO.PessoaFisicaEnvioSMSTanque> listaMensagens, bool agendar = false)
        {
            using (var objWsTanque = new BNETanqueService.AppClient())
            {
                ...
                {
                    var numeroCelular = Convert.ToDecimal(objMensagem.dddCelular.Trim() + objMensagem.numeroCelular.Trim());
                    var mensagem = new BNETanqueService.Mensagem
                    {
                        ci = Convert.ToString(objMensagem.idDestinatario),
                        np = objMensagem.nomePessoa,
                        nc = numeroCelular,
                        dm = objMensagem.mensagem
                    };
                ...
                var receberMensagem = new BNETanqueService.InReceberMensagem
                    {
                        l = listaSMS.ToArray(),
                        cu = idUsuarioFilialPerfilRemetente,
                        e = agendar
                    };
