...
namespace BNE.BLL
{
    public partial class Curriculo : ICloneable // Tabela: BNE_Curriculo
    {
        ...
        #region SpCarregarCurriculoparaWebServiceAllin
        private const string SpCarregarCurriculoparaWebServiceAllin = @"
                    SELECT Eml_Pessoa,
                           Idf_Curriculo,
	                       Des_Tipo_Curriculo,
	                       Num_CPF,
                      ...
                      JOIN plataforma.TAB_Cidade cid WITH (NOLOCK) ON cv.Idf_Cidade_Endereco = cid.Idf_Cidade
                      JOIN BNE.BNE_Tipo_Curriculo TCV ON cv.Idf_Tipo_Curriculo = TCV.Idf_Tipo_Curriculo
                      JOIN BNE.TAB_Pessoa_Fisica PF WITH (NOLOCK) ON cv.Idf_Pessoa_Fisica = PF.Idf_Pessoa_Fisica
                      CROSS APPLY (SELECT TOP 1 Dta_Demissao 
                      ...
                      JOIN BNE.TAB_Usuario_Filial_Perfil ufp WITH (NOLOCK) ON PF.Idf_Pessoa_Fisica = ufp.Idf_Pessoa_Fisica AND ufp.Flg_Inativo = 0
                      LEFT JOIN BNE.BNE_Plano_Adquirido pa WITH (NOLOCK) ON ufp.Idf_Usuario_Filial_Perfil = pa.Idf_Usuario_Filial_Perfil 
											                       AND pa.Idf_Plano_Situacao = 1 AND GETDATE() BETWEEN Dta_Inicio_Plano AND Dta_Fim_Plano
                      JOIN plataforma.TAB_Sexo s ON PF.Idf_Sexo = s.Idf_Sexo
                      CROSS APPLY (SELECT COUNT(*) QTDE 
				                    FROM BNE.BNE_Curriculo_Quem_Me_Viu qmv WITH (NOLOCK)
			                       WHERE qmv.Idf_Curriculo = CV.Idf_Curriculo
			                      )qmv_cv
        ...
        public void GatilhoCadastroBNE()
        {
            try
            {
                ...
                ProcessoAssincrono.IniciarAtividade(AsyncServices.Enumeradores.TipoAtividade.GatilhosBNE, parametros);
            }
            catch (Exception ex)
            {
            ...
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (situacaoCurriculo.Equals(Enumeradores.SituacaoCurriculo.ExclusaoLogica))
                            objPessoaFisica.AtualizandoCadastroExlusaoLogicaParaAtivo(objPessoaFisica, this, objPessoaFisicaComplemento, trans);
                        ...
                        if (novoCadastro)
                        {
                            ...
                            if (!String.IsNullOrEmpty(PessoaFisica.EmailPessoa))
                            {
                                ...
                                PreCadastro.Delete(PessoaFisica.EmailPessoa, PessoaFisica.NomeCompleto, (int)OrigemPreCadastro.PoPup);
        ...
        /// <summary>
        /// M�todo respons�vel por salvar um curr�culo na primeira 
        /// tela de cadastro de curr�culo dentro de uma transação.
        /// </summary>
        /// <param name="objPessoaFisica"></param>
        /// <param name="listFuncaoPretendida"></param>
        /// <param name="objOrigem"></param>
        ...
        /// <param name="objOrigemSecundaria">Fornece uma segunda origem para o currículo, utilizado quando o currículo vem da LANHOUSE, onde ele precisa ganhar Origem da Filial e da LANHOUSE</param>
        /// <remarks>Gieyson Stelmak</remarks>
        public void SalvarMiniCurriculo(SqlTransaction trans, PessoaFisica objPessoaFisica, List<FuncaoPretendida> listFuncaoPretendida,
            ...
            PessoaFisicaFoto objPessoaFisicaFoto, ProfileFacebook.DadosFacebook dadosFacebook, bool celularValidado = false, Origem objOrigemSecundaria = null)
        {
            ...
            try
            {
                ...
                if (novoCadastro && !String.IsNullOrEmpty(PessoaFisica.EmailPessoa))
                {
                    ...
                    //deleta ta tabela pre cadastro caso tiver.
                    PreCadastro.Delete(PessoaFisica.EmailPessoa, PessoaFisica.NomeCompleto, (int)Enumeradores.OrigemPreCadastro.PoPup);
        ...
        public void SalvarDadosPessoais(SqlTransaction trans, PessoaFisica objPessoaFisica, PessoaFisicaComplemento objPessoaFisicaComplemento, Contato objContatoTelefone, Contato objContatoCelular, List<PessoaFisicaRedeSocial> listRedeSocial, ExperienciaProfissional objExperienciaProfissional1, ExperienciaProfissional objExperienciaProfissional2, ExperienciaProfissional objExperienciaProfissional3, ExperienciaProfissional objExperienciaProfissional4, ExperienciaProfissional objExperienciaProfissional5, ExperienciaProfissional objExperienciaProfissional6, ExperienciaProfissional objExperienciaProfissional7, ExperienciaProfissional objExperienciaProfissional8, ExperienciaProfissional objExperienciaProfissional9, ExperienciaProfissional objExperienciaProfissional10, Enumeradores.SituacaoCurriculo? situacaoCurriculo = null)
        {
            ...
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        ExcluirCVSolr(objCurriculo.IdCurriculo);
        ...
        /// <param name="idCurriculo"></param>
        /// <returns></returns>
        public static bool HibernarCurriculo(int idCurriculo)
        {
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                conn.Open();
            ...
            using (SqlConnection conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        BufferAtualizacaoCurriculo.Update(this);
                        BufferAtualizacaoCurriculoAllin.Add(this._idCurriculo);
                    }
                    catch
                    {
                        trans.Rollback();
        ...
        public static DTO.AllInCurriculo CarregarCurriculoExportacaoAllIn(int idCurriculo, SqlTransaction trans)
        {
            ...
            try
            {
                ...
                objCurriculo.QuemMeViuQuantidade = CurriculoQuemMeViu.QuantidadeVisualizacaoCurriculo(new Curriculo(objCurriculo.IdCurriculo));
        ...
        /// <returns></returns>
        public static DTO.Curriculo CarregarCurriculoSolr(int idCurriculo, string palavraChave, bool carregaEmail = false)
        {
            ...
            urlCVSolr = urlCVSolr.Replace("{palavraChave}", string.IsNullOrEmpty(palavraChave) ? "*%3A*" : palavraChave);
            ...
            return MapearCurriculo(resultado.response.docs[0], resultado.highlighting, carregaEmail);
        }
        #endregion

        public static DTO.Curriculo MapearCurriculo(CurriculoCompleto.Response curriculo, Dictionary<string, Solr.Highlight> highlight, bool carregaEmail = false)
        {
        ...
        private string MontarHTMLCurriculo(int? idFilial, int? idUsuarioFilialPerfil, int? idVaga, bool mostrarFoto, bool mostrarDados, bool mostrarLogoCliente, bool mostrarObservacoes, bool adicionarLinkBotaoDadosCompletos, FormatoVisualizacaoImpressao formatoVisualizacao, decimal? cpf, DateTime? dtaNascimento)
        {
            ...
            //string str1 = string.Format("http://{0}/Handlers/LogoBNERegistrarQuemMeViu.ashx?idFilial={1}&idCurriculo={2}", Helper.RecuperarURLAmbiente(), (idFilial != null ? idFilial.Value.ToString() : "0"), curriculo.IdCurriculo);
            ...
            if (curriculo.Experiencias != null && curriculo.Experiencias.Count > 0)
            {
                ...
                foreach (DTO.ExperienciaProfissional experienciaProfissional in curriculo.Experiencias)
                {
                    ...
                    stringBuilder.Append(experienciaProfissional.RazaoSocial + " - de " + experienciaProfissional.DataAdmissao.ToShortDateString() + " até " + (experienciaProfissional.DataDemissao.HasValue ? experienciaProfissional.DataDemissao.Value.ToShortDateString() : "hoje (Emprego Atual)") +
                        " (" + Helper.CalcularTempoEmprego(experienciaProfissional.DataAdmissao.ToString("dd/MM/yyyy HH:mm:ss"), (experienciaProfissional.DataDemissao.HasValue ? experienciaProfissional.DataDemissao.Value.ToShortDateString() : DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss"))) + ")");
                    stringBuilder.Append("</td>");
                    stringBuilder.Append("</tr>");
                    stringBuilder.Append("<tr>");
        ...
        public static List<PessoaFisicaEnvioSMSTanque> RecuperarListaCvsEnvio(int idCidade, int idFuncao, bool estagio)
        {
            ...
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.StoredProcedure, "bne.BNE_SP_Lista_Curriculo_Vaga", parms))
            {
                while (dr.Read())
                {
                    listaCurriculos.Add(new PessoaFisicaEnvioSMSTanque
                    {
                        ...
                        numeroCPF = Convert.ToDecimal(dr["Num_CPF"]),
                        emailPessoa = Convert.ToString(dr["Eml_Pessoa"]),
        ...
        #region CarregarCurriculoparaWebServiceAllin
        /// <summary>
        /// Método usado para carregar um Currículo com os campos necessário para o WeService do Allin
        /// </summary>
        /// <returns>DataTable</returns>
        /// <remarks>Fabiano Charan</remarks>
        public static DataTable CarregarCurriculoparaWebServiceAllin(int idCurriculo)
        {
            var parms = new List<SqlParameter>
                {
                    new SqlParameter { ParameterName = "@Idf_Curriculo", SqlDbType = SqlDbType.Int, Size = 4, Value = idCurriculo }
            ...
            return DataAccessLayer.ExecuteReaderDs(CommandType.Text, SpCarregarCurriculoparaWebServiceAllin, parms).Tables[0];
        }
        #endregion
        ...
        public static void ExcluirCVSolr(int Idf_Curriculo)
        {
        ...
        #region AtualizarCVSolr
        /// <summary>
        /// Atualzar Cv no solr
        /// </summary>
        /// <param name="Idf_Curriculo"></param>
        private static void AtualizarCVSolr(int Idf_Curriculo)
        {
