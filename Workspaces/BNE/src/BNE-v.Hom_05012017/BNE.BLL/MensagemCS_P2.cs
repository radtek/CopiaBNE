...
namespace BNE.BLL
{
    public partial class MensagemCS // Tabela: BNE_Mensagem_CS
    {
                ...
                foreach (var kvp in dicCurriculo)
                {
                    ...
                    if (descontarVisualizacao)
                    {
                        ...
                        if (!objCurriculo.FlagVIP)
                        {
                            ...
                            if (!CurriculoVisualizacao.CarregarPorCurriculoFilial(objCurriculo, objFilial, diasRevisualizacao, null, out objCurriculoVisualizacao))
          ...
          string assunto, string mensagem, Enumeradores.CartaEmail? carta, string emailRemetente, string emailDestinatario, string nomeArquivoAnexo, byte[] arquivoAnexo, SqlTransaction trans)
        {
            ...
            try
            {
                ProcessoAssincrono.IniciarAtividade(TipoAtividade.EnvioEmail, parametros, objMensagem.ArquivoAnexo, objMensagem.NomeAnexo);
            }
            catch (Exception ex)
            {
            ...
            else//não vip enviar e-mail
            {
                ...
                try
                {
                    if (!string.IsNullOrEmpty(objPessoaFisica.EmailPessoa))
                    {
                        ...
                        Enumeradores.CartaEmail CartaLayout = CartaLayout = Enumeradores.CartaEmail.QuemMeViuSmile; ;
                        switch (++ultimaCarta)
                        {
                        ...
                        var Carta = CartaEmail.LoadObject((int)CartaLayout);
                        var emailRemetente = Parametro.RecuperaValorParametro(Enumeradores.Parametro.ContaPadraoEnvioEmail);
                        Carta.ValorCartaEmail = Carta.ValorCartaEmail.Replace("{NomeCompleto}", objPessoaFisica.NomeCompleto.Trim());
                        Carta.ValorCartaEmail = Carta.ValorCartaEmail.Replace("{link}",
                            LoginAutomatico.GerarHashAcessoLogin(objPessoaFisica.CPF, objPessoaFisica.DataNascimento, "/" + Rota.RecuperarURLRota(Enumeradores.RouteCollection.QuemMeViuSemPlano)));

                        //Enviar E-mail para o candidato
                        EmailSenderFactory.Create(TipoEnviadorEmail.Fila)
                            .Enviar(Carta.DescricaoAssunto, Carta.ValorCartaEmail, CartaLayout, emailRemetente,
