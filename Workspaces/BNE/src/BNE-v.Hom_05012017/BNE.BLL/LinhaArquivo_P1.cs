...
namespace BNE.BLL
{
    public partial class LinhaArquivo // Tabela: BNE_Linha_Arquivo
    {
        ...
        private BoletoBancario _boletoBancario;
        ...
        /// Campo opcional.
        /// </summary>
        public BoletoBancario BoletoBancario
        {
            get
            {
                return this._boletoBancario;
            }
            set
            {
                this._boletoBancario = value;
                this._modified = true;
            }
        }
        #endregion
        ...
        #region Consultas
        private const string SPINSERT = "INSERT INTO BNE_Linha_Arquivo (Idf_Arquivo, Idf_Transacao, Num_Linha, Idf_Boleto_Bancario, Idf_Tipo_Linha_Arquivo, Des_Conteudo, Des_Mensagem_Liberacao) VALUES (@Idf_Arquivo, @Idf_Transacao, @Num_Linha, @Idf_Boleto_Bancario, @Idf_Tipo_Linha_Arquivo, @Des_Conteudo, @Des_Mensagem_Liberacao);SET @Idf_Linha_Arquivo = SCOPE_IDENTITY();";
        private const string SPUPDATE = "UPDATE BNE_Linha_Arquivo SET Idf_Arquivo = @Idf_Arquivo, Idf_Transacao = @Idf_Transacao, Num_Linha = @Num_Linha, Idf_Boleto_Bancario = @Idf_Boleto_Bancario, Idf_Tipo_Linha_Arquivo = @Idf_Tipo_Linha_Arquivo, Des_Conteudo = @Des_Conteudo, Des_Mensagem_Liberacao = @Des_Mensagem_Liberacao WHERE Idf_Linha_Arquivo = @Idf_Linha_Arquivo";
        ...
        private void SetParameters(List<SqlParameter> parms)
        {
            ...
            if (this._boletoBancario != null)
                parms[4].Value = this._boletoBancario.IdBoletoBancario;
            else
        ...
        public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
        {
            ...
            string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, Lin.Idf_Linha_Arquivo, Lin.Idf_Arquivo, Lin.Idf_Transacao, Lin.Num_Linha, Lin.Idf_Boleto_Bancario, Lin.Idf_Tipo_Linha_Arquivo, Lin.Des_Conteudo, Lin.Des_Mensagem_Liberacao FROM BNE_Linha_Arquivo Lin";
        ...
        private static bool SetInstance(IDataReader dr, LinhaArquivo objLinhaArquivo)
        {
            try
            {
                if (dr.Read())
                {
                    ...
                    if (dr["Idf_Boleto_Bancario"] != DBNull.Value)
                        objLinhaArquivo._boletoBancario = new BoletoBancario(Convert.ToInt32(dr["Idf_Boleto_Bancario"]));
