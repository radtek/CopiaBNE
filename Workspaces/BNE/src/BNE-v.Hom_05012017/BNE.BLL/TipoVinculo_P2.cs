...
namespace BNE.BLL
{
    public partial class TipoVinculo // Tabela: BNE_Tipo_Vinculo
    {
        ...
        #region SpselectporIdVaga
        private const string SpselectporIdVaga = @"
        select vtv.Idf_Tipo_Vinculo
        from bne.bne_vaga_tipo_vinculo vtv with(nolock)
        where vtv.Idf_Vaga = @Idf_Vaga
        ";
        #endregion
        ...
        #region CarregarPorIdfVaga
        public static int CarregarPorIdfVaga(int idVaga, SqlTransaction trans = null)
        {
            ...
            var ret = trans == null
                ? DataAccessLayer.ExecuteScalar(CommandType.Text, SpselectporIdVaga, parms)
                : DataAccessLayer.ExecuteScalar(trans, CommandType.Text, SpselectporIdVaga, parms);
