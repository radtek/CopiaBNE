...
namespace BNE.Web.Vagas.Controllers
{
    public class VagaController : BaseController
    {
        ...
        public ActionResult VisualizarVaga(int identificador)
        {
            ...
            model.Vaga.Premium = ParametroVaga.Premium((int)Enumeradores.Parametro.VagaPremium, objVaga.IdVaga);
        ...
        public ActionResult VisualizarVagaEmpresa(int identificador)
        {
            ...
            if (vagaTipoVinculo.Count > 0)
            {
                foreach (var objVagaTipoVinculo in vagaTipoVinculo)
                {
                    var objTipoVinculo = TipoVinculo.LoadObject(objVagaTipoVinculo.TipoVinculo.IdTipoVinculo);
                    model.Vaga.TipoVinculo += string.Format("{0} ", objTipoVinculo.DescricaoTipoVinculo);
                }
            }
            #endregion
            ...
            if(model.Vaga.ContratoDeEstagio)
                model.Vaga.URL = SitemapHelper.MontarUrlVagaEstagio(true,model.Vaga.Funcao, model.Vaga.AreaBNE, model.Vaga.NomeCidade, model.Vaga.SiglaEstado, model.Vaga.IdentificadorVaga);
            else
                model.Vaga.URL = SitemapHelper.MontarUrlVaga(model.Vaga.Funcao, model.Vaga.AreaBNE, model.Vaga.NomeCidade, model.Vaga.SiglaEstado, model.Vaga.IdentificadorVaga);
        ...
        public ActionResult Candidatar(VagaCandidatura model, string button, ExperienciaProfissional modelExp, PerguntaEmail modelEmail, PerguntaCelular modelCelular)
        {
            try
            {
                ...
                if (base.IdUsuarioFilialPerfilLogadoCandidato.HasValue)
                {
                    ...
                    //validar se o candidato tem experiência profissional e formação
                    //BLL.ExperienciaProfissional experiencia = BLL.ExperienciaProfissional.CarregarUltimaExperienciaProfissional(base.IdPessoaFisicaLogada.Value);
                    ...
                    foreach (DataRow row in dtaInformacoesPessoais.Rows)
                    {
                        ...
                        //informacoesCurriculo.SaldoCandidatura = (row[7] != ""? Convert.ToInt32(row[7]) : 0);
                    ...
                    var objVaga = BLL.Vaga.LoadObject(model.IdentificadorVaga);
                    ...
                    model.Premium = new VagaCandidatura.PremiumCandidatura { premium = ParametroVaga.Premium((int)Enumeradores.Parametro.VagaPremium, objVaga.IdVaga) };

                    if (model.Premium.premium && !objCurriculo.FlagVIP)
                    {

                        if (respostasCandidato.Count > 0)
                        {
                            //Cookie para guardar o id da pesquisa para o novo BNE
                            HttpCookie cookie = new HttpCookie(objVaga.IdVaga.ToString());
                            foreach (var pergunta in model.Perguntas.Perguntas)
                            {
                                cookie.Value = cookie.Value + pergunta.Id.ToString() + ',';
                            }
                            cookie.Expires = DateTime.Now.AddMinutes(Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.CookieAcessoHorasExpiracao)));//colocar no parametro
                            Response.Cookies.Add(cookie);

                        }
                        FuncaoCategoria objFuncaoCategoria = FuncaoCategoria.RecuperarCategoriaPorCurriculo(objCurriculo);
                        var PrecoV = Plano.LoadObject(Plano.RecuperarCodigoPlanoMensalPorFuncaoCategoria(objFuncaoCategoria)).ValorBase.ToString();

                        var PrecoC = Plano.LoadObject(Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.PlanoCandidaturaPremium))).ValorBase.ToString();
                        base.IdVaga.Value = objVaga.IdVaga;
                        return PartialView("_VagaCandidatura", new VagaCandidatura { Candidatou = false, IdentificadorVaga = model.IdentificadorVaga, Premium = new VagaCandidatura.PremiumCandidatura { PrecoCandidatura = PrecoC, PrecoVip = PrecoV }, FlgVagaArquivada = model.FlgVagaArquivada, FlgInativa = model.FlgInativa });
                    }
                    #endregion
