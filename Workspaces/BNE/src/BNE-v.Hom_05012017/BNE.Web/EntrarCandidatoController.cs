...
namespace BNE.Web
{
    public class EntrarCandidatoController : Controller
    {
        ...
        public ActionResult Usando(string ret, int i, string c, DateTime d, string utm_source, string utm_medium, string utm_campaign, string utm_term, string palavra_chave, string codCa, string codPa)
        {
            ret = string.Format("{0}{1}{2}{3}{4}{5}{6}{7}",
            ret,
            !string.IsNullOrEmpty(utm_source) ? "?utm_source=" + utm_source : string.Empty,
            !string.IsNullOrEmpty(utm_medium) ? "&utm_medium=" + utm_medium : string.Empty,
            !string.IsNullOrEmpty(utm_campaign) ? "&utm_campaign=" + utm_campaign : string.Empty,
            !string.IsNullOrEmpty(utm_term) ? "&utm_term=" + utm_term : "",
            !string.IsNullOrEmpty(palavra_chave) ? "&palavra_chave=" + palavra_chave : string.Empty,
            !string.IsNullOrEmpty(codCa) ? "&codCa=" + codCa : string.Empty,//idCarrinhoAbandonado
            !string.IsNullOrEmpty(codPa) ? "&codPa=" + codPa : string.Empty);//idf do plano adquirido
        ...
        private ActionResult ProcessarLogin(string ret, int curriculoId, string cpfDesc, DateTime dtNasc, int? pfId = null, bool acceptRedirect = false)
        {
            ...
            try
            {
                ...
                if (!BNE.BLL.UsuarioFilialPerfil.CarregarUsuarioFilialPerfilCandidatoAtivo(objPessoaFisica, out objUsuarioFilialPerfil))
                    //return new HttpStatusCodeResult(System.Net.HttpStatusCode.Unauthorized);
                    return new RedirectToRouteResult(BNE.BLL.Enumeradores.RouteCollection.LoginComercialCandidato.ToString(), new System.Web.Routing.RouteValueDictionary { { RedirectParamArg, ret } });
                ...
                if (IdCurriculo.HasValue)
                {
                    ...
                    {
                        gravarOrigem(IdUsuarioFilialPerfilLogadoCandidato.Value);
                ...
                gravarOrigem(objUsuarioFilialPerfil.IdUsuarioFilialPerfil);
                IdUsuarioFilialPerfilLogadoCandidato.Value = objUsuarioFilialPerfil.IdUsuarioFilialPerfil;
                if (objCurriculo.SituacaoCurriculo.IdSituacaoCurriculo.Equals((int)BLL.Enumeradores.SituacaoCurriculo.Invisivel)){
                    objCurriculo.SituacaoCurriculo = new BLL.SituacaoCurriculo((int)BLL.Enumeradores.SituacaoCurriculo.AguardandoPublicacao);
                    objCurriculo.Salvar();
                }
        ...
        #region GravarOrigem
        private void gravarOrigem(int idUsuarioFilialPerfil)
        {
            ...
            bp.GravarOrigemAcesso(idUsuarioFilialPerfil);
        ...
        private bool IsValidParams(int i, string c, DateTime d, out HttpStatusCodeResult res)
        {
            ...
            c = new string(c.Where(a => char.IsNumber(a)).ToArray()).PadLeft(11, '0');
            if (c.Length != 11)
            {
