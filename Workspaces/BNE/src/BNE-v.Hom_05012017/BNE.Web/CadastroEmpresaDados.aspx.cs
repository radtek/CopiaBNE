...
namespace BNE.Web
{
    public partial class CadastroEmpresaDados : BasePage
    {
        ...
        public ControlCNPJReceitaFederal.DadosCNPJReceitaFederal CadastroEmpresaDadosDadosReceita
        {
            get
            {
                if (ViewState["DadosCNPJReceitaFederal"] != null)
                    return (ControlCNPJReceitaFederal.DadosCNPJReceitaFederal)(ViewState["DadosCNPJReceitaFederal"]);
                return null;
            }
            set
            {
                ViewState.Add("DadosCNPJReceitaFederal", value);
            }
        }
        #endregion
        ...
        protected void txtCNPJ_ProblemaComunicacao(object sender, Exception ex)
        {
        ...
        private void ValorCNPJAlterado(ControlCNPJReceitaFederal.DadosCNPJReceitaFederal dadoscnpj)
        {
            ...
            int? idFilial = null;
            if (txtCNPJ.numeroCNPJ.HasValue)
                idFilial = Filial.RecuperarIdentificador(txtCNPJ.numeroCNPJ.Value);

            if (idFilial.HasValue && !IdUsuarioFilialPerfilLogadoUsuarioInterno.HasValue)
            {
                CadastroEmpresaDadosIdFilial = idFilial;
        ...
        private void Salvar()
        {
            ...
            if (CadastroEmpresaDadosDadosReceita != null)
            {
                ...
                {
                    ExibirMensagem("Cidade inválida! Por favor entre em contato 0800 41 2400", TipoMensagem.Erro);
                    return;
                    //é possível que essa mensagem seja exibida caso a Cidade que consta no cadastro da Receita Federal não seja encontrada por alguma diferença, um hífen, espaço a mais, a menos etc. Ver "MUNICÍPIO" e "UF" em ControlCNPJReceitaFederal.cs
                }
            }
            ...
            if (pnlPerfilAdministrador.Visible)
            {
                ...
                if (IdOrigem.HasValue)
                {
                    ...
                    objFilial.TipoParceiro = TipoParceiro.LoadObject(Convert.ToInt32(rcbTipoParceiro.SelectedValue));
                }
                else
                {
        ...
        {
            txtCNPJ.ExibirValidadorReceitaFederal = false;
        ...
        private void PreencherCampos()
        {
            ...
            if (objFilial != null)
            {
                ...
                if (pnlPerfilAdministrador.Visible)
                {
                    ...
                    txtCNPJ.ExibirValidadorReceitaFederal = objFilial.SituacaoFilial.IdSituacaoFilial == (int)Enumeradores.SituacaoFilial.FaltaDadosReceita;
                }
                ...
                if (objFilial.SituacaoFilial.IdSituacaoFilial == (int)Enumeradores.SituacaoFilial.FaltaDadosReceita && CadastroEmpresaDadosDadosReceita != null)
                {
                    if (CadastroEmpresaDadosDadosReceita.Ativa)
                        PreencherDadosReceitaFederal(CadastroEmpresaDadosDadosReceita);
                    else
        ...
        private void PreencherDadosReceitaFederal(ControlCNPJReceitaFederal.DadosCNPJReceitaFederal dados)
        {
            ...
            txtNomeFantasia.Valor = !string.IsNullOrWhiteSpace(dados.NomeFantasia) ? dados.NomeFantasia.Trim() : dados.RazaoSocial.Trim();
