...
/*
 *Controle do tracker do chat comercial
*/
//Verifica se o tracker deve ser iniciado
function verificaTracker() {
    _wtk_first_time = false;
    _wtk_track_interval = 5000;

    var TRACKER_CONTROLLER_SESSION_ID = getCookie("TRACKER_CONTROLLER_SESSION_ID");
    if (TRACKER_CONTROLLER_SESSION_ID != null && TRACKER_CONTROLLER_SESSION_ID == GetSession()) {
        iniciarTrack(getCookie("TRACKER_CONTROLLER_DESCRIPTION"));
    }
}

//Funcao para iniciar o track, gravando no cookie a requisição
function iniciarTrack(description) {
    _webtrack_ignore = false;
    _wtk_the_src = _wtk_the_src.replace(/&info\[page_title\]=.*?&/i, "&info[page_title]=" + description + "&");
    WTK_do_Track();

    setCookie("TRACKER_CONTROLLER_DESCRIPTION", description, null);
    setCookie("TRACKER_CONTROLLER_SESSION_ID", GetSession(), null);
}
/*
...
var autocomplete = {
    ...
    type: {
        cidade: 'Cidade', bairro: 'Bairro', unicoBairro: 'unicoBairro', funcao: 'Funcao'
    },
    URL: {
        URLCompleteCidade: '',
        URLCompleteFuncao: '',
...
function openModalVideo() {
    $('#frame-modal-video').show();
    $('#preview-video').hide();
    $('#openVideoTutorial').show();
    setCookie(document.getElementById('ucBreadCrumbs_hdfVideo').value, 4);
}

function closeImgVideo() {
    $('#preview-video').hide();
    $('#openVideoTutorial').show();
    setCookie(document.getElementById('ucBreadCrumbs_hdfVideo').value, 4);
}
function closeModalVideo() {
    $('#frame-modal-video').hide();
   
    document.getElementsByClassName('setVideo')[0].contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*');
}

$(document).ready(function () {
    $("#openVideoTutorial").on("mouseover", function () {
        $("#openVideoTutorialMaster-tooltip").show();
    }).on("mouseout", function () {
        $("#openVideoTutorialMaster-tooltip").hide();
    });
   
    if (getCookie(document.getElementById('ucBreadCrumbs_hdfVideo').value)) {
        $('#preview-video').hide();
        $('#openVideoTutorial').show();
    }
    else {
        $('#preview-video').show();
        $('#openVideoTutorial').hide();
        }
});
