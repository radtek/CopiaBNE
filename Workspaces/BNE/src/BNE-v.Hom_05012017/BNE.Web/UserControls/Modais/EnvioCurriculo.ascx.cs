...
namespace BNE.Web.UserControls.Modais
{
    public partial class EnvioCurriculo : BaseUserControl
    {
        ...
        public void Inicializar()
        {
            ...
            {
                hdfIdUfp.Value = base.IdUsuarioFilialPerfilLogadoEmpresa.Value.ToString();
                txtEnvioPara.Text = EmailEnvioCurriculo.UltimoEmailUtilizadoParaEnvio(base.IdUsuarioFilialPerfilLogadoEmpresa.Value);
            }
        ...
        {
            string[] emailPara ;
                emailPara = txtEnvioPara.Text.Split(';', ':', ','); 
            var listEmail = new List<string>();
            int cont = 0;

            foreach (string p in emailPara)
            {
                var email = TratarEmail(p);
        ...
        public void Salvar(Filial objFilial)
        {
            ...
            if (base.IdUsuarioFilialPerfilLogadoEmpresa.HasValue)
            {
                foreach (var email in listEmailDestino)
                    EmailEnvioCurriculo.ExisteNaLista(email, base.IdUsuarioFilialPerfilLogadoEmpresa.Value);

                EmailEnvioCurriculo.UpdateUltimoEnviado(base.IdUsuarioFilialPerfilLogadoEmpresa.Value, listEmailDestino[0]);
            }
        ...
        public void LimparCampos()
        {
            ...
            hdfEmailEnviar.Value = string.Empty;
        ...
        private void ValidarEEfetuarEnvioEmpresa()
        {
            ...
            if (!cbOcultarDados.Checked) //Só faz a validação caso necessite que os dados da pessoa fisica deva ser mostrado.
            {
                ...
                int saldoVisualizacaoFilial = objFilial.SaldoVisualizacao();
        ...
        public string ValidarDestinatarios(string emailPara)
        {
            string[] emails = emailPara.Split(';', ':', ',');
            string emailErro = string.Empty;
            int cont = 0;
            foreach (string t in emails)//Valida o email
            {
                var email = TratarEmail(t);
                if (!ValidarEmail(t))
                {
                    emailErro += " " + email;
                    cont += 1;
