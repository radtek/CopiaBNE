...
namespace BNE.Web.UserControls
{
    public partial class ucEndereco : BaseUserControl
    {
        ...
        public void InicializarMini(string validationGroup, string cep)
        {
            AjustarValidationGroup(validationGroup);
            HabilitarCamposEndereco(ValidarVariosEnderecosPorCEP(txtCEP.Valor));
            lblLogradouro.Visible = txtLogradouroEndereco.Visible= lblBairro.Visible= txtBairroEndereco.Visible= lblCidade.Visible = txtCidadeEndereco.Visible=
            lblNumero.Visible = txtNumero.Visible = lblComplemento.Visible = txtComplementoEndereco.Visible = txtCEP.LblErro.Visible = false;
            txtCEP.Valor = cep;
            txtCEP_ValorAlterado(txtCEP, new EventArgs());
        }
        #endregion
