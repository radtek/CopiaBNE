    ...
    public partial class DetalhesPlano : BaseUserControl
    {
        ...
        protected void gvParcelas_ItemCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            else if (e.CommandName.Equals("Estornar"))
            {
                ...
                if (Transacao.EfetuarCancelamentoCartaoDeCreditoCielo(objPagamento.DescricaoIdentificador))
                    descricaoCRM += descricaoCRM + " Estorno Realizado TID: " + objPagamento.DescricaoIdentificador;
                else
                    descricaoCRM += descricaoCRM + " Falha ao realizar o estorno TID: " + objPagamento.DescricaoIdentificador;
        ...
        private void Salvar()
        {
            try
            {
                ...
                if (!IdPagamento.HasValue)
                {
                    ...
                    {
                        var objPlanoParcela = PlanoParcela.CriarParcelaRecorrenciaPeloPlanoAdquirido(objPlanoAdquirido);
                ...
                switch (Convert.ToInt32(rcbSituacao.SelectedValue))
                {
                    ...
                    case (int)Enumeradores.PagamentoSituacao.Pago:
                        if (objPagamento.Liberar(DateTime.Now) && Plano.CarregarPlanoDePagamento(objPagamento.IdPagamento).FlagRecorrente)
                            PlanoAdquirido.SalvarNovaDataFimPlano(IdPlanoAdquirido);
