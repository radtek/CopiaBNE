    ...
    public partial class AnunciarVaga : BasePage
    {
        ...
        protected void btnAvancar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                #region [CarregarPlanos]
                Plano objPlano = Plano.LoadObject(Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.VendaPlanoCIA_PublicacaoImediataVaga)));
                litPrecoDestaquePara.Text = objPlano.ValorBase.ToString();

                objPlano = Plano.LoadObject(Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.VendaPlanoCIA_PlanoBasicoIdentificador)));
                litPlano50PrecoPara.Text = objPlano.ValorBase.ToString();
                ...
                else if (objVaga.PossuiPlanoPublicacaoImediata() != null || base.STC.ValueOrDefault)//Se já foi comprado o plano de publicação imediata, não ajusta data
                {
                    pnlCodigoVaga.Visible = true;
                    PublicarFacebook();
                }
                else
                {
                    //lblPublicacaoImediata.Text = new Plano(Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.VendaPlanoCIA_PublicacaoImediataVaga))).RecuperarValor().ToString();
                    //litDiasAtrasoPublicacaoVaga.Text = Parametro.RecuperaValorParametro(Enumeradores.Parametro.VendaPlanoCIA_PublicacaoImediataVagaQuantidadeDiasAtraso);
                    pnlCodigoVaga.Visible = false;
                    pnlPublicacaoImediata.Visible = true;
                    btnVagasAnunciadas.Visible = false;
                }
            }
        }
        #endregion
        ...
        #region lkbPlanoDestaque_Click
        protected void lkbPlanoDestaque_Click(object sender, EventArgs e)
        {
            ComprarPublicacaoImediata();
        }

        #endregion

        #region lnkGratis_Click
        protected void lnkGratis_Click(object sender, EventArgs e)
        {
            pnlPublicacaoImediata.Visible = false;
            pnlCodigoVaga.Visible = true;
            PublicarFacebook();
        }
        #endregion

        #region lnkPlano50_Click
        protected void lnkPlano50_Click(object sender, EventArgs e)
        {
            if (base.IdFilial.HasValue && base.IdUsuarioFilialPerfilLogadoEmpresa.HasValue)
            {
                ...
                base.PagamentoIdentificadorPlanoAdquirido.Value = PlanoAdquirido.CriarPlanoAdquiridoPJ(objUsuarioFilialPerfil, objFilial, objUsuarioFilial,
                    Plano.LoadObject(Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.VendaPlanoCIA_PlanoBasicoIdentificador))), 0).IdPlanoAdquirido;

                Redirect(GetRouteUrl(Enumeradores.RouteCollection.PagamentoPlanoCIA.ToString(), null));
            }
        }
        #endregion
        ...
        private void PreencherCampos()
        {
            if (IdVaga.HasValue)
            {
                ...
                if (!IdUsuarioFilialPerfilLogadoUsuarioInterno.HasValue && !ClonarVaga)
                {
        ...
        private Vaga SalvarVaga()
        {
            ...
            objVaga.SalvarVaga(listVagaDisponibilidade, listVagaTipoVinculo, listVagaPergunta, auditada, (base.IdUsuarioFilialPerfilLogadoEmpresa.HasValue ? base.IdUsuarioFilialPerfilLogadoEmpresa.Value : base.IdUsuarioFilialPerfilLogadoUsuarioInterno.Value), null);
            ...
            //calcular se é premium
            //objVaga.Cidade.CompleteObject();
            //objVaga.Cidade.Estado.CompleteObject();


            //Vaga.VagaPremium(valorAte.HasValue ? valorAte : valorDe, objVaga.Cidade.Estado.IdEstado, objFuncao.IdFuncao, objVaga);

            #endregion

            if (!empresaEmAuditoria && !auditada) //Se a vaga não está publicada, manda para publicação automática
            {
                objVaga.Publicar();
            }
