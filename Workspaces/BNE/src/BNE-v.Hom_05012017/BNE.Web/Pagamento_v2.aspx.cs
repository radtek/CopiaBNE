...
namespace BNE.Web
{
    public partial class Pagamento_v2 : BasePagePagamento
    {
        ...
        private void Inicializar(ref bool flgRecorrente)
        {
            ...
            #region Compra de Candidatura Premium

            if ((RouteData.Values.Count > 0 || !string.IsNullOrEmpty(Request.QueryString["vaga"]) && base.IdUsuarioFilialPerfilLogadoCandidato.HasValue))
            {//vaga premium 
                try
                {
                    PlanoAdquirido oPlanoAdquirido = new PlanoAdquirido();
                    int idPlanoAd = 0;
                    if (!string.IsNullOrEmpty(Request.QueryString["vaga"]))
                    {//Veio pelo bne novo, cria plano adquirido
                        base.IdVaga.Value = Convert.ToInt32(Request.QueryString["vaga"]);
                        //Verifica se o id da vaga é premium
                        if (!ParametroVaga.Premium((int)Enumeradores.Parametro.VagaPremium, base.IdVaga.Value))
                            return;
                        //CarregaPlano
                        BLL.Plano oPlano = Plano.LoadObject(Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.PlanoCandidaturaPremium)));
                        //Carrega usuario
                        UsuarioFilialPerfil oUsuarioFilialPerfil = UsuarioFilialPerfil.LoadObject(base.IdUsuarioFilialPerfilLogadoCandidato.Value);

                        oPlanoAdquirido = PlanoAdquirido.CriarPlanoAdquiridoPF(oUsuarioFilialPerfil, oPlano, null);
                        idPlanoAd = oPlanoAdquirido.IdPlanoAdquirido;
                        //Vincula a vaga ao planoAdquirido
                        PlanoAdquiridoDetalhes.CriarPlanoAdquiridoDetalheVagaPremium(oPlanoAdquirido, base.IdVaga.Value);
                    }
                    else if (RouteData.Values["idfPlanoAdquirido"] != null)
                    {
                        idPlanoAd = Convert.ToInt32(RouteData.Values["idfPlanoAdquirido"]);
                        oPlanoAdquirido = PlanoAdquirido.LoadObject(idPlanoAd);
                    }

                    if (CandidatouVaga(oPlanoAdquirido))
                        return;

                    //ter certeza que o idPlanoAdquirido vindo pela rota pertence ao usuario logado
                    if (oPlanoAdquirido != null && (base.IdUsuarioFilialPerfilLogadoCandidato.Value == oPlanoAdquirido.UsuarioFilialPerfil.IdUsuarioFilialPerfil))
                    {
                        base.PagamentoIdentificadorPlanoAdquirido.Value = idPlanoAd;
                        base.ValorBasePlano.Value = oPlanoAdquirido.ValorBase;
                        base.PagamentoIdentificadorPlano.Value = oPlanoAdquirido.Plano.IdPlano;
                        base.PagamentoFormaPagamento.Value = (int)Enumeradores.TipoPagamento.CartaoCredito;
                    }
                }
                catch (Exception ex)
            ...
            #region Carrinho Abandonado Vaga Premium - Aplicar Desconto

            if (!string.IsNullOrEmpty(Request.QueryString["codCa"]) &&
                !string.IsNullOrEmpty(Request.QueryString["codPa"]))
            {
                try
                {
                    int idCarrinhoAbandonado = Convert.ToInt32(Request.QueryString["codCa"]);
                    PlanoAdquirido objPlanoAd = PlanoAdquirido.LoadObject(Convert.ToInt32(Request.QueryString["codPa"]));
                    ...
                    if (!ParametroVaga.Premium((int)Enumeradores.Parametro.VagaPremium, objAdquiridoDetalhe.Vaga.IdVaga))
                        return;//vaga deixou de  ser premium (inativada,arquivada,editada salario e não entrou no perfil de premium)

                    if (CandidatouVaga(objPlanoAd))
                        return;
                    CurriculoCarrinhoAbandonado objCarrinhoAbandonado = CurriculoCarrinhoAbandonado.LoadObject(idCarrinhoAbandonado);

                    objCarrinhoAbandonado.CodigoDesconto.CompleteObject();

                    if (objCarrinhoAbandonado.Curriculo.IdCurriculo != base.IdCurriculo.Value
                        || objCarrinhoAbandonado.CodigoDesconto.TipoCodigoDesconto.IdTipoCodigoDesconto != (int)Enumeradores.TipoCodigoDesconto.DescontoVagaPremium)
                        return;
                    ...
                    #region Validação do código de desconto
                    if (!BLL.CodigoDesconto.CarregarPorCodigo(objCarrinhoAbandonado.CodigoDesconto.DescricaoCodigoDesconto, out codigo))
                    {
                    ...
                    //CarregaPlano
                    BLL.Plano oPlano = Plano.LoadObject(Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.PlanoCandidaturaPremium)));
                    decimal precoComDesconto = oPlano.ValorBase;

                    BLL.CodigoDesconto objCodigoDesconto = new BLL.CodigoDesconto(objCarrinhoAbandonado.CodigoDesconto.IdCodigoDesconto);
                    objCodigoDesconto.CalcularDesconto(ref precoComDesconto, oPlano);

                    base.ValorBasePlano.Value = precoComDesconto;
                    ltValorPlano.Text = precoComDesconto.ToString();

                    //Atualizar o preço com desconto
                    oPlano.ValorBase = precoComDesconto;
                    //Carrega usuario
                    UsuarioFilialPerfil oUsuarioFilialPerfil = UsuarioFilialPerfil.LoadObject(base.IdUsuarioFilialPerfilLogadoCandidato.Value);


                    objPlanoAd.ValorBase = precoComDesconto;

                    if (CandidatouVaga(objPlanoAd))
                        return;

                    // seta sessao com as informacoes sobre o desconto
                    base.PagamentoIdentificadorPlanoAdquirido.Value = objPlanoAd.IdPlanoAdquirido;
                    base.ValorBasePlano.Value = objPlanoAd.ValorBase;
                    base.PagamentoIdentificadorPlano.Value = objPlanoAd.Plano.IdPlano;
                    base.PagamentoFormaPagamento.Value = (int)Enumeradores.TipoPagamento.CartaoCredito;
        ...
        private void trataRedirecionamento(Pagamento objPagamento, bool sucesso, string msgErro)
        {
            ...
            #region Fazer a candidatura vaga premium

            if (base.PagamentoIdentificadorPlano.Value.Equals(Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.PlanoCandidaturaPremium))))
            {
                Curriculo objCurriculo = Curriculo.LoadObject(base.IdCurriculo.Value);

                Vaga objVaga = Vaga.LoadObject(PlanoAdquiridoDetalhes.RecuperarIdVagaPorPlanoAdquirido(base.PagamentoIdentificadorPlanoAdquirido.Value));
                Origem objOrigem = Origem.LoadObject(base.IdOrigem.Value);
                var respostasCandidato = new List<VagaCandidatoPergunta>();
                int? qtdCandidatura;
                var c = base.RecuperarCookie(objVaga.IdVaga.ToString(), false);
                if (c != null)
                {
                    string[] ids = c.Value.ToString().Split(',');
                    foreach (var id in ids)
                    {
                        try
                        {
                            if (!String.IsNullOrEmpty(id))
                                respostasCandidato.Add(new VagaCandidatoPergunta(Convert.ToInt32(id)));
                        }
                        catch (Exception ex)
                        {
                            EL.GerenciadorException.GravarExcecao(ex);
                ...
                VagaCandidato.Candidatar(objCurriculo, objVaga, objOrigem, respostasCandidato, Common.Helper.RecuperarIP(), base.STC.ValueOrDefault, false, false, Enumeradores.OrigemCandidatura.Site, out qtdCandidatura);
                //Encerrar o plano de candidatura premium apos confirmado o pagamento
                PlanoAdquirido.EncerrarCandidaturaPremium(objPagamento.PlanoParcela.PlanoAdquirido.IdPlanoAdquirido);
            }
            #endregion
        ...
        private bool ValidarPagamentoCartaoCredito(out BLL.Pagamento objPagamento, out string erro)
        {
            ...
            mesValidadeCartao, anoValidadeCartao, numeroDigitoVerificador, null, out erro);
        }
        #endregion
