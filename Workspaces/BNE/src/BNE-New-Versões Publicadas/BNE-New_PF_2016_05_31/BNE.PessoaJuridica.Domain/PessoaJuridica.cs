        ...
        public Model.Usuario SalvarPessoaJuridica(Command.CriarOuAtualizarPessoaJuridica command)
        {
            try
            {
                ...
                if (command.Id != 0)
                    ...
                    objPessoaJuridica = CarregarPorCNPJ(command.NumeroCNPJ) ?? new Model.PessoaJuridica { Endereco = new Model.Endereco(), Guid = Guid.NewGuid(), IP = command.IP };
                ...
                /*
                 * 
                 * Comentado porque o processo que manda essa carta de boas vindas é o de auditoria
                 * 
                 * 
                if (novoCadastro && !string.IsNullOrWhiteSpace(command.Usuario.Email))
                {
                    #region Email - Boas Vindas
                    var boasVindas = new
                    {
                        Nome = "Nome",
                        Email = "Email",
                        NomeEmpresa = objPessoaJuridica.NomeFantasia
                    };
                    _emailService.EnviarEmail(Model.Enumeradores.Email.BoasVindasPJ, "bne@bne.com.br", command.Usuario.Email, boasVindas);
                    #endregion
                }
                */
