...
namespace BNE.PessoaFisica.Domain
{
    public class PessoaFisica
    {
        ...
        #region GetPlano
        public Model.Plano GetPlano(decimal cpf)
        {
                Mapper.Models.Vaga.PlanoPremium plano = new Mapper.ToOld.PessoaFisica().CarregarPlanoPremium(cpf);
                Model.Plano planoR = new Model.Plano { PrecoCandidatura = plano.PrecoCandidatura, PrecoVip = plano.PrecoVip };
        ...
        public bool IndicarAmigos(Command.Indicacao objIndicacao)
        {
            var commandObjects = MapearIndicacaoAmigo(objIndicacao);
            bool retorno = new Mapper.ToOld.PessoaFisica().IndicarAmigos(commandObjects);
        ...
        /// <returns></returns>
        public Tuple<string,bool> Candidatar(Command.PreCurriculo preCurriculo, bool? jaExiste = false)
        {
            try
            {
                ...
                if (!jaExiste.Value)
                {
                    if (preCurriculo.DataNascimento != null)
                    {
                        ...
                        if (nomePessoaFisica == "")
                        {
                            pessoaexiste = new Mapper.ToOld.PessoaFisica().ExistePessoaFisica(preCurriculo.CPF);
                ...
                List<Tuple<int, bool?, string>> list = new List<Tuple<int, bool?, string>>();
                
                if (preCurriculo.Perguntas != null)
                {
                    foreach (var pergunta in preCurriculo.Perguntas)
                    {
                        list.Add(new Tuple<int, bool?, string>(pergunta.idVagaPergunta, pergunta.flgRespostaPergunta, pergunta.resposta));
                    }
                }

                bool result = new Mapper.ToOld.PessoaFisica().SalvarCandidatura(preCurriculo.CPF, preCurriculo.IdVaga,false, list);

                if (result)
                {//candidatura salva
                    BNE.PessoaFisica.Model.PessoaFisica pessoa = RetornarFisicaJaExiste(preCurriculo.CPF, preCurriculo.DataNascimento.Value);
                    DescontarCandidatura(_curriculo.GetByIdPessoaFisica(pessoa.Id).Id);
                    _unitOfWork.Commit();
                   
                }

                return new Tuple<string,bool>(nomePessoaFisica,result);
            }
            catch (Exception ex)
            {
                return new Tuple<string, bool>("", false);
        ...
        public bool CadastrarMiniCV(Command.PessoaFisica pessoaFisica, out bool candidatura)
        {
            ...
            //Chamar o mapeamento
            Mapper.Models.PessoaFisica.PessoaFisica commandObject = MapearNovoToAntigo(objCurriculo, baseData, objCurriculoOrigem, objFuncaoPretendida, objPessoaFisica,objCurriculoParametro, objEmailPessoaFisica.Endereco ,0, dddCelular, celular);
        ...
        public bool PostMiniCurriculo(Command.PreCurriculo preCurriculo, out bool candidatura)
        {
            ...
            try
            {
                ...
                if (idPessoaFisica > 0)
                {
                    ...
                    {
                        return Candidatar(preCurriculo,true).Item2;
                    }
                ...
                //Chamar o mapeamento
                Mapper.Models.PessoaFisica.PessoaFisica commandObject = MapearNovoToAntigo(objCurriculo, baseData, objCurriculoOrigem, objFuncaoPretendida, objPessoaFisica, objCurriculoParametro, preCurriculo.Email,preCurriculo.IdVaga, dddCelular, celular);
        ...
        private Mapper.Models.Indicacao.Indicacao MapearIndicacaoAmigo(Command.Indicacao objIndicacao)
        {
            AutoMapper.Mapper.CreateMap<Command.Indicacao, Mapper.Models.Indicacao.Indicacao>();
            AutoMapper.Mapper.CreateMap<Command.AmigoIndicado, Mapper.Models.Indicacao.AmigoIndicado>();

            var commandObjcts = AutoMapper.Mapper.Map<Command.Indicacao,Mapper.Models.Indicacao.Indicacao>(objIndicacao);
