bne.components.web.textbox = function (control, required, controlType, maxLength) {
    ...
    validateFormat: function validateFormat(controle, controlType) {
        $.validator.addMethod("validateFormatTextbox", function (value, element, validation) {
            ...
            switch (validation) {
                case 'site':
                    validationExpression = '(((http|https|ftp)\:\/\/|(www))(((25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])\.){3}(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])|([a-zA-Z0-9_\\-\\.])+\\.(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum|uk|me|ind|eng)))((:[a-zA-Z0-9]*)?\/?([a-zA-Z0-9\-\._\?\,\&apos;\/\+&amp;amp;%\$#\=~])*)';
                    break;
                case 'email':
