...
{
    public partial class Plugin // Tabela: atividade.TAB_Plugin
    {
        #region Atributos
        private short _idPlugin;
        ...
        #region Consultas
        private const string SPINSERT = "INSERT INTO atividade.TAB_Plugin (Idf_Tipo_Plugin, Des_Plugin, Des_Plugin_Metadata, Idf_Tipo_Atividade) VALUES (@Idf_Tipo_Plugin, @Des_Plugin, @Des_Plugin_Metadata);SET @Idf_Plugin = SCOPE_IDENTITY();";
        private const string SPUPDATE = "UPDATE atividade.TAB_Plugin SET Idf_Tipo_Plugin = @Idf_Tipo_Plugin, Des_Plugin = @Des_Plugin, Des_Plugin_Metadata = @Des_Plugin_Metadata WHERE Idf_Plugin = @Idf_Plugin";
        private const string SPDELETE = "DELETE FROM atividade.TAB_Plugin WHERE Idf_Plugin = @Idf_Plugin";
        private const string SPSELECTID = "SELECT * FROM atividade.TAB_Plugin WHERE Idf_Plugin = @Idf_Plugin";
        #endregion
        ...
        private List<SqlParameter> GetParameters()
        {
            ...
            parms.Add(new SqlParameter("@Idf_Plugin", SqlDbType.SmallInt, 2));
            ...
            using (SqlConnection conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        this._idPlugin = Convert.ToInt16(cmd.Parameters["@Idf_Plugin"].Value);
        ...
        public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
        {
            ...
            string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, Plu.Idf_Plugin, Plu.Idf_Tipo_Plugin, Plu.Des_Plugin, Plu.Des_Plugin_Metadata, Plu.Cod_Plugin, Plu.Idf_Tipo_Atividade FROM plataforma.TAB_Plugin Plu";
        ...
        private static bool SetInstance(IDataReader dr, Plugin objPlugin)
        {
            try
            {
                if (dr.Read())
                {
                    objPlugin._idPlugin = Convert.ToInt16(dr["Idf_Plugin"]);
                    objPlugin._tipoPlugin = new TipoPlugin(Convert.ToByte(dr["Idf_Tipo_Plugin"]));
