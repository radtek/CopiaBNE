...
namespace BNE.Mensagem.AsyncServices.BLL
{
    public partial class PluginsCompatibilidade // Tabela: plataforma.TAB_Plugins_Compatibilidade
    {
        ...
        select
         pc.* 
        from atividade.TAB_Plugins_Compatibilidade pc
          join atividade.TAB_Plugin pe on (pc.Idf_Plugin_Entrada = pe.Idf_Plugin)
          join atividade.TAB_Plugin ps on (pc.Idf_Plugin_Saida = ps.Idf_Plugin)
        where 
