        ...
        public ResponseMessageResult Candidatar(Models.PreCurriculo objPreCurriculo)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    List<Domain.Command.Pergunta> lista = new List<Domain.Command.Pergunta>();

                    if (objPreCurriculo.vagaTela.Perguntas != null)
                    {
                        foreach (var pergunta in objPreCurriculo.vagaTela.Perguntas)
                        {
                            var p = Mapper.Map<Models.Pergunta, Domain.Command.Pergunta>(pergunta);

                            lista.Add(new Domain.Command.Pergunta
                            {
                                flgRespostaPergunta = p.flgRespostaPergunta,
                                descricaoVagaPergunta = p.descricaoVagaPergunta,
                                resposta = p.resposta,
                                idVagaPergunta = p.idVagaPergunta,
                                tipoResposta = p.tipoResposta,
                                flagResposta = p.flagResposta
                            });
                        }
                    }
                    command.Perguntas = lista;
                    #endregion

                    var resultado = _preCurriculo.CandidatarCurriculo(command);
                    if (!string.IsNullOrWhiteSpace(resultado.Nome))
                    {
                        return new ResponseMessageResult(Request.CreateResponse(HttpStatusCode.Created, resultado));
                    }
        ...
        public ResponseMessageResult CadastroMini(Models.PreCurriculo objPreCurriculo)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    var command = Mapper.Map<Models.PreCurriculo, Domain.Command.PreCurriculo>(objPreCurriculo);
                    bool candidatura;
                    bool resultado = _preCurriculo.GerarMiniCurriculo(command, out candidatura);
