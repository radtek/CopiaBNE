...
{
    public partial class PluginsCompatibilidade // Tabela: atividade.TAB_Plugins_Compatibilidade
	{
		#region Atributos
		private short _idPluginsCompatibilidade;
		...
		/// Campo auto-numerado.
		/// </summary>
		public short IdPluginsCompatibilidade
		{
			get
			{
		...
		public PluginsCompatibilidade(short idPluginsCompatibilidade)
		{
		...
		#region Consultas
        private const string SPINSERT = "INSERT INTO atividade.TAB_Plugins_Compatibilidade (Idf_Plugin_Entrada, Idf_Plugin_Saida) VALUES (@Idf_Plugin_Entrada, @Idf_Plugin_Saida);SET @Idf_Plugins_Compatibilidade = SCOPE_IDENTITY();";
        private const string SPUPDATE = "UPDATE atividade.TAB_Plugins_Compatibilidade SET Idf_Plugin_Entrada = @Idf_Plugin_Entrada, Idf_Plugin_Saida = @Idf_Plugin_Saida WHERE Idf_Plugins_Compatibilidade = @Idf_Plugins_Compatibilidade";
        private const string SPDELETE = "DELETE FROM atividade.TAB_Plugins_Compatibilidade WHERE Idf_Plugins_Compatibilidade = @Idf_Plugins_Compatibilidade";
        private const string SPSELECTID = "SELECT * FROM atividade.TAB_Plugins_Compatibilidade WHERE Idf_Plugins_Compatibilidade = @Idf_Plugins_Compatibilidade";
		#endregion
		...
		private List<SqlParameter> GetParameters()
		{
			...
			parms.Add(new SqlParameter("@Idf_Plugins_Compatibilidade", SqlDbType.SmallInt, 2));
            parms.Add(new SqlParameter("@Idf_Plugin_Entrada", SqlDbType.SmallInt, 2));
            parms.Add(new SqlParameter("@Idf_Plugin_Saida", SqlDbType.SmallInt, 2));
			return(parms);
		}
		#endregion
			...
			using (SqlConnection conn = new SqlConnection(DataAccessLayer.CONN_STRING))
			{
				...
				using (SqlTransaction trans = conn.BeginTransaction())
				{
					try
					{
						...
						this._idPluginsCompatibilidade = Convert.ToInt16(cmd.Parameters["@Idf_Plugins_Compatibilidade"].Value);
		...
		public static void Delete(List<int> idPluginsCompatibilidade)
		{
			...
			for (int i = 0; i < idPluginsCompatibilidade.Count; i++)
			{
				...
				query += nomeParametro;
                parms.Add(new SqlParameter(nomeParametro, SqlDbType.SmallInt, 2));
		...
		public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
		{
			...
			string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, Plu.Idf_Plugins_Compatibilidade, Plu.Idf_Plugin_Entrada, Plu.Idf_Plugin_Saida FROM plataforma.TAB_Plugins_Compatibilidade Plu";
		...
		private static bool SetInstance(IDataReader dr, PluginsCompatibilidade objPluginsCompatibilidade)
		{
			try
			{
				if (dr.Read())
				{
					objPluginsCompatibilidade._idPluginsCompatibilidade = Convert.ToInt16(dr["Idf_Plugins_Compatibilidade"]);
                    objPluginsCompatibilidade._pluginEntrada = new Plugin(Convert.ToInt16(dr["Idf_Plugin_Entrada"]));
                    objPluginsCompatibilidade._pluginSaida = new Plugin(Convert.ToInt16(dr["Idf_Plugin_Saida"]));
