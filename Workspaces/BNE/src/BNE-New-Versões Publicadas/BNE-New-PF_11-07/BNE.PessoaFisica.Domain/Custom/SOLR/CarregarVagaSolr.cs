    ...
    public class CarregarVagaSolr
    {
        public static Command.Vaga CarregarVaga(string idVaga, string urlSLOR)
        {
            ...
            if (resultado.response.docs.Count > 0)
            {
                ...
                vaga.DescricaoDeficiencia = document.Des_Deficiencia;
                vaga.Idf_Deficiencia = document.Idf_Deficiencia;
                vaga.Bairro = document.Nme_Bairro != null ? Mapper.ToOld.PessoaFisica.AjustarString(document.Nme_Bairro): string.Empty;

                vaga.FlgPremium = document.Flg_Vaga_Arquivada ? false : document.Flg_Vaga_Premium;
            ...
            else
            {
                ...
                if(vagaSQL != null)
                {
                    ...
                    vaga.Idf_Deficiencia = vagaSQL.Idf_Deficiencia;
                    vaga.eAprendiz = vagaSQL.eAprendiz;
                    vaga.eEfetivo = vagaSQL.eEfetivo;
                    vaga.eEstagio = vagaSQL.eEstagio;
                    vaga.FlgPremium = vagaSQL.FlgPremium;
                    vaga.Bairro = vagaSQL.Bairro;
