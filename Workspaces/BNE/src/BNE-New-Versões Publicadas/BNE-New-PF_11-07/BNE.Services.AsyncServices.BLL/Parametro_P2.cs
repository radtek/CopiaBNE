...
namespace BNE.Services.AsyncServices.BLL
{
    public partial class Parametro // Tabela: TAB_Parametro
    {
        ...
        #region Parametros
        private static Dictionary<Services.AsyncServices.BLL.Enumeradores.Parametro, string> Parametros
        {
            get
            {
        ...
        private static Dictionary<Services.AsyncServices.BLL.Enumeradores.Parametro, string> ListarParametrosCACHE()
        {
            var listaParametros = new List<Services.AsyncServices.BLL.Enumeradores.Parametro>();

            foreach (Services.AsyncServices.BLL.Enumeradores.Parametro parametro in Enum.GetValues(typeof(Services.AsyncServices.BLL.Enumeradores.Parametro)))
            {
                listaParametros.Add(parametro);
            }

            var parms = new List<SqlParameter>();
            var itensParametros = new Dictionary<Services.AsyncServices.BLL.Enumeradores.Parametro, string>();
            ...
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, query, parms))
            {
                while (dr.Read())
                {
                    var parametro = (Services.AsyncServices.BLL.Enumeradores.Parametro)Enum.Parse(typeof(Services.AsyncServices.BLL.Enumeradores.Parametro), dr["Idf_Parametro"].ToString());
        ...
        public static string RecuperaValorParametro(Services.AsyncServices.BLL.Enumeradores.Parametro parametro, SqlTransaction trans = null)
        {
            #region Cache
            if (HabilitaCache)
        ...
        /// <returns>Dicionário com valores recuperados do banco.</returns>
        public static Dictionary<Services.AsyncServices.BLL.Enumeradores.Parametro, string> ListarParametros(List<Services.AsyncServices.BLL.Enumeradores.Parametro> idsParametros)
        {
            try
            {
                #region Cache
        ...
        /// </summary>
        public static void SalvarParametros(Dictionary<Services.AsyncServices.BLL.Enumeradores.Parametro, string> dicionarioParametros)
        {
            var parms = new List<SqlParameter>();

            string query = String.Empty;
            int i = 1;
            foreach (KeyValuePair<Services.AsyncServices.BLL.Enumeradores.Parametro, string> kvp in dicionarioParametros)
            {
