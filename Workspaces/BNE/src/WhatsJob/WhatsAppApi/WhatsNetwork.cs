...
namespace WhatsAppApi
{
    ...
    public class WhatsNetwork
    {
        ...
        /// </summary>
        /// <returns></returns>
        public byte[] ReadData(int length = 1024, int? retry = null)
        {
            return Socket_read(length, retry);
        }
        ...
        public byte[] ReadNextNode(int? retry = null)
        {
            byte[] nodeHeader = this.ReadData(3, retry);
            ...
            do
            {
                byte[] nodeBuff = this.ReadData(toRead, retry);
        ...
        private byte[] Socket_read(int length, int? retry = null)
        {
            if (!socket.Connected)
            {
