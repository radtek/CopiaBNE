...
namespace WhatsAppApi
{
    public class WhatsSendBase : WhatsAppBase
    {
        ...
        public void PollMessages(bool autoReceipt = true, int? retry = null)
        {
            while (pollMessage(autoReceipt, retry)) ;
        }

        public bool pollMessage(bool autoReceipt = true, int? retry = null)
        {
            if (this.loginStatus == CONNECTION_STATUS.CONNECTED || this.loginStatus == CONNECTION_STATUS.LOGGEDIN)
            {
                ...
                try
                {
                    nodeData = this.whatsNetwork.ReadNextNode(retry);
                    if (nodeData != null)
                    {
        ...
        protected void handleIq(ProtocolTreeNode node)
        {
                ...
                && node.GetChild("group") != null)
            {
                ...
                foreach (ProtocolTreeNode group in node.GetChild("groups").children)
                {
                ...
                && node.GetChild("participant") != null)
            {
                ...
                foreach (ProtocolTreeNode part in node.GetAllChildren("participant"))
                {
