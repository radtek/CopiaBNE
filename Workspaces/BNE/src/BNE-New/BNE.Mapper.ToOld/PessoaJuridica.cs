...
using BNE.BLL;
using BNE.Mapper.Models.PessoaJuridica;
...
namespace BNE.Mapper.ToOld
{
    public class PessoaJuridica
    {
            ...
            using (SqlConnection conn = new SqlConnection(BLL.DataAccessLayer.CONN_STRING))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        objFilial.SituacaoFilial = new SituacaoFilial((int)BLL.Enumeradores.SituacaoFilial.AguardandoPublicacao);

                        if (objCNAESubClasse == null && objNaturezaJuridica == null)
                            objFilial.SituacaoFilial = new SituacaoFilial((int)BLL.Enumeradores.SituacaoFilial.FaltaDadosReceita);
                        ...
                        MapUsuariosAdicionais(commonObject.UsuariosAdicionais, objFilial, trans);
        ...
        private void MapUsuarios(List<Usuario> usuarios, BLL.Filial objFilial, bool novoCadastroPJ, SqlTransaction trans)
        {
            if (usuarios != null)
            {
                foreach (var usuario in usuarios)
                {
                    ...
                    objPessoaFisica.FlagWhatsApp = usuario.FlgWhatsApp;
        ...
        private void MapUsuariosAdicionais(List<UsuarioAdicional> usuariosAdicionais, Filial objFilial, SqlTransaction trans)
        {
            if (usuariosAdicionais != null)
            {
                foreach (var usuarioAdicional in usuariosAdicionais)
                {
                    BLL.UsuarioFilialContato objUsuarioFilialContato = new UsuarioFilialContato
                    {
                        Filial = objFilial,
                        NomeContato = usuarioAdicional.Nome,
                        EmailContato = usuarioAdicional.Email,
                        FlagInativo = false,
                    };

                    objUsuarioFilialContato.Save(trans);
                }
            }
        }
        ...
        public decimal GetPlanoValorDe(int idplano)
        {
            return Plano.LoadObject(idplano).ValorDe;
        }

        public decimal GetPlanoValorPara(int idplano)
        {
            return Plano.LoadObject(idplano).ValorBase;
        }
    }
}
