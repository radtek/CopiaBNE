...
bne.components.web.decimal = function (control, required, controlType, validarSalarioMinimo) {
    ...
    validarPretensaoMaxima = function validarPretensaoMaxima(control) {

        $.validator.addMethod("validarPretensaoMaxima", function (value, element) {
            var validado = false;
            var valorMaximo = 100000;
            var pretensaoSalarial = (Math.round(parseFloat(value.replace('.', '')) * 100) / 100);

            if (pretensaoSalarial <= valorMaximo) {
                validado = true;
            }

            return validado;
        }, "* Sálario deve ser menor que R$100.000,00");

        $(control).rules("add", { validarPretensaoMaxima: true });
    }
    ...
    if (validarSalarioMinimo) {
        ...
        validarPretensaoMaxima(control);
    }
}