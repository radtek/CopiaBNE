...
namespace BNE.Web.Vagas.Controllers
{
    public class VagaController : BaseController
    {
        ...
        public ActionResult VisualizarVaga(int identificador)
        {
            ...
            var vagaDisponibilidade = VagaDisponibilidade.ListarDisponibilidadesPorVaga(objVaga.IdVaga);
        ...
        public PerguntaCelular PerguntarSobreCelular(VagaCandidatura vagaModel)
        {
            ...
            EL.GerenciadorException.GravarExcecao(new Exception("Vaga >> PerguntarSobreCelular PF: " + objPessoaFisica.IdPessoaFisica));

            /*
            if (!objPessoaFisica.FlagCelularConfirmado)
            {
                DateTime? dataEnvio;
                PessoaFisica.ValidacaoCelularCodigoEnviado(objPessoaFisica.NumeroDDDCelular, objPessoaFisica.NumeroCelular, out dataEnvio);
                if (!dataEnvio.HasValue)
                {
                    PessoaFisica.ValidacaoCelularEnviarCodigo(objPessoaFisica.NumeroDDDCelular, objPessoaFisica.NumeroCelular);
                }
            }
            */
            ...
            model.IdPerguntaHistorico = PerguntaSalaVipHistorico.SalvarHistoricoPerguntaExibicao(base.IdPessoaFisicaLogada.Value, BLL.Enumeradores.PerguntaSalaVip.Celular);
            ...
            try
            {
                ...
                if (base.IdUsuarioFilialPerfilLogadoCandidato.HasValue)
                {
                    ...
                    if (VagaCandidato.CurriculoJaCandidatouVaga(objCurriculo, objVaga))
                    {
                        ...
                        return PartialView("_VagaCandidatura", new VagaCandidatura { Candidatou = true, IdentificadorVaga = model.IdentificadorVaga, Sucesso = new VagaCandidatura.SucessoCandidatura { Protocolo = protocolo, NomeCandidato = nomeCandidato } });
                    }
                    #endregion
                    ...
                    if (existePerguntas && model.Perguntas == null)
                    {
                        ...
                        var perguntasCandidatura = listaVagaPergunta.Select(objVagaPergunta => new PerguntasCandidatura.Pergunta { Descricao = objVagaPergunta.DescricaoVagaPergunta, Id = objVagaPergunta.IdVagaPergunta }).ToList();

                        return PartialView("_VagaCandidatura", new VagaCandidatura { IdentificadorVaga = model.IdentificadorVaga, Perguntas = new PerguntasCandidatura { Perguntas = perguntasCandidatura } });
                    }
                    #endregion
                    ...
                    if (existePerguntas)
                    {
                        if (model.Perguntas.Perguntas.Count(p => p.Resposta == null) > 0)
                        {
                            //TODO: Mensagem de erro
                            return PartialView("_VagaCandidatura", model);
                        }
                        respostasCandidato.AddRange(model.Perguntas.Perguntas.Select(pergunta => new VagaCandidatoPergunta { VagaPergunta = new VagaPergunta(pergunta.Id), FlagResposta = pergunta.Resposta.Value }));
                    }
                    #endregion
                    ...
                    VagaCandidato.Candidatar(objCurriculo, objVaga, objOrigem, respostasCandidato, Common.Helper.RecuperarIP(), base.STC.ValueOrDefault, base.STCUniversitario.Value, false, out quantidadeCandidaturas);
                    #endregion
                    ...
                    if (quantidadeCandidaturas >= 0)
                    {
                        ...
                        if (quantidadeCandidaturasRestantes.Equals(-1) || quantidadeCandidaturasRestantes.Equals(0))
                        ...
                        return PartialView("_VagaCandidatura", new VagaCandidatura { Candidatou = candidatou, IdentificadorVaga = model.IdentificadorVaga, Degustacao = new VagaCandidatura.DegustacaoCandidatura { DescricaoCandidaturaRestante = textoCandidatura, QuantidadeCandidaturaRestante = quantidadeCandidaturasRestantes } });
                    }
                    #endregion
                }
        ...
        {
            var url = string.Concat("http://", Helper.RecuperarURLAmbiente(), "/vip");
            return Redirect(url);
        }
        #endregion
        ...
        public PartialViewResult DadosEmpresa(int vaga)
        {
            ...
            if (objVaga.FlagConfidencial)
            {
                ...
                objDadosEmpresa.MensagemEmpresaConfidencial = "Essa empresa optou por deixar seus dados confidenciais.";
            }
            else
            {
                if (base.IdCurriculo.HasValue)
        ...
        public PartialViewResult EnviarCompartilhamentoEmail(string button, CompartilhamentoEmail compartilhamentoEmail)
        {
            ...
            else if (button.Equals("enviar"))
            {
                ...
                var objUsuarioFilialPerfil = new UsuarioFilialPerfil(base.IdUsuarioFilialPerfilLogadoCandidato.Value);
                ...
                {
                    MensagemCS.EnviarCompartilhamentoVaga(objUsuarioFilialPerfil, objVaga, compartilhamentoEmail.URL, destinatario);
                    qtdeEmails++;
                }
        ...
        #region VagaEmpresaPossuiPlano
        [ActionName("CG")]
        public ActionResult VagaEmpresaPossuiPlano(int i)
        {
            try
            {
                var objVaga = BLL.Vaga.LoadObject(i);
                return Json(objVaga.Filial.PossuiPlanoAtivo());
            }
            catch (Exception ex)
            {
                EL.GerenciadorException.GravarExcecao(ex);
        ...
        public void SalvarCelular(PerguntaCelular modelCelular, bool celularAlterado)
        {
            ...
            EL.GerenciadorException.GravarExcecao(new Exception("Vaga >> SalvarCelular PF: " + objPessoa.IdPessoaFisica));
        ...
        public void SalvarExperienciaProfissional(ExperienciaProfissional modelExp, bool experienciaAlterada)
        {
            if (modelExp.RazSocial != null)
            {
                ...
                exp.SalvarExperienciaProfissional(exp);
            }
        }
