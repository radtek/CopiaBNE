...
namespace BNE.Services.Plugins.PluginSaida.AllInOutput
{
    public class EnvioSaidaAllInPainelCicloVida : EnvioSaidaAllInBase
    {
        ...
        {
            this._valueModelResult = objResult;
        }
        ...
        protected override System.Threading.Tasks.Task<string> ProduceLoginToken()
        {
            //return AllInTriggers.AllInPainel.LoginTokenPainelCall().ExecuteAsync();
            return TaskEx.FromResult(string.Empty);
        }

        public override async System.Threading.Tasks.Task Do(string loginToken)
        {
            ...
            //await ReprocessIfIsAuthenticationFailure(respostaCicloDeVida);
        ...
        private System.Threading.Tasks.Task<bool> ValidateAndProcessAnswer(string respostaCicloDeVida)
        {
            return TaskEx.Run(() =>
                {
                    return respostaCicloDeVida.IndexOf("OK", StringComparison.OrdinalIgnoreCase) > -1;
                });
        }
    }
}
