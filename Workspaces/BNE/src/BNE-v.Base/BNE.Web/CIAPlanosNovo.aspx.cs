...
namespace BNE.Web
{
    public partial class CIAPlanosNovo : BasePagePagamento
    {
        ...
        String _nomePessoa, _numRG, _razaoSocial, _numCNPJ, _descRua, _nomeCidade, _estado, _email, _numeroRua, _numeroCEP;
        int _numeroParcelas, _quantidadeUsuarios, _quantidadeSms, _tempoPlano;
        decimal _valorPlano, _numCPF;
        ...
        private void NotificarVendaPlanoAbaixoMinimo()
        {
            if (base.PagamentoIdCodigoDesconto.HasValue)
            {
                ...
                if (objCodigoDesconto.UsuarioFilialPerfil != null)
                {
                    ...
                    var valoresDestinatarios =
                        Parametro.RecuperaValorParametro(BLL.Enumeradores.Parametro.EmailVendaCiaAbaixoMinimo);
                    ...
                    {
                        var lista = valoresDestinatarios.Split(';');
                        string assunto;
                        var carta =
                            CartaEmail.RetornarConteudoBNE(Enumeradores.CartaEmail.VendaPlanoVendedorAbaixoMinimo,
                                out assunto);
                        ...
                        foreach (var destinatario in lista)
                        {
                            EmailSenderFactory.Create(TipoEnviadorEmail.Fila)
                                .Enviar(assunto, carta, emailRemetente, destinatario.Trim());
                        }
        ...
        private void Redirecionar(PlanoEscolhido planoEscolhido)
        {
            if (base.PagamentoIdentificadorPlano.HasValue)
            {
                ...
                if (objPlano.FlagEnviarContrato)
                    EnviarContratoEmail(planoEscolhido);
            }
        ...
        private void PreencherCampos()
        {
            ...
            if (objPlanoBasico.QuantidadeDiasValidade == 365)
                litPeriodoBasico.Text = "12 meses";
            else
            {
                int meses = (int)Math.Floor((decimal)(objPlanoBasico.QuantidadeDiasValidade / 30));
                if (meses > 0)
                    litPeriodoBasico.Text = meses.ToString() + " meses";
                if (objPlanoBasico.QuantidadeDiasValidade - meses * 30 > 0)
                {
                    litPeriodoBasico.Text += meses > 0 ? " e " : "";
                    litPeriodoBasico.Text += (objPlanoBasico.QuantidadeDiasValidade - meses * 30).ToString() + " dias";
                }
            }
            ...
            litDescricaoParcelaBasico.Text = objPlanoBasico.QuantidadeParcela > 1 ? "por mês" : "parcela única";
            ...
            if (objPlanoIndicado.QuantidadeDiasValidade == 365)
                litPeriodoIndicado.Text = "12 meses";
            else
            {
                int meses = (int)Math.Floor((decimal)(objPlanoIndicado.QuantidadeDiasValidade / 30));
                if (meses > 0)
                    litPeriodoIndicado.Text = meses.ToString() + " meses";
                if (objPlanoIndicado.QuantidadeDiasValidade - meses * 30 > 0)
                {
                    litPeriodoIndicado.Text += meses > 0 ? " e " : "";
                    litPeriodoIndicado.Text += (objPlanoIndicado.QuantidadeDiasValidade - meses * 30).ToString() + " dias";
                }
            }
            ...
            litDescricaoParcelaIndicado.Text = objPlanoIndicado.QuantidadeParcela > 1 ? "por mês" : "parcela única";
            ...
            if (objPlanoPlus.QuantidadeDiasValidade == 365)
                litPeriodoPlus.Text = "12 meses";
            else
            {
                int meses = (int)Math.Floor((decimal)(objPlanoPlus.QuantidadeDiasValidade / 30));
                if (meses > 0)
                    litPeriodoPlus.Text = meses.ToString() + " meses";
                if (objPlanoPlus.QuantidadeDiasValidade - meses * 30 > 0)
                {
                    litPeriodoPlus.Text += meses > 0 ? " e " : "";
                    litPeriodoPlus.Text += (objPlanoPlus.QuantidadeDiasValidade - meses * 30).ToString() + " dias";
                }
            }
            ...
            litDescricaoParcelaPlus.Text = objPlanoPlus.QuantidadeParcela > 1 ? "por mês" : "parcela única";
        ...
        public void GerarPDFContrato(string razaoSocial, string numCNPJ, string descRua, string numeroRua, string estado,
                                     ...
                                     int quantidadeSms, int quantidadeUsuarios)
        {
            ...
            var pdf = GerarContratoPlano.ContratoPadraoPdf(razaoSocial, numCNPJ, descRua, numeroRua, estado,
                                                           ...
                                                           numeroParcelas, tempoPlano, quantidadeSms, quantidadeUsuarios);
        ...
        #region EnviarContratoEmail
        public void EnviarContratoEmail(PlanoEscolhido planoEscolhido)
        {
            try
            {
                AtualizaValoresContrato(planoEscolhido);

                if (planoEscolhido == PlanoEscolhido.Nenhum)
                    throw new InvalidOperationException("Não foi selecionado nenhum plano antes de enviar o contrato");
                ...
                PessoaFisica.CarregarPorIdUsuarioFilialPerfil(idUsuario, out _nomePessoa, out _numRG, out _numCPF, out _email);
                Filial.RecuperarConteudoFilialParaContratoPorFilial(idUsuario, out _razaoSocial, out _numCNPJ, out _descRua, out _numeroRua, out _nomeCidade, out _estado, out _numeroCEP);

                if (idUsuario > 0)
                    _quantidadeUsuarios = UsuarioFilialPerfil.LoadObject(idUsuario).Filial.RecuperarQuantidadeAcessosAdquiridos();

                var pdf = GerarContratoPlano.ContratoPadraoPdf(_razaoSocial, _numCNPJ, _descRua, _numeroRua, _estado,
                                                               _nomeCidade, _numeroCEP, _nomePessoa, _numRG, _numCPF, _valorPlano,
                                                               _numeroParcelas, _tempoPlano, _quantidadeSms, _quantidadeUsuarios);
                #endregion

                EmailSenderFactory
                    .Create(TipoEnviadorEmail.Fila)
                    .Enviar("Contrato Plano BNE", "Segue contrato em anexo", "financeiro@bne.com.br", _email, "Contrato_BNE.pdf", pdf);
            }
            catch (Exception ex)
            {
        ...
        public void AtualizaValoresContrato(PlanoEscolhido planoEscolhido)
        {
            ...
            Plano objPlano = Plano.LoadObject(PlanoBasicoIdentificador);
        ...
        private void VerContrato()
        {
            ...
            PessoaFisica.CarregarPorIdUsuarioFilialPerfil(idUsuario, out _nomePessoa, out _numRG, out _numCPF, out _email);
            Filial.RecuperarConteudoFilialParaContratoPorUsuarioFilialPerfil(idUsuario, out _razaoSocial, out _numCNPJ, out _descRua, out _numeroRua, out _nomeCidade, out _estado, out _numeroCEP);
            ...
            if (base.PagamentoIdentificadorPlano.HasValue)
            {
                ...
                if (objPlano.FlagEnviarContrato)
                {
                    ...
                    GerarPDFContrato(_razaoSocial, _numCNPJ, _descRua, _numeroRua, _estado,
                        _nomeCidade, _numeroCEP, _nomePessoa, _numRG, _numCPF, _valorPlano, _numeroParcelas, _tempoPlano,
                        _quantidadeSms, _quantidadeUsuarios);
                }
                else
                {
