...
namespace SistMars.pessoa
{
	...
	/// </summary>
	public class resultadomail : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			if(!Page.IsPostBack)
			{
				try
				{
					...
					if(Request.QueryString["ANA_CodigoAnalise"] != null)
					{
						//caso tenha sido passado codigo da analise (pelo menut/relat, por exemplo)
						imgContinuar.Visible = false;
                        CarregarDadosAvaliacao("p_ListarAvaliacaoCodigo ", Request.QueryString["ANA_CodigoAnalise"].Trim(),"@ANA_CodigoAnalise");
					}
					else if(Request["Enquete"] != null)
					{
						//caso tenha sido passado por parametro o codigo da pessoa (quando retorna da enquete)
						imgContinuar.Visible = false;
						CarregarDadosAvaliacao("p_ListarAvaliacaoPessoa " , Request.Cookies["PES_CodigoPessoa"].Value,"@PES_CodPessoa");
					}
					else
					{
		...
		private void AnalisarResultado()
		{
			...
			if(colunasLinhasCertas != "'")
                colunasLinhasCertas = colunasLinhasCertas.Substring(0, colunasLinhasCertas.Length-4);
			else
				colunasLinhasCertas = "";
			...
			try
			{
                ...
                parms.Add(new SqlParameter("@TotalPontos", SqlDbType.Int, 4));
                ...
                DataAccessLayer.ExecuteReaderDs(CommandType.StoredProcedure, "p_CadastrarAvaliacao", parms);
			}
			catch(Exception ex)
			{
		...
		private void CarregarDadosAvaliacao(string proc,string codAnalise,string parameters)
		{
			...
			try
			{
                ...
                param.Add(new SqlParameter(parameters, SqlDbType.Int, 4));
                param[0].Value = codAnalise;

                dt = DataAccessLayer.ExecuteReaderDs(CommandType.StoredProcedure,proc,param).Tables[0];
                ...
                parms[0].Value = dt.Rows[0]["ANA_CodigoAnalise"].ToString();				
