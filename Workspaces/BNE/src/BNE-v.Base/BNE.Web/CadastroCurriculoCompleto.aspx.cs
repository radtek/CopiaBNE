...
namespace BNE.Web
{
    public partial class CadastroCurriculoCompleto : BasePage
    {
        ...
        private bool Salvar(out string mensagemErro)
        {
            ...
            ucDadosPessoais.Salvar();
            ucFormacaoCursos.Salvar();
            ucDadosComplementares.Salvar();
            objCurriculo = Curriculo.LoadObject(IdCurriculo);

            if (objCurriculo.SituacaoCurriculo.IdSituacaoCurriculo == 6 || objCurriculo.SituacaoCurriculo.IdSituacaoCurriculo == 7)
            {
                ExcluirCVSolr(IdCurriculo);
            }
            else
            { 
            EnviarCVSolr(IdCurriculo);
            }
        ...
        #region EnviarCVSolr
        public static void EnviarCVSolr(int Idf_Curriculo)
        {
        ...
        {
            if (base.IdPerfil.HasValue && base.IdPerfil.Value.Equals((int)Enumeradores.Perfil.AdministradorSistema))
            {
                //Se for administrador Exibe combo box com as situações
