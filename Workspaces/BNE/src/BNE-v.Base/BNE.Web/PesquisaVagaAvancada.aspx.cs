...
namespace BNE.Web
{
    ...
    public partial class PesquisaVagaAvancada : BasePage
    {
        ...
        #region rcbEstado_SelectedIndexChanged
        protected void rcbEstado_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            aceCidade.ContextKey = rcbEstado.SelectedValue;
            upCidade.Update();
        }
        #endregion
        ...
        private void Inicializar()
        {
            ...
            CarregarParametros();
            ucTipoContratoFuncao.CarregarParametros();
        ...
        private void PreencherCampos()
        {
            ...
            List<PesquisaVagaDisponibilidade> listPesquisaVagaDisponibilidade = PesquisaVagaDisponibilidade.ListarPorPesquisaList(objPesquisaVaga.IdPesquisaVaga);
            foreach (PesquisaVagaDisponibilidade objPesquisaVagaDisponibilidade in listPesquisaVagaDisponibilidade)
                rcbDisponibilidade.SetItemChecked(objPesquisaVagaDisponibilidade.Disponibilidade.IdDisponibilidade.ToString(CultureInfo.CurrentCulture), true);

            List<VagaTipoVinculo> listVagaTipoVinculo = VagaTipoVinculo.ListarTipoVinculoPorVaga(IdPesquisaVaga.Value);
            foreach (var checkItem in ucTipoContratoFuncao.TipoContratoItens)
            {
            ...
            if (objPesquisaVaga.AreaBNE != null)
            {
                ...
                if (base.STC.Value)
                    ucTipoContratoFuncao.SetAceFuncaoContextKey(String.Format("{0};{1}",
                        base.IdOrigem.Value.ToString(CultureInfo.CurrentCulture), rcbAreaBNE.SelectedValue));
                else
                    ucTipoContratoFuncao.SetAceFuncaoContextKey(String.Format("{0};{1}", string.Empty,
                        rcbAreaBNE.SelectedValue));
            }
            else
            {
                if (base.STC.Value)
                    ucTipoContratoFuncao.SetAceFuncaoContextKey(String.Format("{0};{1}",
                        base.IdOrigem.Value.ToString(CultureInfo.CurrentCulture), string.Empty));
                else
                    ucTipoContratoFuncao.SetAceFuncaoContextKey(String.Format("{0};{1}", string.Empty, string.Empty));
            }

            if (objPesquisaVaga.Deficiencia != null)
                //{
                //rcbDefiente.SelectedValue = "1";
        ...
        private void Buscar()
        {
            ...
            objPesquisaVaga.Salvar(listPesquisaVagaDisponibilidade, listPesquisaVagaTipoVinculo);
        ...
        protected void ucTipoContratoFuncao_OnFuncaoValida(object sender, EventArgs e)
        {
            ...
            if (Funcao.CarregarPorDescricao(ucTipoContratoFuncao.FuncaoDesc, out objFuncao))
            {
                ...
                if (base.STC.Value)
                    ...
                    ucTipoContratoFuncao.SetAceFuncaoContextKey(String.Format("{0};{1}", string.Empty, rcbAreaBNE.SelectedValue));
        ...
        protected void ucTipoContratoFuncao_OnFuncaoInvalida(object sender, EventArgs e)
        {
            if (base.STC.Value)
                ...
                ucTipoContratoFuncao.SetAceFuncaoContextKey(null);
