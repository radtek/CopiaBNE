...
namespace BNE.Web.UserControls.Forms.CadastroCurriculo
{
    public partial class ConferirDados : BaseUserControl
    {
        ...
        protected void txtCEP_ValorAlterado(object sender, EventArgs e)
        {
            ...
            {
                wsCEP.cepws servico = new wsCEP.cepws();

                ServiceAuth.GerarHashAcessoWS(servico);

                wsCEP.CEP objCep = new wsCEP.CEP();
                objCep.Cep = txtCEP.Valor.Replace("-", string.Empty).Trim();
                ...
                try
                {
                    if (servico.RecuperarQuantidadeEnderecosPorCEP(objCep, ref qtdeCepEncontrados))
                    {
                        ...
                        else
                        {
                            ...
                            aceCidade.ContextKey = objCep.Estado;
                            lblCidade.Text = txtCidade.Text = String.Format("{0}/{1}", objCep.Cidade, objCep.Estado);

                            txtCEP.DisplayMensagemErroWS = "none";

                            PreencherCamposEndereco(objCep.Cep, objCep.Logradouro, txtNumero.Text, txtComplemento.Text, objCep.Bairro);
        ...
        {
            aceCidade.ContextKey = string.Empty;
            ...
            if (!string.IsNullOrEmpty(cep))
            {
                ...
                objCep.Logradouro = Server.HtmlDecode(logradouro);
            ...
            //upnAbaDadosPessoaisDados.Update();
            txtNumero.Focus();
            var parametros = new
            {
        ...
        {
            ((Principal)this.Page.Master).ExibirMensagemConfirmacao("Confirmação", "Currículo enviado com sucesso!", false, "OK");
        }
        #endregion
        ...
        private bool Salvar()
        {
            ...
            SalvarDadosPessoaFisica(objPessoaFisica);
            #endregion
            ...
            #region Currículo
            SalvarDadosCurriculo(objCurriculo, out objCurriculoOrigem);
            #endregion
        ...
        //[Obsolete("Obtado por não utilização/disponibilização.")]
        //private List<CurriculoTipoVinculo> RetornaTipoContratoMarcados()
        //{
        //    return (from item in (chblTipoContrato.Items ?? new ListItemCollection()).OfType<ListItem>()
        //            where item.Selected
        //            select
        //                new CurriculoTipoVinculo
        //                {
        //                    IdCurriculo = IdCurriculo.ValueOrDefault,
        //                    IdTipoVinculo = Convert.ToInt32(item.Value)
        //                }).ToList();
        //}
        #endregion

        #region SalvarDadosPessoaFisica
        private void SalvarDadosPessoaFisica(PessoaFisica objPessoaFisica)
        {
        ...
        private void SalvarDadosCurriculo(Curriculo objCurriculo, out CurriculoOrigem objCurriculoOrigem)
        {
...
#warning ver este método
        ...
        private void LimparCamposEspecializacao()
        {
            ...
            txtAnoConclusaoEspecializacao.Valor = String.Empty;
            aceCidadeEspecializacao.ContextKey = String.Empty;
        ...
        private void LimparCamposCursos()
        {
            ...
            aceCidadeComplementar.ContextKey = String.Empty;
        }
        #endregion
        ...
        public static bool ValidarVariosEnderecosPorCEP(string valor)
        {
            if (!string.IsNullOrEmpty(valor))
            {
                ...
                wsCEP.CEP objCep = new wsCEP.CEP();
                objCep.Cep = valor.Replace("-", string.Empty).Trim();
