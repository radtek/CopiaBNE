...
namespace BNE.Web.UserControls.Forms.SalaAdministrador
{
    public partial class PlanoFidelidade : BaseUserControl
    {
        ...
        private void Inicializar()
        {
            ...
            UIHelper.CarregarRadComboBox(rcbSituacaoEmpresa, SituacaoFilial.Listar(), "Idf_Situacao_Filial", "Des_Situacao_Filial");
        ...
        private void PreencherCamposPlano(PlanoAdquirido objPlanoAdquirido)
        {
            ...
            if (objPlanoAdquirido.Filial != null)
            {
                ...
                if (objPlanoAdquirido.PlanoSituacao.IdPlanoSituacao.Equals((int)Enumeradores.PlanoSituacao.Liberado))
                {
                    ...
                    if (PlanoQuantidade.CarregarPorPlano(objPlanoAdquirido.IdPlanoAdquirido, out objPlanoQuantidade))
                    {
                        ...
                        pnlAtualizarPlano.Visible = btnAtualizarPlano.Visible = true;
            ...
            {
                txtEnviarPara.Valor = objPlanoAdquiridoDetalhes.EmailEnvioBoleto;
            ...
            else
            {
                if (IdFilial.HasValue)
                {
                    ...
                    if (UsuarioFilialPerfil.CarregarPorPerfilFilial((int)Enumeradores.Perfil.AcessoEmpresaMaster, IdFilial.Value, out objUsuarioFilialPerfil))
                    {
                        ...
                        {
                            txtEnviarPara.Valor = objUsuarioFilial.EmailComercial;
        ...
        public void LimparCampos()
        {
            ...
            txtEnviarPara.Valor = String.Empty;
            rbSim.Checked = true;
        ...
        private PlanoAdquirido Salvar(Plano objPlano)
        {
            ...
            {
                //Detalhes
                objPlanoAdquiridoDetalhes.EmailEnvioBoleto = txtEnviarPara.Valor;
                ...
                {
                    objPlanoAdquirido.SalvarPlanoAdquiridoParcelaPagamento(objUsuarioFilialPerfil, objFilial, objPlano, objPlanoAdquiridoDetalhes);
                    ExibirMensagemConfirmacao("Confirmação de Cadastro", "Plano atualizado com sucesso!", false);
                }
                else
                {
                    objPlanoAdquirido.SalvarPlanoAdquiridoParcelaPagamento(objUsuarioFilialPerfil, objFilial, objPlano, objPlanoAdquiridoDetalhes, txtDataEnvioBoleto.ValorDatetime.Value, txtDataVencimentoBoleto.ValorDatetime.Value);
            ...
            else if (IdPessoaFisica.HasValue)
            {
                ...
                if (UsuarioFilialPerfil.CarregarUsuarioFilialPerfilCandidatoAtivo(new PessoaFisica(IdPessoaFisica.Value), out objUsuarioFilialPerfil))
                    objPlanoAdquirido.SalvarPlanoAdquiridoParcelaPagamento(objUsuarioFilialPerfil, null, objPlano, objPlanoAdquiridoDetalhes, txtDataEnvioBoleto.ValorDatetime, txtDataVencimentoBoleto.ValorDatetime);
        ...
        private void CriarAdicional(PlanoAdquirido objPlanoAdquirido)
        {
            ...
            CobrancaBoleto.GerarBoleto(objPagamento);
        }
        #endregion
            ...
            try
            {
                if (IdFilial.HasValue)
                {
                    ...
                    if (PlanoAdquirido.CarregarPlanoVigentePessoaJuridicaPorSituacao(new Filial(IdFilial.Value), new PlanoSituacao((int)Enumeradores.PlanoSituacao.Liberado), out objPlanoAdquirido))
                    {
                        ...
                        PlanoQuantidade objPlanoQuantidade;
                        if (PlanoQuantidade.CarregarPlanoAtualVigente(IdFilial.Value, out objPlanoQuantidade))
                        {
                            ...
                            const string templateDescricao = "Plano {NomePlano} atualizado. Visualizacao: {VisualizacaoAnterior} para {VisualizacaoAtual} / SMS: {SMSAnterior} para {SMSAtual}";
        ...
        protected void btnLiberarCliente_Click(object sender, EventArgs e)
        {
            ...
            {
                PlanoAdquirido objPlanoAdquirido = PlanoAdquirido.LoadObject(IdPlanoAdquirido.Value);
                objPlanoAdquirido.LiberarPlanoAdquirido(new UsuarioFilialPerfil(base.IdUsuarioFilialPerfilLogadoUsuarioInterno.Value));
        ...
        protected void btnSalvarSituacaoFilial_Click(object sender, EventArgs e)
        {
            ...
            {
                Filial objFilial = Filial.LoadObject(IdFilial.Value);
                objFilial.SituacaoFilial = new SituacaoFilial(Convert.ToInt32(rcbSituacaoEmpresa.SelectedValue));
