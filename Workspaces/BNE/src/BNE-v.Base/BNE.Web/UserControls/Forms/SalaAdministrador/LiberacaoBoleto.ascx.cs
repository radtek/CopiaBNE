        ...
        protected void btEnviar_Click(object sender, EventArgs e)
        {
            if (fAnexo.HasFile)
            {
                try
                {
                    ...
                    {
                        string falha;
                        if ((linha.CobrancaBoleto == null && linha.Transacao == null) || linha.Falha(out falha)) continue;
                        ...
                        {
                            objPagamento = linha.CobrancaBoleto.Pagamento;
                            infoBol.Nome = linha.CobrancaBoleto.NomePessoaSacado;
                            infoBol.NumeroBoleto = linha.CobrancaBoleto.NumeroNossoNumero;
                            infoBol.NumeroDocumento = linha.CobrancaBoleto.NumeroCPFSacado.HasValue ? linha.CobrancaBoleto.NumeroCPFSacado.Value : linha.CobrancaBoleto.NumeroCNPJSacado.Value;
                        }
                        ...
                        if (String.IsNullOrEmpty(objPagamento.PlanoParcela.PlanoAdquirido.Plano.DescricaoPlano))
                            objPagamento.PlanoParcela.PlanoAdquirido.Plano.CompleteObject();
                        infoBol.Plano = objPagamento.PlanoParcela.PlanoAdquirido.Plano.DescricaoPlano;
                        if (String.IsNullOrEmpty(objPagamento.TipoPagamento.DescricaoTipoPagamaneto))
                        ...
                        if (objPagamento.PlanoParcela.PlanoAdquirido.Plano.PlanoTipo == null)
                            objPagamento.PlanoParcela.PlanoAdquirido.Plano.CompleteObject();
                        if (objPagamento.PlanoParcela.PlanoAdquirido.Plano.PlanoTipo.IdPlanoTipo == (int)Enumeradores.PlanoTipo.PessoaFisica)
                        {
