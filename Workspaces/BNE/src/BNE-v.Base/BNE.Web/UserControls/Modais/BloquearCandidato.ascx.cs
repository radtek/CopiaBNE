...
namespace BNE.Web.UserControls.Modais
{
    public partial class BloquearCandidato : BaseUserControl
    {
        ...
        public void InicializarBloquear(int? idCurriculo, int? idFilial, string Nome)
        {
        ...
        protected void btnDesbloquearCandidato_Click(object sender, EventArgs e)
        {
            ...
            if (IDCurriculo.HasValue)
            {
                if (Curriculo.DesbloquearCurriculo(IDCurriculo.Value, tbxMotivo.Text.Trim(), base.IdUsuarioFilialPerfilLogadoUsuarioInterno.Value))
                {
                    if (Fechar != null)
                    {
                        tbxMotivo.Text = string.Empty;
                        Fechar("Currículo Desbloqueado com Sucesso!");
                    }
                }
            }
        ...
        protected void btnSimBloquear_Click(object sender, EventArgs e)
        {
            if (IDCurriculo.HasValue)
            {
                if (Curriculo.BloquearCurriculo(IDCurriculo.Value, tbxMotivo.Text.Trim(), base.IdUsuarioFilialPerfilLogadoUsuarioInterno.Value))
                {
                    if (Fechar != null)
                    {
                        tbxMotivo.Text = string.Empty;
