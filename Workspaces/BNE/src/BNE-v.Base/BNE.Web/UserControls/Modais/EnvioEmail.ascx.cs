    ...
    public partial class EnvioEmail : System.Web.UI.UserControl
    {
        ...
        protected void btnEnviar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Tipo == EnvioEmail.TipoEnvioEmail.BoletoPagamento)
                {
                    para = txtEmail.Valor;
                    de = EnderecoEmail;
                }
                ...
                if (ArquivoAnexo != null)
                {
                    ...
                    {
                        byte[] pdf;
                        string fileName = CobrancaBoleto.nome_pdf;
         

                        pdf = PDF.RecuperarPDFUsandoTextSharp("ArquivosTemporarios/" + fileName);

                        EmailSenderFactory
                        .Create(TipoEnviadorEmail.Fila)
                        .Enviar(assunto, mensagem, de, para, ArquivoAnexo.First().Key, pdf);
                    }
                    else
                    {
                        EmailSenderFactory
                        .Create(TipoEnviadorEmail.Fila)
                        .Enviar(assunto, mensagem, de, para, ArquivoAnexo.First().Key, ArquivoAnexo.First().Value);
