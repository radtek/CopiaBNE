...
namespace BNE.Web
{
    public partial class PesquisaVaga : BasePage
    {
        ...
        #region btnModalConfirmacaoIPhoneConfimar_Click
        protected void btnModalConfirmacaoIPhoneConfimar_Click(object sender, EventArgs e)
        {
            mpeModalConfirmacaoIPhone.Hide();
            ValidarCandidato();
        }
        #endregion

        #region btnModalConfirmacaoIPhoneCancelar_Click
        protected void btnModalConfirmacaoIPhoneCancelar_Click(object sender, EventArgs e)
        {
            mpeModalConfirmacaoIPhone.Hide();
        }
        #endregion
        ...
        private IEnumerable TratamentoParaWebEstagios(BLL.PesquisaVaga objPesquisaVaga, DataTable buscaVagaFullTextResult)
        {
            ...
            if (objPesquisaVaga != null)
            {
                var tipoVinculos = PesquisaVagaTipoVinculo.ListarPorPesquisaList(objPesquisaVaga.IdPesquisaVaga);

                if (tipoVinculos.Count == 1
                    && tipoVinculos.Any(obj => obj.TipoVinculo.IdTipoVinculo == (int)BNE.BLL.Enumeradores.TipoVinculo.Estágio))
                {
            ...
            //Valdecir 02/10/2014 - erro resultado de pesquisa - vagas de estagio aparecendo como vagas de emprego.. remover este bloco 
          ...
          /*  int moveCount = 0;
            for (int i = 0; i < originalCopy.Length; i++)
            {
                ...
                if (!Convert.IsDBNull(original.Des_Tipo_Vinculo)
                    && original.Des_Tipo_Vinculo.IndexOf("Estágio", StringComparison.OrdinalIgnoreCase) > -1)
                {
                    var vagaWebEstagios = original.ShallowCopy(); // BNE.Web.Code.DynamicRow object
                    vagaWebEstagios.Des_Tipo_Vinculo = "Estágio";

                    original.Des_Tipo_Vinculo = original.Des_Tipo_Vinculo.Replace("Estágio", "");
                    collectionResult.Insert(i + moveCount, vagaWebEstagios);
                    moveCount = moveCount + 1;
                }
            }*/

            //var duplicateCount = collectionResult.Count - originalCopy.Length;
        ...
        private bool AdicionarFiltroTipoContratoEm(BLL.PesquisaVaga objPesquisaVaga)
        {
            ...
            {
                var vinculos =
                    PesquisaVagaTipoVinculo.ListarPorPesquisaList(IdPesquisaVaga ?? objPesquisaVaga.IdPesquisaVaga);
                ...
                {
                    addPesquisaVinculoAprendiz =
                        vinculos.All(
                            obj =>
                                obj.TipoVinculo.IdTipoVinculo !=
                                (int)Enumeradores.TipoVinculo.Aprendiz);
                    addPesquisaVinculoEstagio =
                        vinculos.All(
                            obj =>
                                obj.TipoVinculo.IdTipoVinculo !=
                                (int)Enumeradores.TipoVinculo.Estágio);
                }
            }
            var novosVinculos = new List<PesquisaVagaTipoVinculo>();
            bool save = false;
        ...
        private void CandidatarVaga(Vaga objVaga, Curriculo objCurriculo, UserControls.Modais.VagaRespostaEventArgs e)
        {
            ...
            VagaCandidato.Candidatar(objCurriculo, objVaga, objOrigem, listVagaPergunta,
                Common.Helper.RecuperarIP(), base.STC.Value, (objOrigemFilial != null && !objOrigemFilial.Filial.PossuiSTCUniversitario() && !objOrigemFilial.Filial.PossuiSTCLanhouse()), false, out quantidadeCandidaturas);
        ...
        #region ExibirModalConfirmacaoIPhone
        private void ExibirModalConfirmacaoIPhone(string nome)
        {
            litNome.Text = nome;
            upNome.Update();
            mpeModalConfirmacaoIPhone.Show();
        }
        #endregion
        ...
        {
            string url = "/img/bne/logo_bne_facebook.png";

            if (base.STC.HasValue && base.STC.Value == true)
            {
                url = "/img/bne/logo_vaga_facebook.png";
            }

            return String.Concat("http://", BNE.Web.Code.UIHelper.RecuperarURLAmbiente(), url);
        }
