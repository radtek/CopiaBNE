...
namespace BNE.BLL
{
    public partial class Arquivo // Tabela: BNE_Arquivo
    {
        ...
        public static Arquivo GerarArquivo(Enumeradores.TipoArquivo tipoArquivo)
        {
            ...
            switch (tipoArquivo)
            {
                case BNE.BLL.Enumeradores.TipoArquivo.RemessaRegistroBoletos: //REMESSA - Boletos registrados
                    CobrancaBoleto.ListaBoletosParaRegistro().ForEach(b => lstInclusao.Add(b));
                    CobrancaBoleto.ListaBoletosParaCancelamento().ForEach(b => lstCancelamento.Add(b));
                    break;
        ...
        /// </summary>
        /// <param name="lstInclusao">Lista de objetos do tipo CobrancaBoleto ou Transacao para gerar os registros de inclusao</param>
        /// <param name="lstCancelamento">Lista de objetos do tipo CobrancaBoleto ou Transacao para gerar os registros de cancelamento</param>
        /// <returns></returns>
        private static Arquivo GerarArquivo(List<object> lstInclusao, List<object> lstCancelamento, Enumeradores.TipoArquivo tipoArquivo)
        {
            ...
            try
            {
                ...
                foreach (object objCobrancaBoleto in lstInclusao)
                {
                    LinhaArquivo.GerarRegistroRemessa(objArquivo, objCobrancaBoleto);
                }

                foreach (object objCobrancaBoleto in lstCancelamento)
                {
                    LinhaArquivo.GerarRegistroRemessa(objArquivo, objCobrancaBoleto, false);
                }
        ...
        private LinhaArquivo AdicionarLinha(String linha)
        {
            ...
            //Ao definir a descricao do conteudo, o tipo da linha já é reconhecida e o objeto Transacao/CobrancaBoleto já é carregado.
        ...
        private void ConsistirRetorno(int idUsuarioLogado)
        {
            ...
            using (SqlConnection conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                foreach (LinhaArquivo linha in this.Linhas)
                {
                    ...
                    if (linha.CobrancaBoleto == null && linha.Transacao == null)
                    {
                        continue;
                    }
                        ...
                        try
                        {
                            ...
                            if (linha.CobrancaBoleto != null)
                            {
                                ...
                                if (linha.ConfirmacaoCancelamento())
                                {
                                    ...
                                    {
                                        linha.CobrancaBoleto.Liquidar(trans, linha.RecuperarDataPagamento(), idUsuarioLogado);
