...
namespace BNE.BLL
{
	public partial class Filial // Tabela: TAB_Filial
	{
		...
		public bool FlagOfereceCursos
		{
			get
			{
				return this._flagOfereceCursos;
			}
			set
			{
		...
		public string DescricaoPaginaFacebook
		{
			get
			{
				return this._descricaoPaginaFacebook;
			}
			set
			{
        ...
        public int? QuantidadeUsuarioAdicional
        {
            get
            {
                return this._quantidadeUsuarioAdicional;
            }
            set
            {
		...
		#region Consultas
                private const string SPINSERT = "INSERT INTO TAB_Filial (Flg_Matriz, Num_CNPJ, Raz_Social, Nme_Fantasia, Idf_CNAE_Principal, Idf_Natureza_Juridica, Idf_Endereco, End_Site, Num_DDD_Comercial, Num_Comercial, Flg_Inativo, Dta_Cadastro, Dta_Alteracao, Qtd_Funcionarios, Des_IP, Flg_Oferece_Cursos, Idf_Situacao_Filial, Des_Pagina_Facebook, Qtd_Usuario_Adicional, Des_Localizacao, Idf_Tipo_Parceiro) VALUES (@Flg_Matriz, @Num_CNPJ, @Raz_Social, @Nme_Fantasia, @Idf_CNAE_Principal, @Idf_Natureza_Juridica, @Idf_Endereco, @End_Site, @Num_DDD_Comercial, @Num_Comercial, @Flg_Inativo, @Dta_Cadastro, @Dta_Alteracao, @Qtd_Funcionarios, @Des_IP, @Flg_Oferece_Cursos, @Idf_Situacao_Filial, @Des_Pagina_Facebook, @Qtd_Usuario_Adicional, @Des_Localizacao, @Idf_Tipo_Parceiro);SET @Idf_Filial = SCOPE_IDENTITY();";
                private const string SPUPDATE = "UPDATE TAB_Filial SET Flg_Matriz = @Flg_Matriz, Num_CNPJ = @Num_CNPJ, Raz_Social = @Raz_Social, Nme_Fantasia = @Nme_Fantasia, Idf_CNAE_Principal = @Idf_CNAE_Principal, Idf_Natureza_Juridica = @Idf_Natureza_Juridica, Idf_Endereco = @Idf_Endereco, End_Site = @End_Site, Num_DDD_Comercial = @Num_DDD_Comercial, Num_Comercial = @Num_Comercial, Flg_Inativo = @Flg_Inativo, Dta_Cadastro = @Dta_Cadastro, Dta_Alteracao = @Dta_Alteracao, Qtd_Funcionarios = @Qtd_Funcionarios, Des_IP = @Des_IP, Flg_Oferece_Cursos = @Flg_Oferece_Cursos, Idf_Situacao_Filial = @Idf_Situacao_Filial, Des_Pagina_Facebook = @Des_Pagina_Facebook, Qtd_Usuario_Adicional = @Qtd_Usuario_Adicional, Des_Localizacao = @Des_Localizacao, Idf_Tipo_Parceiro = @Idf_Tipo_Parceiro WHERE Idf_Filial = @Idf_Filial";
		private const string SPDELETE = "DELETE FROM TAB_Filial WHERE Idf_Filial = @Idf_Filial";
		private const string SPSELECTID = "SELECT * FROM TAB_Filial WHERE Idf_Filial = @Idf_Filial";
		#endregion
		...
		private void SetParameters(List<SqlParameter> parms)
		{
			...
			parms[14].Value = this._quantidadeFuncionarios;
			parms[15].Value = this._descricaoIP;
			parms[16].Value = this._flagOfereceCursos;
			parms[17].Value = this._situacaoFilial.IdSituacaoFilial;

			if (!String.IsNullOrEmpty(this._descricaoPaginaFacebook))
				parms[18].Value = this._descricaoPaginaFacebook;
			else
				parms[18].Value = DBNull.Value;

            if (this._quantidadeUsuarioAdicional != null)
                parms[19].Value = this._quantidadeUsuarioAdicional;
            else
                parms[19].Value = DBNull.Value;

            if (this._descricaoLocalizacao != null)
                parms[20].Value = this._descricaoLocalizacao;
            else
                parms[20].Value = SqlGeography.Null;

            if (this._tipoParceiro != null)
                parms[21].Value = this._tipoParceiro.IdTipoParceiro;
            else
		...
		/// <returns>Verdadeiro ou falso informando se a operação foi executada com sucesso.</returns>
		/// <remarks>Gieyson Stelmak</remarks>
		private static bool SetInstance(IDataReader dr, Filial objFilial)
		{
			try
			{
				if (dr.Read())
