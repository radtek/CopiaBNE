...
namespace BNE.BLL
{
    public partial class LinhaArquivo // Tabela: BNE_Linha_Arquivo
    {
        ...
        /// Campo obrigatório.
        /// Ao ser definido um valor, reconhece o tipo de linha, consistindo o conteúdo com o tipo do arquivo e carrega o objeto Transacao ou CobrancaBoleto relativo.
        /// </summary>
        ...
        /// <summary>
        /// Gera um registro de remessa no objArquivo informado para o objeto tipo Transacao ou CobrancaBoleto informado
        /// </summary>
        /// <param name="objArquivo">Arquivo onde o registro deve ser gerado</param>
        /// <param name="objCobranca">Objeto do tipo Transacao ou CobrancaBoleto relativo ao qual o registro será gerado</param>
        /// <exception cref="Exception">Lança uma exceção caso não seja possível gerar o registro.</exception>
        /// <exception cref="Exception">Lança uma exceção caso o parâmetro objCobranca não seja do tipo CobrancaBoleto ou Transacao.</exception>
        public static void GerarRegistroRemessa(Arquivo objArquivo, object objCobranca, bool inclusao = true)
        {
            if (objCobranca.GetType() != typeof(CobrancaBoleto) && objCobranca.GetType() != typeof(Transacao))
            {
                throw new Exception(String.Format("Somente instancias das classes CobrancaBoleto ou Transacao são aceitas (objCobranca informado é do tipo {0})", typeof(Transacao).ToString()));
            }

            switch ((Enumeradores.TipoArquivo)objArquivo.TipoArquivo.IdTipoArquivo)
            {
                case BNE.BLL.Enumeradores.TipoArquivo.RemessaRegistroBoletos:
                    GerarRegistroRegistroBoletoHSBC(objArquivo, (CobrancaBoleto)objCobranca, inclusao);
                    break;
                case BNE.BLL.Enumeradores.TipoArquivo.RemessaDebitoHSBC:
        ...
        /// <param name="objCobrancaBoleto">Objeto CobrancaBoleto para qual a linha deve ser gerada</param>
        private static void GerarRegistroRegistroBoletoHSBC(Arquivo objArquivo, CobrancaBoleto objCobrancaBoleto, bool inclusao = true)
        {
            String linha = "";
            String temp = "";
            ...
            linha += "02"; //CNPJ
            //Número de Inscricao
            linha += Helper.NumericoParaTamanhoExato(objCobrancaBoleto.NumeroCNPJCedente.Value, 14);
            //Zero
            linha += "0";
            // Agência Cedente
            ...
            linha += Helper.NumericoParaTamanhoExato(objCobrancaBoleto.NumeroNossoNumero, 25);
            //Nosso Número
            ...
            //Vencimento
            linha += objCobrancaBoleto.DataVencimento.Value.ToString("ddMMyy");
            //Valor do Titulo
            linha += Helper.MonetarioParaTamanhoExato(objCobrancaBoleto.ValorBoleto.Value, 13);
            //Banco Cobrador
            linha += "399";
            //Agência Depositária
            ...
            linha += "A"; //Aceito
            //Data Emissão
            linha += objCobrancaBoleto.DataEmissao.Value.ToString("ddMMyy");
            //Instrução 1
            linha += "67";
            //Instrução 2
            linha += "00";
            ...
            linha += Helper.MonetarioParaTamanhoExato(0, 13);
            //Codigo de Inscricao
            if (objCobrancaBoleto.NumeroCPFSacado.HasValue)
            {
                linha += Helper.NumericoParaTamanhoExato(1, 2);//CPF
            }
            else if (objCobrancaBoleto.NumeroCNPJSacado.HasValue)
            {
            ...
            if (objCobrancaBoleto.NumeroCPFSacado.HasValue)
            {
                linha += Helper.NumericoParaTamanhoExato(objCobrancaBoleto.NumeroCPFSacado.Value, 14);//CPF
            }
            else if (objCobrancaBoleto.NumeroCNPJSacado.HasValue)
            {
                linha += Helper.NumericoParaTamanhoExato(objCobrancaBoleto.NumeroCNPJSacado.Value, 14);//CNPJ
            }
            else
            {
            ...
            //Nome do Pagador
            if (!String.IsNullOrEmpty(objCobrancaBoleto.RazaoSocialSacado))
            {
                linha += Helper.StringParaTamanhoExato(objCobrancaBoleto.RazaoSocialSacado, 40);
            }
            else
            {
                linha += Helper.StringParaTamanhoExato(objCobrancaBoleto.NomePessoaSacado, 40);
            }
            //Endereço do pagador
            linha += Helper.StringParaTamanhoExato(objCobrancaBoleto.DescricaoLogradouroSacado, 38);
            //Instruções de não recebimento do boleto
            linha += "  ";
            //Bairro do pagador
            linha += Helper.StringParaTamanhoExato(objCobrancaBoleto.DescricaoBairroSacado, 12);
            //Cep do Pagador + Sufixo do pagador
            linha += Helper.NumericoParaTamanhoExato(Convert.ToDecimal(objCobrancaBoleto.NumeroCepSacado), 8);
            //Cidade do Pagador
            if (String.IsNullOrEmpty(objCobrancaBoleto.CidadeSacado.NomeCidade))
            {
                objCobrancaBoleto.CidadeSacado.CompleteObject();
            }
            linha += Helper.StringParaTamanhoExato(objCobrancaBoleto.CidadeSacado.NomeCidade, 15);
            //Sigla da UF
            linha += Helper.StringParaTamanhoExato(objCobrancaBoleto.CidadeSacado.Estado.SiglaEstado, 2);
            //Sacador / Avalista
        ...
        private static void GerarRemessaDebitoHSBC(Arquivo objArquivo, Transacao objTransacao, bool inclusao = true)
        {
            ...
            //Data de vencimento tem que ser no mínimo 4 dias úteis. Para garantir o débito, enviaremos 7 dias para vencimento.
            Int32 diasMinimoParaVencimento = Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.PagamentoDebitoDiasMinimosVencimentoParaEnvio));
            DateTime dtaVencimento = objPagamento.DataVencimento.Value;
            if ((objPagamento.DataVencimento.Value - DateTime.Now).TotalDays < diasMinimoParaVencimento)
            {
        ...
        /// <summary>
        /// Carrega o objeto Transacao/CobrancaBoleto relativo à linha.
        /// </summary>
        public void CarregarTransacoes()
        {
            switch ((Enumeradores.TipoLinhaArquivo)this.TipoLinhaArquivo.IdTipoLinhaArquivo)
            {
                ...
                case BNE.BLL.Enumeradores.TipoLinhaArquivo.RetornoRegistroBoletoHSBC:
                    String CodigoDoDocumento = this.DescricaoConteudo.Substring(37, 13);
                    CobrancaBoleto objCobrancaBoleto;
                    if (CobrancaBoleto.CarregarPeloNossoNumero(CodigoDoDocumento, out objCobrancaBoleto))
                    {
                        this.CobrancaBoleto = objCobrancaBoleto;
                    }
                    else
                    {
        ...
        public bool Falha(out String erro)
        {
            ...
            switch ((Enumeradores.TipoLinhaArquivo)this.TipoLinhaArquivo.IdTipoLinhaArquivo)
            {
                ...
                case BNE.BLL.Enumeradores.TipoLinhaArquivo.RetornoDebitoHSBC:
                    ...
                    CodigosDeFalha.Add("01", "Débito não efetuado - insulficiencia de saldo");
                    ...
                    if (CodigosDeFalha[s] != null) //Falha
                    {
