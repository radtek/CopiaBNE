...
namespace BNE.BLL
{
    public partial class CurriculoQuemMeViu // Tabela: BNE_Curriculo_Quem_Me_Viu
    {
        ...
        FROM    BNE_Curriculo_Quem_Me_Viu WITH(NOLOCK)
        WHERE   Idf_Filial = @Idf_Filial 
                AND DATEADD(DAY,30,Dta_Quem_Me_Viu) >= GETDATE()";
        #endregion
        ...
        WHERE c.Flg_Vip = 0 
        AND c.Flg_Inativo = 0 
        AND c.Idf_Situacao_Curriculo NOT IN (5,6) 
        ...
        /// <returns></returns>
        public static List<int> ListarCurriculoVisualizados(int idFilial)
        {
        ...
        public static void SalvarQuemMeViu(Filial objFilial, Curriculo objCurriculo)
        {
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            var objCurriculoQuemMeViu = new CurriculoQuemMeViu
                            {
        ...
        /// <param name="totalRegistros">Total de registros encontrados</param>
        /// <returns>DataTable com os registros encontrados</returns>
        public static DataTable RecuperarQuemMeViuAdministrador(int idCurriculo, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
        {
            var parms = new List<SqlParameter>
                {
                    new SqlParameter{ ParameterName = "@Idf_Curriculo", SqlDbType = SqlDbType.Int, Size = 4, Value = idCurriculo}, 
        ...
        public static int QuantidadeVisualizacaoCurriculo(int idPessoaFisica)
        {
            var parms = new List<SqlParameter>
                {
