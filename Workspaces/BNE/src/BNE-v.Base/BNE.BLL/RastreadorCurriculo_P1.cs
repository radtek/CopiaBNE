...
namespace BNE.BLL
{
	public partial class RastreadorCurriculo // Tabela: BNE_Rastreador_Curriculo
	{
		...
		#region Consultas
		private const string SPINSERT = "INSERT INTO BNE_Rastreador_Curriculo (Idf_Rastreador, Idf_Curriculo, Dta_Cadastro, Flg_Inativo) VALUES (@Idf_Rastreador, @Idf_Curriculo, @Dta_Cadastro, @Flg_Inativo);SET @Idf_Rastreador_Curriculo = SCOPE_IDENTITY();";
		private const string SPUPDATE = "UPDATE BNE_Rastreador_Curriculo SET Idf_Rastreador = @Idf_Rastreador, Idf_Curriculo = @Idf_Curriculo, Dta_Cadastro = @Dta_Cadastro, Flg_Inativo = @Flg_Inativo WHERE Idf_Rastreador_Curriculo = @Idf_Rastreador_Curriculo";
		...
		private const string SPSELECTID = "SELECT * FROM BNE_Rastreador_Curriculo WHERE Idf_Rastreador_Curriculo = @Idf_Rastreador_Curriculo";
		#endregion
		...
		public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
		{
			...
			string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, Ras.Idf_Rastreador_Curriculo, Ras.Idf_Rastreador, Ras.Idf_Curriculo, Ras.Dta_Cadastro, Ras.Flg_Inativo FROM BNE_Rastreador_Curriculo Ras";
		...
		/// <returns>Verdadeiro ou falso informando se a operação foi executada com sucesso.</returns>
		/// <remarks>Gieyson Stelmak</remarks>
		private static bool SetInstance(IDataReader dr, RastreadorCurriculo objRastreadorCurriculo)
		{
			try
			{
				if (dr.Read())
				{
					...
					objRastreadorCurriculo._rastreador = new Rastreador(Convert.ToInt32(dr["Idf_Rastreador"]));
					objRastreadorCurriculo._curriculo = new Curriculo(Convert.ToInt32(dr["Idf_Curriculo"]));
