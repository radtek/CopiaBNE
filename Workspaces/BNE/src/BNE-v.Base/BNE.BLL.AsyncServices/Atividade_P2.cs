...
namespace BNE.BLL.AsyncServices
{
    public partial class Atividade // Tabela: TAB_Atividade
    {
        ...
        #region Spinformararquivogerado
        private const String Spinformararquivogerado = @"update TAB_Atividade set Des_Caminho_Arquivo_Gerado = @Des_Caminho_Arquivo_Gerado where Idf_Atividade = @Idf_Atividade";
        #endregion
        ...
        from  
          TAB_Atividade atv
          join plataforma.TAB_Plugins_Compatibilidade pc on (atv.Idf_Plugins_Compatibilidade = pc.Idf_Plugins_Compatibilidade)
          join plataforma.TAB_Plugin plu on (pc.Idf_Plugin_Entrada = plu.Idf_Plugin)
          join plataforma.TAB_Tipo_Atividade ta on (plu.Idf_Tipo_Atividade = ta.Idf_Tipo_Atividade)  
        where 
        ...
        @"
        select 
          atv.Idf_Atividade, plu.Idf_Tipo_Atividade 
        from 
          TAB_Atividade atv
          join plataforma.TAB_Plugins_Compatibilidade pc on (atv.Idf_Plugins_Compatibilidade = pc.Idf_Plugins_Compatibilidade)
          join plataforma.TAB_Plugin plu on (pc.Idf_Plugin_Entrada = plu.Idf_Plugin)
        ...
        private static bool SetInstance(IDataReader dr, Atividade objAtividade)
        {
            try
            {
                if (dr.Read())
                {
                    ...
                    if (dr["Des_Caminho_Arquivo_Gerado"] != DBNull.Value)
                        objAtividade._descricaoCaminhoArquivoGerado = Convert.ToString(dr["Des_Caminho_Arquivo_Gerado"]);
                    ...
                    if (dr["Idf_Tipo_Saida"] != DBNull.Value)
                        objAtividade._tipoSaida = new TipoSaida(Convert.ToInt32(dr["Idf_Tipo_Saida"]));
            ...
            var parms = new List<SqlParameter>
                {
                    ...
                    new SqlParameter("@Idf_Tipo_Saida", SqlDbType.Int, 4),
                };
            return (parms);
        }
        #endregion
        ...
        private void SetParameters(List<SqlParameter> parms)
        {
            ...
            if (this._tipoSaida != null)
                parms[12].Value = this._tipoSaida.IdTipoSaida;
            else
        ...
        #region InformarArquivoGerado
        /// <summary>
        /// Informar na atividade qual foi o arquivo gerado
        /// </summary>
        /// <param name="idAtividade">A atividade a ser atualizada</param>
        /// <param name="desCaminhoArquivo">O arquivo gerado</param>
        public static void InformarArquivoGerado(int idAtividade, String desCaminhoArquivo)
        {
            var parms = new List<SqlParameter>
                {
                    new SqlParameter { ParameterName = "@Idf_Atividade", SqlDbType = SqlDbType.Int, Value = idAtividade } ,
                    new SqlParameter { ParameterName = "@Des_Caminho_Arquivo_Gerado", SqlDbType = SqlDbType.Text, Value = desCaminhoArquivo }
                };

            DataAccessLayer.ExecuteNonQuery(CommandType.Text, Spinformararquivogerado, parms);
        }
        #endregion
