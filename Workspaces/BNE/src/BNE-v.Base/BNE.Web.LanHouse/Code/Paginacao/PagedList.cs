...
namespace BNE.Web.LanHouse.Code.Paginacao
{
    public class PagedList<T> : List<T>, IPagedList<T>
    {
        ...
        {
            if (index < 0)
                throw new ArgumentOutOfRangeException("index", "Valor não pode ser menor que 0.");
            if (pageSize < 1)
                throw new ArgumentOutOfRangeException("pageSize", "Valor não pode ser menor que 1.");
            ...
            Populate(source, realTotalPages, realTotalCount);
        }

        public virtual void Populate(IQueryable<T> source, int realTotalPages, int realTotalCount)
        {
