...
{
    partial class AllInEmailSincronizacaoLista : ServiceBase
    {
        private const string EventSourceName = "AllInEmailSincronizacaoLista";
        ...
        public string FtpHost
        {
            get
            {
                return Settings.Default.AllInFtpHost;
            }
        }
        ...
        public double DaysLimitToRemoveOldFiles
        {
            get
            {
                return Settings.Default.AllInDiasLimiteHistorico;
            }
        }
        #endregion
                    ...
                    DoCandidato(firstRunning).ContinueWith(t =>
                       {
                           if (t.Status == TaskStatus.RanToCompletion)
                               EventLogWriter.LogEvent(EventSourceName, String.Format("Exportação de currículos da plataforma AllIn Finalizado: {0}.", DateTime.Now), EventLogEntryType.Information, (int)EventID.InicioExecucao);
                           else
                               EventLogWriter.LogEvent(EventSourceName, String.Format("Exportação de currículos da plataforma AllIn ERRO ao completar ({0}): {1}.", t.Status, DateTime.Now), EventLogEntryType.Information, (int)EventID.InicioExecucao);
                       })
                       .ContinueWith(t =>
                       {
                           return DoEmpresa(firstRunning);
                       }).Unwrap()
                           .ContinueWith(t =>
                               {
                                   if (t.Status == TaskStatus.RanToCompletion)
                                       EventLogWriter.LogEvent(EventSourceName, String.Format("Exportação de empresas da plataforma AllIn Finalizado: {0}.", DateTime.Now), EventLogEntryType.Information, (int)EventID.InicioExecucao);
                                   else
                                       EventLogWriter.LogEvent(EventSourceName, String.Format("Exportação de empresas da plataforma AllIn ERRO ao completar ({0}): {1}.", t.Status, DateTime.Now), EventLogEntryType.Information, (int)EventID.InicioExecucao);
        ...
        {
            EventLogWriter.LogEvent(EventSourceName, String.Format("Exportação de e-mails e currículos da plataforma AllIn Iniciado: {0}.", DateTime.Now), EventLogEntryType.Information, (int)EventID.InicioExecucao);

            var login = new BNE.BLL.Parametro((int)BLL.Enumeradores.Parametro.AllinFtpBneLogin);
            var senha = new BNE.BLL.Parametro((int)BLL.Enumeradores.Parametro.AllinFtpBneSenha);
            var fileName = new BNE.BLL.Parametro((int)BLL.Enumeradores.Parametro.AllinAtualizacaoArquivoNome);
            var fromDate = new BNE.BLL.Parametro((int)BLL.Enumeradores.Parametro.AllinAtualizacaoCurriculosApartirDe);
            ...
            handleParameters(() => login.CompleteObject(), string.Format("login ({0}={1})", BLL.Enumeradores.Parametro.AllinFtpBneLogin.ToString(), (int)BLL.Enumeradores.Parametro.AllinFtpBneLogin));
            handleParameters(() => senha.CompleteObject(), string.Format("senha ({0}={1})", BLL.Enumeradores.Parametro.AllinFtpBneSenha.ToString(), (int)BLL.Enumeradores.Parametro.AllinFtpBneSenha));
            handleParameters(() => fileName.CompleteObject(), string.Format("fileName ({0}={1})", BLL.Enumeradores.Parametro.AllinAtualizacaoArquivoNome.ToString(), (int)BLL.Enumeradores.Parametro.AllinAtualizacaoArquivoNome));
            handleParameters(() => fromDate.CompleteObject(), string.Format("fromDate ({0}={1})", BLL.Enumeradores.Parametro.AllinAtualizacaoCurriculosApartirDe.ToString(), (int)BLL.Enumeradores.Parametro.AllinAtualizacaoCurriculosApartirDe));

            return StartCurriculoProcess(login.ValorParametro, senha.ValorParametro, fileName.ValorParametro, new KeyValuePair<BLL.Parametro, string>(fromDate, fromDate.ValorParametro));
        }


        public Task DoEmpresa(bool firstRunning)
        {
            EventLogWriter.LogEvent(EventSourceName, String.Format("Exportação de e-mails para empresas usando plataforma AllIn Iniciado: {0}.", DateTime.Now), EventLogEntryType.Information, (int)EventID.InicioExecucao);

            var login = new BNE.BLL.Parametro((int)BLL.Enumeradores.Parametro.AllinFtpBneLogin);
            var senha = new BNE.BLL.Parametro((int)BLL.Enumeradores.Parametro.AllinFtpBneSenha);
            var fileName = new BNE.BLL.Parametro((int)BLL.Enumeradores.Parametro.AllinEmpresasArquivoNome);
            var fromDate = new BNE.BLL.Parametro((int)BLL.Enumeradores.Parametro.AllinAtualizacaoEmpresasApartirDe);
            ...
            handleParameters(() => login.CompleteObject(), string.Format("login ({0}={1})", BLL.Enumeradores.Parametro.AllinFtpBneLogin.ToString(), (int)BLL.Enumeradores.Parametro.AllinFtpBneLogin));
            handleParameters(() => senha.CompleteObject(), string.Format("senha ({0}={1})", BLL.Enumeradores.Parametro.AllinFtpBneSenha.ToString(), (int)BLL.Enumeradores.Parametro.AllinFtpBneSenha));
            handleParameters(() => fileName.CompleteObject(), string.Format("fileName ({0}={1})", BLL.Enumeradores.Parametro.AllinEmpresasArquivoNome.ToString(), (int)BLL.Enumeradores.Parametro.AllinEmpresasArquivoNome));
            handleParameters(() => fromDate.CompleteObject(), string.Format("fromDate ({0}={1})", BLL.Enumeradores.Parametro.AllinAtualizacaoEmpresasApartirDe.ToString(), (int)BLL.Enumeradores.Parametro.AllinAtualizacaoEmpresasApartirDe));

            return StartEmpresasProcess(login.ValorParametro, senha.ValorParametro, fileName.ValorParametro, new KeyValuePair<BLL.Parametro, string>(fromDate, fromDate.ValorParametro));
        }
        ...
        public void RemoveOldFiles(FtpClient conn, string file, DateTime scheduledTime)
        {
            ...
            var filterItems = allItems.Where(a => ((a.FullName ?? string.Empty).Split('/').LastOrDefault() ?? string.Empty).StartsWith(toFind, StringComparison.OrdinalIgnoreCase));
            foreach (var item in filterItems)
            {
                var splitName = (item.FullName ?? string.Empty).Split('_');
                splitName = splitName.SelectMany(a => a.Split('.')).ToArray();

                DateTime processDate = default(DateTime);
                if (splitName.Any(a => DateTime.TryParseExact((a ?? string.Empty).Trim(), "ddMMyyyy", CultureInfo.GetCultureInfo("pt-br").DateTimeFormat, DateTimeStyles.None, out processDate)))
                {
        ...
        private DateTimeOffset GetNextExecution()
        {
            ...
            string[] horaMinuto = Settings.Default.AllInAtualizarListaEmails.Split(':');
        ...
        private Task StartEmpresasProcess(string login, string senha, string fileDefaultPath, KeyValuePair<BLL.Parametro, string> date)
        {
            DateTime fromDate;
            if (!DateTime.TryParseExact(date.Value, "dd/MM/yyyy", CultureInfo.GetCultureInfo("pt-br").DateTimeFormat, DateTimeStyles.None, out fromDate))
            {
            ...
            t1.ContinueWith(t =>
            {
                ...
                {
                    LocalLog(new InvalidOperationException(string.Format("ProcessResult='{0}'", t1.Result)), "By Process Result (Empresas)");
                }
        ...
        private Task StartCurriculoProcess(string login, string senha, string fileDefaultPath, KeyValuePair<BLL.Parametro, string> date)
        {
            DateTime fromDate;
            if (!DateTime.TryParseExact(date.Value, "dd/MM/yyyy", CultureInfo.GetCultureInfo("pt-br").DateTimeFormat, DateTimeStyles.None, out fromDate))
            {
            ...
            t1.ContinueWith(t =>
            {
                ...
                {
                    LocalLog(new InvalidOperationException(string.Format("ProcessResult='{0}'", t1.Result)), "By Process Result (Currículos)");
                }
            }, TaskContinuationOptions.OnlyOnRanToCompletion);
        ...
        private void ProcessCompletedWithSuccess(string login, string senha, string fileDefaultPath, KeyValuePair<BLL.Parametro, string> date, DateTime fromDate, DateTime scheduledTime)
        {
        ...
        private KeyValuePair<Lazy<Stream>, System.Reactive.Disposables.CompositeDisposable> GetLazyFtpStreamBuilder(string login, string senha, string file)
        {
            var disposer = new System.Reactive.Disposables.CompositeDisposable();
            ...
            return new KeyValuePair<Lazy<Stream>, System.Reactive.Disposables.CompositeDisposable>(lazyStream, disposer);
        }

        private static void LocalLog(Exception exp, string details)
        {
            EL.GerenciadorException.GravarExcecao(exp, details + " (AllIn)");
            EmailSenderFactory.Create(TipoEnviadorEmail.Fila).Enviar("appBNE - Erro: Serviço de sincronização da AllIn (" + details + ")", exp.DumpExInternal(), "gieyson@bne.com.br", "gieyson@bne.com.br");
        }
    }
