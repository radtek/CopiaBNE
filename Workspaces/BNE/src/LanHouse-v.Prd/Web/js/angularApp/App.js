(function () {
    ...
    var app = angular.module('LanHouse', ['ngRoute', 'LocalStorageModule', 'angular-loading-bar', 'LoginController', 'authService', 'authInterceptorService', 'ui.mask', 'LanDirectives',
        'LanControllers', 'VagasController', 'HeaderController', 'VipController', 'xeditable', 'ui.bootstrap', 'webcam', 'EmpresasController', 'ngCookies', 'camposDirectives', 'angulartics', 'angulartics.google.analytics']);
    ...
    app.controller('CarregarFilialController', ['$q', '$scope', '$http', '$location', 'authService', '$rootScope', '$cookieStore', function ($q, $scope, $http, $location, $authService, $rootScope, $cookieStore) {
        ...
        //Definindo dimensao personalizada com a nome da lan para os relatórios do analytics
        if (window.ga && nomeLan !== undefined && nomeLan != null) {
            window.ga('set', 'dimension1', nomeLan);
        } else {
            window.ga('set', 'dimension1', '(Lan nao definida)');
        }
        ...
        $http.get(urlAPI + "Filial?partialName=" + lan.replace('/', ''))
            .success(function (data, status, headers, config) {
                ...
                if (data === undefined || data == null) {
                    ...
                    //Definindo dimensao personalizada com a nome da lan para os relatórios do analytics
                    if (window.ga && lan !== undefined && lan != null) {
                        window.ga('set', 'dimension1', lan);
                    } else {
                        window.ga('set', 'dimension1', '(Lan nao definida)');
                    }
