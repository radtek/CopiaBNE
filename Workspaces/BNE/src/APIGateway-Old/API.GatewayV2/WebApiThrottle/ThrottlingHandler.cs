...
namespace WebApiThrottle
{
    ...
    public class ThrottlingHandler : DelegatingHandler
    {
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            //@ Por Reinaldo Aparcido
            //  se o cliente tiver key vazia ele não tem permissão de acesso
            if (identity.ClientKey == "") 
            {
                return UsuarioProibidoResponse(request);
            }
            //------------------------
        ...
        protected virtual Task<HttpResponseMessage> UsuarioProibidoResponse(HttpRequestMessage request)
        {
            var response = request.CreateResponse(HttpStatusCode.Unauthorized, "Acesso negado! Verifique se o campo apiKey foi informado no cabeçalho e possui valor correto.");
