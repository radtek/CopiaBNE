...
namespace BNE.BLL.CloudTag
{
	public partial class Funcao // Tabela: TAB_Funcao
	{
		...
		private int? _idFuncaoAgrupadora;
		...
		#region IdFuncaoAgrupadora
		/// <summary>
		/// Campo opcional.
		/// </summary>
		public int? IdFuncaoAgrupadora
		{
			get
			{
				return this._idFuncaoAgrupadora;
			}
			set
			{
				this._idFuncaoAgrupadora = value;
				this._modified = true;
			}
		}
		#endregion 
		...
		private void SetParameters(List<SqlParameter> parms)
		{
			...
			if (this._idFuncaoAgrupadora.HasValue)
				parms[1].Value = this._idFuncaoAgrupadora;
			else
				parms[1].Value = DBNull.Value;

			parms[2].Value = this._descricaoFuncao;
			parms[3].Value = this._idFuncaoCategoria;

			if (this._idClasseSalarial.HasValue)
				parms[4].Value = this._idClasseSalarial;
			else
				parms[4].Value = DBNull.Value;


			if (this._idEscolaridade.HasValue)
				parms[5].Value = this._idEscolaridade;
			else
				parms[5].Value = DBNull.Value;


			if (this._idAreaBNE.HasValue)
				parms[6].Value = this._idAreaBNE;
			else
		...
		{
			List<SqlParameter> parms = new List<SqlParameter>();
			string query = "delete from TAB_Funcao where Idf_Funcao in (";
			...
			try
			{
				if (dr.Read())
				{
					...
					if (dr["Idf_Funcao_Agrupadora"] != DBNull.Value)
						objFuncao._idFuncaoAgrupadora = Convert.ToInt32(dr["Idf_Funcao_Agrupadora"]);
					objFuncao._descricaoFuncao = Convert.ToString(dr["Des_Funcao"]);
					objFuncao._idFuncaoCategoria = Convert.ToInt32(dr["Idf_Funcao_Categoria"]);
					if (dr["Idf_Classe_Salarial"] != DBNull.Value)
						objFuncao._idClasseSalarial = Convert.ToInt32(dr["Idf_Classe_Salarial"]);
					if (dr["Idf_Escolaridade"] != DBNull.Value)
						objFuncao._idEscolaridade = Convert.ToInt32(dr["Idf_Escolaridade"]);
					if (dr["Idf_Area_BNE"] != DBNull.Value)
						objFuncao._idAreaBNE = Convert.ToInt32(dr["Idf_Area_BNE"]);
