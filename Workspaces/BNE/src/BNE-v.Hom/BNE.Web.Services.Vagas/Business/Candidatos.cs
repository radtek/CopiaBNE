...
namespace BNE.Web.Services.Vagas.Business
{
    public class Candidatos
    {
        ...
        (SELECT IIF(COUNT(*) > 0 ,1,0) FROM BNE.TAB_Usuario_Filial_Perfil ufp 
        JOIN BNE.BNE_Plano_Adquirido pa ON ufp.Idf_Usuario_Filial_Perfil = pa.Idf_Usuario_Filial_Perfil  AND pa.Idf_Plano_Situacao = 1
        WHERE GETDATE() BETWEEN pa.Dta_Inicio_Plano AND pa.Dta_Fim_Plano
        AND ufp.Flg_Inativo = 0
        AND Idf_Pessoa_Fisica = pf.Idf_Pessoa_Fisica ) AS Flg_VIP,
        vc.Idf_Curriculo, 
        ...
        public static List<CurriculoCurtoDTO> ObterCandidatos(int idf_vaga, int idf_filial, int pagina, out int total_registros, out int total_paginas)
        {
            ...
            var parmsPagination = new List<SqlParameter>
            {
                ...
                new SqlParameter("@Idf_Filial", SqlDbType.Int) { Value = idf_filial }
            };
                        ...
                        using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, QRY_SELECT_CANDIDATOS, parms))
                        {
                            while(dr.Read())
                            {
                                curriculos.Add(new CurriculoCurtoDTO()
                                {
                                    ...
                                    Vip = dr.IsDBNull(dr.GetOrdinal("Flg_Vip")) ? false : (dr.GetInt32(dr.GetOrdinal("Flg_Vip")) > 0)
                                });
                            }
                        }
                    }
