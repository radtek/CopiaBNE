...
namespace BNE.BLL.Notificacao
{
	public partial class UtmUrl // Tabela: alerta.TAB_UtmUrl
	{
		...
		#region Consultas
		private const string SPINSERT = "INSERT INTO alerta.TAB_UtmUrl (Idf_UtmUrl, Nme_UtmUrl, Vlr_UtmUrl) VALUES (@Idf_UtmUrl, @Nme_UtmUrl, @Vlr_UtmUrl);";
		private const string SPUPDATE = "UPDATE alerta.TAB_UtmUrl SET Nme_UtmUrl = @Nme_UtmUrl, Vlr_UtmUrl = @Vlr_UtmUrl WHERE Idf_UtmUrl = @Idf_UtmUrl";
		private const string SPDELETE = "DELETE FROM alerta.TAB_UtmUrl WHERE Idf_UtmUrl = @Idf_UtmUrl";
		private const string SPSELECTID = "SELECT * FROM alerta.TAB_UtmUrl WITH(NOLOCK) WHERE Idf_UtmUrl = @Idf_UtmUrl";
		#endregion
		...
		private List<SqlParameter> GetParameters()
		{
			...
			parms.Add(new SqlParameter("@Idf_UtmUrl", SqlDbType.Int, 4));
			parms.Add(new SqlParameter("@Nme_UtmUrl", SqlDbType.VarChar, 70));
			parms.Add(new SqlParameter("@Vlr_UtmUrl", SqlDbType.VarChar));
			return(parms);
		}
		#endregion
		...
		{
			List<SqlParameter> parms = new List<SqlParameter>();
			string query = "delete from alerta.TAB_UtmUrl where Idf_UtmUrl in (";
		...
		public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
		{
			...
			string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, ta..Idf_UtmUrl, ta..Nme_UtmUrl, ta..Vlr_UtmUrl FROM alerta.TAB_UtmUrl ta";
		...
		/// <remarks>Gieyson Stelmak</remarks>
		public static UtmUrl LoadObject(int idUtmUrl)
		{
			using (IDataReader dr = LoadDataReader(idUtmUrl))
			{
				UtmUrl objUtmUrl = new UtmUrl();
		...
		/// <returns>Instância de UtmUrl.</returns>
		/// <remarks>Gieyson Stelmak</remarks>
		public static UtmUrl LoadObject(int idUtmUrl, SqlTransaction trans)
		{
		...
		/// <returns>Verdadeiro ou falso informando se a operação foi executada com sucesso.</returns>
		/// <remarks>Gieyson Stelmak</remarks>
		private static bool SetInstance(IDataReader dr, UtmUrl objUtmUrl, bool dispose = true)
		{
			try
			{
				if (dr.Read())
				{
					objUtmUrl._idUtmUrl = Convert.ToInt32(dr["Idf_UtmUrl"]);
					objUtmUrl._nomeUtmUrl = Convert.ToString(dr["Nme_UtmUrl"]);
					objUtmUrl._valorUtmUrl = Convert.ToString(dr["Vlr_UtmUrl"]);
