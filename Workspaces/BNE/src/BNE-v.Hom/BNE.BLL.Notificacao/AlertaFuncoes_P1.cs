...
namespace BNE.BLL.Notificacao
{
	public partial class AlertaFuncoes // Tabela: alerta.Tab_Alerta_Funcoes
	{
        ...
        private const string SPDELETE = "UPDATE alerta.Tab_Alerta_Funcoes SET Flg_Inativo = 1 WHERE Idf_Funcao = @Idf_Funcao AND Idf_Curriculo = @Idf_Curriculo";
		...
		public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
		{
			...
			string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, bAl.Idf_Funcao, bAl.Idf_Curriculo, bAl.Des_Funcao, bAl.Flg_Similar, bAl.Flg_Inativo FROM alerta.Tab_Alerta_Funcoes bAl WITH(NOLOCK) ";
