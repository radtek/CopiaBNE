...
namespace BNE.BLL.Notificacao
{
	public partial class Carta // Tabela: TAB_Carta
    {
		...
		#region Consultas
		private const string SPINSERT = "INSERT INTO TAB_Carta (Idf_Carta, Nme_Carta, Vlr_Carta) VALUES (@Idf_Carta, @Nme_Carta, @Vlr_Carta);";
		private const string SPUPDATE = "UPDATE TAB_Carta SET Nme_Carta = @Nme_Carta, Vlr_Carta = @Vlr_Carta WHERE Idf_Carta = @Idf_Carta";
		private const string SPDELETE = "DELETE FROM TAB_Carta WHERE Idf_Carta = @Idf_Carta";
		private const string SPSELECTID = "SELECT * FROM TAB_Carta WITH(NOLOCK) WHERE Idf_Carta = @Idf_Carta";
		#endregion
		...
		private List<SqlParameter> GetParameters()
		{
			...
			parms.Add(new SqlParameter("@Idf_Carta", SqlDbType.Int, 4));
			parms.Add(new SqlParameter("@Nme_Carta", SqlDbType.VarChar, 70));
			parms.Add(new SqlParameter("@Vlr_Carta", SqlDbType.VarChar));
			return(parms);
		}
		#endregion
		...
		{
			List<SqlParameter> parms = new List<SqlParameter>();
			string query = "delete from TAB_Carta where Idf_Carta in (";
		...
		public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
		{
			...
			string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, Car.Idf_Carta, Car.Nme_Carta, Car.Vlr_Carta, Car.Dta_Cadastro, Car.Des_Assunto FROM TAB_Carta Car";
		...
		/// <remarks>Gieyson Stelmak</remarks>
		public static Carta LoadObject(int idCarta)
		{
		...
		/// <returns>Instância de Carta.</returns>
		/// <remarks>Gieyson Stelmak</remarks>
		public static Carta LoadObject(int idCarta, SqlTransaction trans)
		{
		...
		/// <returns>Verdadeiro ou falso informando se a operação foi executada com sucesso.</returns>
		/// <remarks>Gieyson Stelmak</remarks>
		private static bool SetInstance(IDataReader dr, Carta objCarta)
		{
			try
			{
				if (dr.Read())
				{
					objCarta._idCarta = Convert.ToInt32(dr["Idf_Carta"]);
					objCarta._nomeCarta = Convert.ToString(dr["Nme_Carta"]);
					objCarta._valorCarta = Convert.ToString(dr["Vlr_Carta"]);
