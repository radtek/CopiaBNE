...
namespace Mandrill
{
  ...
  public partial class MandrillApi : IMandrillApi
  {
    ...
    ///   The <see cref="Task" />.
    /// </returns>
    public async Task<List<EmailResult>> SendMessage(SendMessageRequest request, bool withHttpWebRequest = false)
    {
      string path = "messages/send.json";

      List<EmailResult> resp = await Post<List<EmailResult>>(path, request, withHttpWebRequest);
      return resp;
    }
