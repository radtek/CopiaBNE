...
namespace BNE.BLL.AsyncServices
{
    public partial class TipoAtividade // Tabela: TAB_Tipo_Atividade
	{
		...
		private int _numeroDiasExpiracao;
		...
		#region NumeroDiasExpiracao
		/// <summary>
		/// Campo obrigatório.
		/// </summary>
		public int NumeroDiasExpiracao
		{
			get
			{
				return this._numeroDiasExpiracao;
			}
			set
			{
				this._numeroDiasExpiracao = value;
				this._modified = true;
			}
		}
		#endregion 
		...
		#region Consultas
		private const string SPINSERT = "INSERT INTO TAB_Tipo_Atividade (Des_Tipo_Atividade, Flg_Inativo, Num_Dias_Expiracao, Dta_Cadastro) VALUES (@Des_Tipo_Atividade, @Flg_Inativo, @Num_Dias_Expiracao, @Dta_Cadastro);SET @Idf_Tipo_Atividade = SCOPE_IDENTITY();";
		private const string SPUPDATE = "UPDATE TAB_Tipo_Atividade SET Des_Tipo_Atividade = @Des_Tipo_Atividade, Flg_Inativo = @Flg_Inativo, Num_Dias_Expiracao = @Num_Dias_Expiracao, Dta_Cadastro = @Dta_Cadastro WHERE Idf_Tipo_Atividade = @Idf_Tipo_Atividade";
		private const string SPDELETE = "DELETE FROM TAB_Tipo_Atividade WHERE Idf_Tipo_Atividade = @Idf_Tipo_Atividade";
		private const string SPSELECTID = "SELECT * FROM TAB_Tipo_Atividade WHERE Idf_Tipo_Atividade = @Idf_Tipo_Atividade";
		#endregion
		...
		private List<SqlParameter> GetParameters()
		{
			...
			parms.Add(new SqlParameter("@Num_Dias_Expiracao", SqlDbType.Int, 4));
		...
		private void SetParameters(List<SqlParameter> parms)
		{
			...
			parms[3].Value = this._numeroDiasExpiracao;
			...
			using (SqlConnection conn = new SqlConnection(DataAccessLayer.CONN_STRING))
			{
				...
				using (SqlTransaction trans = conn.BeginTransaction())
				{
					try
					{
						...
						this._idTipoAtividade = Convert.ToInt32(cmd.Parameters["@Idf_Tipo_Atividade"].Value);
		...
		{
			List<SqlParameter> parms = new List<SqlParameter>();
			string query = "delete from TAB_Tipo_Atividade where Idf_Tipo_Atividade in (";
		...
		public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
		{
			...
			string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, Tip.Idf_Tipo_Atividade, Tip.Des_Tipo_Atividade, Tip.Flg_Inativo, Tip.Num_Dias_Expiracao, Tip.Dta_Cadastro FROM TAB_Tipo_Atividade Tip";
		...
		private static bool SetInstance(IDataReader dr, TipoAtividade objTipoAtividade)
		{
			try
			{
				if (dr.Read())
				{
					objTipoAtividade._idTipoAtividade = Convert.ToInt32(dr["Idf_Tipo_Atividade"]);
					...
					objTipoAtividade._numeroDiasExpiracao = Convert.ToInt32(dr["Num_Dias_Expiracao"]);
					objTipoAtividade._dataCadastro = Convert.ToDateTime(dr["Dta_Cadastro"]);
