...
namespace BNE.BLL
{
    public partial class Plano // Tabela: BNE_Plano
    {
        ...
        private const string SPQuantidadeMaximaParcelaPlano = @"
            SELECT  Qtd_Parcela
            FROM    BNE.BNE_Plano
            WHERE   Idf_Plano = @idf_plano
        ";
        ...
        public static int QuantidadeMaximaParcela(int idPlanoSelecionado)
        {
            var parametro = new List<SqlParameter>
            {
                new SqlParameter {ParameterName = "@idf_plano", SqlDbType = SqlDbType.Int, Size = 4, Value = idPlanoSelecionado}
            };
            return
                Convert.ToInt32(DataAccessLayer.ExecuteScalar(CommandType.Text, SPQuantidadeMaximaParcelaPlano,
                    parametro));
        }
