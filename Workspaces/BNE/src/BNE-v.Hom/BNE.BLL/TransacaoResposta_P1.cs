...
namespace BNE.BLL
{
	public partial class TransacaoResposta // Tabela: BNE_Transacao_Resposta
	{
        ...
        private const string SPSELECTIDTRANSACAO = "SELECT * FROM BNE_Transacao_Resposta WITH(NOLOCK) WHERE Idf_Transacao = @Idf_Transacao";
        #endregion
        ...
        private List<SqlParameter> GetParameters()
		{
			...
			parms.Add(new SqlParameter("@Des_Codigo_Autorizacao", SqlDbType.VarChar, 20));
			parms.Add(new SqlParameter("@Des_Transacao", SqlDbType.VarChar, 20));
	    ...
	    public static TransacaoResposta AlterarRespostaPorIdTransacao(int idTransacao, SqlTransaction trans = null)
	    {
            var parms = new List<SqlParameter>
                {
                    new SqlParameter { ParameterName = "@Idf_Transacao", SqlDbType = SqlDbType.Int, Size = 8, Value = idTransacao }
                };

            SqlDataReader dr;

            using (dr = DataAccessLayer.ExecuteReader(trans, CommandType.Text, SPSELECTIDTRANSACAO, parms))
            {
                TransacaoResposta objTransacaoResposta = new TransacaoResposta();
                if (SetInstance(dr, objTransacaoResposta))
                    return objTransacaoResposta;
