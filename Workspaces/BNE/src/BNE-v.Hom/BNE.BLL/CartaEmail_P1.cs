...
namespace BNE.BLL
{
    public partial class CartaEmail // Tabela: BNE_Carta_Email
    {
        ...
        #region Consultas
        private const string SPINSERT = "INSERT INTO BNE_Carta_Email (Idf_Carta_Email, Nme_Carta_Email, Vlr_Carta_Email, Dta_Cadastro, Des_Assunto, Idf_Template) VALUES (@Idf_Carta_Email, @Nme_Carta_Email, @Vlr_Carta_Email, @Dta_Cadastro, @Des_Assunto, @Idf_Template);";
        private const string SPUPDATE = "UPDATE BNE_Carta_Email SET Nme_Carta_Email = @Nme_Carta_Email, Vlr_Carta_Email = @Vlr_Carta_Email, Dta_Cadastro = @Dta_Cadastro, Des_Assunto = @Des_Assunto, Idf_Template = @Idf_Template WHERE Idf_Carta_Email = @Idf_Carta_Email";
        ...
        private void SetParameters(List<SqlParameter> parms)
        {
            ...
            if (this._template != null)
                parms[5].Value = this._template.IdCartaEmail;
            else
        ...
        public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
        {
            ...
            string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, Car.Idf_Carta_Email, Car.Nme_Carta_Email, Car.Vlr_Carta_Email, Car.Dta_Cadastro, Car.Des_Assunto, Car.Idf_Template FROM BNE_Carta_Email Car";
        ...
        /// <returns>Verdadeiro ou falso informando se a operação foi executada com sucesso.</returns>
        /// <remarks>Gieyson Stelmak</remarks>
        private static bool SetInstance(IDataReader dr, CartaEmail objCartaEmail, bool dispose = true)
        {
            try
            {
                if (dr.Read())
                {
                    ...
                    if (dr["Idf_Template"] != DBNull.Value)
                        objCartaEmail._template = new CartaEmail(Convert.ToInt32(dr["Idf_Template"]));
