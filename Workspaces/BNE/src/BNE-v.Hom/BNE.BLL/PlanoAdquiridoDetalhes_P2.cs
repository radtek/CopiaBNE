    ...
    public partial class PlanoAdquiridoDetalhes: ICloneable // Tabela: BNE_Plano_Adquirido_Detalhes
    {
        ...
        private const string sp_CarregarPlanoRecorrenteBoleto = @"
            ...
            WHERE   pa.Idf_Filial IS NOT NULL
                    ...
                    AND Flg_Boleto_Recorrente = 1
            ";
        #endregion

        private const string SP_EMAIL_NOTA = @"

                SELECT  CASE WHEN P.Idf_Plano_Tipo = 1 THEN PF.Eml_Pessoa
                             WHEN PP.Eml_Envio_Boleto IS NOT NULL
                                  AND PP.Eml_Envio_Boleto <> '' THEN PP.Eml_Envio_Boleto
                             ELSE PAD.Eml_Envio_Boleto
                        END AS 'Eml_Pessoa'
        
                FROM    BNE.BNE_Plano P
                        LEFT JOIN BNE.BNE_Plano_Adquirido PA WITH ( NOLOCK ) ON PA.Idf_Plano = P.Idf_Plano
                        LEFT JOIN BNE.BNE_Plano_Parcela PP WITH ( NOLOCK ) ON PP.Idf_Plano_Adquirido = PA.Idf_Plano_Adquirido
                        ...
                        INNER JOIN BNE.TAB_Usuario_Filial_Perfil UFP WITH ( NOLOCK ) ON UFP.Idf_Usuario_Filial_Perfil = PA.Idf_Usuario_Filial_Perfil
...
";
        ...
        public static string RetornaEmailNotas(int idf_Pagamento)
        {
            var parms = new List<SqlParameter>
                {
                    new SqlParameter { ParameterName = "@idf_Pagamento", SqlDbType = SqlDbType.Int, Value = idf_Pagamento }
                };

            return DataAccessLayer.ExecuteScalar(CommandType.Text, SP_EMAIL_NOTA, parms).ToString();
        }
