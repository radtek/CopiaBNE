...
namespace BNE.BLL
{
    public partial class Vaga // Tabela: BNE_Vaga
    {
        ...
        FROM    BNE.BNE_Vaga V WITH(NOLOCK)
				...
				outer apply 
				(select top 1 vc.des_curso as descricao, tc.des_curso from bne_vaga_curso vc with(nolock)
				left join tab_curso tc with(nolock) on vc.idf_curso = tc.idf_curso
				        where vc.idf_Vaga = v.idf_vaga) as curso
 ...
 DECLARE @data DATE 
        ...
        #region spVagasInativasFilial
        private const string spVagasInativasFilial = @"
        ...
        private const string spVagasFilial = @"
                ...
                SET @iSelect = 'SELECT
						                ...
						                v.* FROM BNE.BNE_Vaga v WITH(NOLOCK)
				                WHERE 1 = 1'
				
                IF ( @Idf_Filial IS NOT NULL ) 
                BEGIN
	                SET @iSelect = @iSelect + ' AND Idf_Filial = ' + CONVERT(VARCHAR, @Idf_Filial) 
                END

                IF ( @Idf_Usuario_Filial_Perfil IS NOT NULL ) 
                BEGIN
	                SET @iSelect = @iSelect + ' AND Idf_Usuario_Filial_Perfil = ' + CONVERT(VARCHAR, @Idf_Usuario_Filial_Perfil) 
                END

                IF ( @Flg_Inativo IS NOT NULL ) 
                BEGIN
	                SET @iSelect = @iSelect + ' AND Flg_Inativo = ' + CONVERT(VARCHAR, @Flg_Inativo) 
                END

                IF ( @Flg_Vaga_Arquivada IS NOT NULL ) 
                BEGIN
	                SET @iSelect = @iSelect + ' AND Flg_Vaga_Arquivada = ' + CONVERT(VARCHAR, @Flg_Vaga_Arquivada) 
                END

                IF ( @Flg_Auditada IS NOT NULL ) 
                BEGIN
	                SET @iSelect = @iSelect + ' AND Flg_Auditada = ' + CONVERT(VARCHAR, @Flg_Auditada) 
                END

                IF ( @Idf_Tipo_Vinculo IS NOT NULL ) 
                BEGIN
	                SET @iSelect = @iSelect + ' AND EXISTS(SELECT TOP 1 * FROM BNE.BNE_Vaga_Tipo_Vinculo vtv WHERE v.Idf_Vaga = vtv.Idf_Vaga AND vtv.Idf_Tipo_Vinculo = ' + CONVERT(VARCHAR, @Idf_Tipo_Vinculo) + ')'
                END
        ...
        private const string Spvagascompoucacandidaturas = @"
        SELECT  V.Idf_Vaga ,
                PF.Num_CPF ,
                PF.Dta_Nascimento ,
                PF.Nme_Pessoa,
                F.Raz_Social,
                Inscritos.Qtd AS Qtd_Inscritos ,
                ISNULL(Fun.Des_Funcao, V.Des_Funcao) AS Des_Funcao ,
                V.Eml_Vaga,
                V.Cod_Vaga
        FROM    BNE.BNE_Vaga V WITH ( NOLOCK )
                INNER JOIN BNE.TAB_Usuario_Filial_Perfil UFP WITH ( NOLOCK ) ON V.Idf_Usuario_Filial_Perfil = UFP.Idf_Usuario_Filial_Perfil
                INNER JOIN BNE.TAB_Pessoa_Fisica PF WITH ( NOLOCK ) ON UFP.Idf_Pessoa_Fisica = PF.Idf_Pessoa_Fisica
                INNER JOIN BNE.TAB_Filial F WITH ( NOLOCK ) ON F.Idf_Filial = UFP.Idf_Filial
                LEFT JOIN plataforma.TAB_Funcao Fun WITH ( NOLOCK ) ON V.Idf_Funcao = Fun.Idf_Funcao
                CROSS APPLY ( SELECT    COUNT(Idf_Vaga_Candidato) AS Qtd
                                FROM      BNE.BNE_Vaga_Candidato VC
                                WHERE     VC.Idf_Vaga = V.Idf_Vaga
                            ) AS Inscritos
                LEFT JOIN BNE.BNE_Vaga_Integracao VI WITH(NOLOCK) ON V.Idf_Vaga = VI.Idf_Vaga
        WHERE   V.Dta_Abertura BETWEEN DATEADD(day, -1 * @Dias, CONVERT(DATE, GETDATE()))
                            AND     DATEADD(day, -1 * @Dias + 1, CONVERT(DATE, GETDATE()))
                AND v.Flg_Inativo = 0
                AND v.Flg_Auditada = 1
                AND v.Flg_Vaga_Rapida = 0  
                AND Inscritos.Qtd <= @QuantidadeMinimaCurriculos
                AND VI.Idf_Vaga_Integracao IS NULL 
        ";

        private const string Spvalidarvagapertenceaousuario = @"SELECT COUNT(Idf_Vaga) FROM BNE.BNE_Vaga WHERE Idf_Vaga = @Idf_Vaga AND Idf_Usuario_Filial_Perfil = @Idf_Usuario_Filial_Perfil";
        ...
        /// </param>
        public void SalvarVaga(List<VagaDisponibilidade> listVagaDisponibilidade, List<VagaTipoVinculo> listVagaTipoVinculo, List<VagaPergunta> listVagaPergunta, List<VagaCurso> listVagaCurso, bool auditada, int? idfUsuarioFilialPerfilLogado, Enumeradores.VagaLog? processo, SqlTransaction trans)
        {
            SalvarVaga(listVagaDisponibilidade, listVagaTipoVinculo, listVagaPergunta, listVagaCurso, auditada, trans, idfUsuarioFilialPerfilLogado, processo);
        }

        public void SalvarVaga(List<VagaDisponibilidade> listVagaDisponibilidade, List<VagaTipoVinculo> listVagaTipoVinculo, List<VagaPergunta> listVagaPergunta, List<VagaCurso> listVagaCurso, bool auditada, int? idfUsuarioFilialPerfilLogado, Enumeradores.VagaLog? processo)
        {
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                conn.Open();
        ...
        public void SalvarVagaRapida(List<VagaTipoVinculo> vinculos, string nome, string ddd, string fone, VagaLog processo)
        {
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                conn.Open();
                ...
                using (var trans = conn.BeginTransaction())
                {
                    try
                    {
                        SalvarVaga(null, vinculos, null, null, false, trans, null, processo);

                        var vagaRapidaDados = new VagaRapidaContato(this, nome, ddd, fone);

                        vagaRapidaDados.Save(trans);
        ...
        private void SalvarVaga(
            List<VagaDisponibilidade> listVagaDisponibilidade,
            List<VagaTipoVinculo> listVagaTipoVinculo,
            List<VagaPergunta> listVagaPergunta,
            List<VagaCurso> listVagaCurso,
            bool auditada,
            SqlTransaction trans,
            int? idfUsuarioFilialPerfilLogado,
            Enumeradores.VagaLog? Processo)
        {
            ...
            VagaCurso.DeletePorVaga(_idVaga, trans);
            if (listVagaCurso != null)
            {
                foreach (var objVagaCurso in listVagaCurso)
                {
                    objVagaCurso.Save(trans);
                }
            }
            ...
            EnvioEmailWebEstagiosIntegracao(listVagaTipoVinculo);

            if (auditada)
                DispararPluginEnvioCandidatoVagaPerfil();

            if (!this.Filial.EmpresaEmAuditoria() && !auditada) //Se a vaga não está publicada, manda para publicação automática
            {
                this.Publicar(novoCadastro);
            }
        }

        #region BuscarVagasSolr

        private static string AggregateForFieldQuery(string[] values)
        {
            return values.Count() == 1 ?
                    $"\"{values[0]}\"" :
                    values.Aggregate((i, j) => $"\"{i}\" OR \"{j}\"");
        }
        private static string AggregateForFieldCursoQuery(string[] values)
        {
            return values.Count() == 1 ?
                    $"*{values[0]}*" :
                    values.Aggregate((i, j) => $"*{i}* OR *{j}*");
        ...
        private static string MontaUrlSolr(int pagina,
            ...
            string[] facetFields)
        {
            StringBuilder urlBuscaVagas = new StringBuilder();
            urlBuscaVagas.Append(Parametro.RecuperaValorParametro(Enumeradores.Parametro.UrlVagasSolr));

            urlBuscaVagas.Append("?wt=json");

            var start = pagina * registrosPorPagina;
            urlBuscaVagas.Append($"&start={start}");
            urlBuscaVagas.Append($"&rows={registrosPorPagina}");

            if (Oportunidade.HasValue)
                urlBuscaVagas.Append($"&fq=Flg_Vaga_Arquivada:{Oportunidade.Value}");

            if (!string.IsNullOrWhiteSpace(query))
                urlBuscaVagas.Append($"&q={HttpUtility.UrlEncode(query)}");

            if (salarioMinimo.HasValue && salarioMinimo.Value > 0)
                urlBuscaVagas.Append($"&fq=Vlr_Salario_De:[{salarioMinimo.Value.ToString("0.00", System.Globalization.CultureInfo.InvariantCulture)} TO *]");

            if (salarioMaximo.HasValue && salarioMaximo.Value > 0)
                urlBuscaVagas.Append($"&fq=Vlr_Salario_Para:[* TO {salarioMaximo.Value.ToString("0.00", System.Globalization.CultureInfo.InvariantCulture)}]");

            if (idTipoVinculo != null && idTipoVinculo.Count() > 0)
                urlBuscaVagas.Append($"&fq=Idf_Tipo_Vinculo:({string.Join(" OR ", idTipoVinculo)})");

            if (funcao != null && funcao.Count() > 0)
                urlBuscaVagas.Append($"&fq=Des_Funcao:({AggregateForFieldQuery(funcao)})");

            if (area != null && area.Count() > 0)
                urlBuscaVagas.Append($"&fq=Des_Area_BNE:({AggregateForFieldQuery(area)})");

            if (escolaridade != null && escolaridade.Count() > 0)
                urlBuscaVagas.Append($"&fq=Idf_Escolaridade:({AggregateForFieldQuery(escolaridade)})");

            if (deficiencia != null && deficiencia.Count() > 0)
            {
                if (!deficiencia.Select(x => x.Equals((int)Enumeradores.Deficiencia.Qualquer)).First())
                    urlBuscaVagas.Append($"&fq=Idf_Deficiencia:({string.Join(" OR ", deficiencia)})");
                else//Selecionou qualquer vai pesquisar todas as deficiencias.
                    urlBuscaVagas.Append($"&fq=Idf_Deficiencia:( 1 OR 2 OR 3 OR 4 OR 5 OR 6 OR 7)");
            }

            if (siglaEstado != null && siglaEstado.Count() > 0)
                urlBuscaVagas.Append($"&fq=Sig_Estado:({AggregateForFieldQuery(siglaEstado)})");

            if (cidade != null && cidade.Count() > 0)
                urlBuscaVagas.Append($"&fq=Nme_Cidade:({AggregateForFieldQuery(cidade)})");

            if (empresa != null && empresa.Count() > 0)
                urlBuscaVagas.Append($"&fq=Nme_Empresa_Busca:({AggregateForFieldQuery(empresa)})");

            if (confidencial.HasValue)
                urlBuscaVagas.Append($"&fq=Flg_Confidencial:{confidencial.Value}");

            if (campanha.HasValue)
                urlBuscaVagas.Append($"&fq=Campanha:{campanha.Value}");
            ...
            if (dataInicio.HasValue || dataFim.HasValue)
            {
                ...
                if (dataInicio.HasValue)
                    filtroInicio = $"{dataInicio.Value.ToString("yyyy-MM-dd")}T00:00:00Z";

                if (dataFim.HasValue)
                {
                    if (dataFim.Value.Equals(DateTime.Now.Date))//data fim do dia tem que pegar o horario da consulta
                        filtroFim = $"{dataFim.Value.ToString("yyyy-MM-dd")}T{DateTime.Now.ToString("HH:mm:ss")}Z";
                    else
                        filtroFim = $"{dataFim.Value.ToString("yyyy-MM-dd")}T23:59:59Z";
                }

                urlBuscaVagas.Append($"&fq=Dta_Abertura:[{filtroInicio} TO {filtroFim}]");
            }

            if (idOrigem != null && idOrigem.Count() > 0)
                urlBuscaVagas.Append($"&fq=Idf_Origem:({string.Join("+OR+", idOrigem)})");

            if (curso != null && curso.Count() > 0)
                urlBuscaVagas.Append($"&fq=Des_Curso_Busca:({AggregateForFieldCursoQuery(curso)})");

            if (idcurso != null && idcurso.Count() > 0)
                urlBuscaVagas.Append($"&fq=Idf_Curso:({AggregateForFieldQuery(idcurso.Select(x => x.ToString()).ToArray())})");

            if (disponibilidade != null && disponibilidade.Count() > 0)
                urlBuscaVagas.Append($"&fq=Idf_Disponibilidade:({string.Join(" AND ", disponibilidade)})");//Falta indexar no Solr

            if (ofereceCurso.HasValue)
                urlBuscaVagas.Append($"&fq=Flg_Oferece_Cursos:{ofereceCurso.Value}");

            if (cidadeRegiao.HasValue)
                urlBuscaVagas.Append($"&fq=Idf_Cidade_Regiao:{cidadeRegiao.Value}");

            if (funcaoAgrupadora.HasValue)
                urlBuscaVagas.Append($"&fq=Idfs_Funcoes_Sinonimo:{funcaoAgrupadora.Value}");

            if (curriculo.HasValue)
                urlBuscaVagas.Append($"&fq=-({{!join from=Idf_Filial to=Idf_Filial fromIndex=bloqueio_empresa}}Idf_Curriculo:{curriculo.Value})");

            if (idfFilial.HasValue)//não mostrar as vagas confidenciais da empresa
                urlBuscaVagas.Append($"&fq=Idf_Filial:{idfFilial.Value}");

            if (usuarioFilial != null && usuarioFilial.Count() > 0)
                urlBuscaVagas.Append($"&fq=Idf_Usuario_Filial_Perfil:({string.Join(" OR ", usuarioFilial)})");//Falta indexar no Solr


            if (!string.IsNullOrEmpty(Ordenacao))
                urlBuscaVagas.Append($"&sort={Ordenacao}");

            if (facetFields != null && facetFields.Count() > 0)
            {
                urlBuscaVagas.Append("&facet=true");
                foreach (var field in facetFields)
                    urlBuscaVagas.Append($"&facet.field={field}");
            }

            return urlBuscaVagas.ToString();
        }

        public static SolrResponse<VagasSolr.DocVaga> BuscarVagasSolr(int pagina,
            int registrosPorPagina,
            string query,
            decimal? salarioMinimo,
            decimal? salarioMaximo,
            ...
            string[] facetFields)
        {
            var urlBuscaVagas = MontaUrlSolr(pagina,
                registrosPorPagina,
                query,
                salarioMinimo,
                salarioMaximo,
                ...
                UsuarioFilial,
                facetFields);
            var resultado = VagasSolr.EfetuarRequisicao(urlBuscaVagas);
        ...
        private void EnvioEmailWebEstagiosIntegracao(List<VagaTipoVinculo> tiposVinculo)
        {
            if (tiposVinculo == null || tiposVinculo.Count == 0)
                return;

            var safeTipoVinculo = tiposVinculo.Where(obj => obj.TipoVinculo != null).ToList();
            ...
            var emailDestino = Parametro.RecuperaValorParametro(Enumeradores.Parametro.EmailWebEstagiosIntegracaoAnuncioVaga);
            ...
            foreach (var item in emailDestino.Split(new[] { ";" }, StringSplitOptions.RemoveEmptyEntries))
            {
        ...
        #region ObterVagasFilial
        /// <summary>
        ///     Método utilizado por retornar as colunas de um registro no banco de dados.
        /// </summary>
        /// <param name="pagina">Página a ser retornada comecando com 1</param>
        /// <param name="registrosPorPagina">Número de registros por página</param>
        /// <returns>List<Vaga></returns>
        /// <remarks>Francisco Ribas</remarks>
        public static List<Vaga> ObterVagasFilial(int pagina,
            int registrosPorPagina,
            int? idFilial,
            int? idUsuarioFilialPerfil,
            int? idTipoVinculo,
            bool? flgInativo,
            bool? flgArquivada,
            bool? flgAuditada,
            out int totalRegistros)
        {
            var parms = new List<SqlParameter>
            {
                ...
                new SqlParameter("@Flg_Inativo", SqlDbType.Bit),
                new SqlParameter("@Flg_Vaga_Arquivada", SqlDbType.Bit),
                new SqlParameter("@Flg_Auditada", SqlDbType.Bit),
                new SqlParameter("@Idf_Tipo_Vinculo", SqlDbType.Int, 4),
                new SqlParameter("@CurrentPage", SqlDbType.Int, 4),
                new SqlParameter("@PageSize", SqlDbType.Int, 4),
            };

            if (idFilial.HasValue) parms[0].Value = idFilial.Value;
            else parms[0].Value = DBNull.Value;

            if (idUsuarioFilialPerfil.HasValue) parms[1].Value = idUsuarioFilialPerfil.Value;
            else parms[1].Value = DBNull.Value;

            if (flgInativo.HasValue) parms[2].Value = flgInativo.Value;
            else parms[2].Value = DBNull.Value;

            if (flgArquivada.HasValue) parms[3].Value = flgArquivada.Value;
            else parms[3].Value = DBNull.Value;

            if (flgAuditada.HasValue) parms[4].Value = flgAuditada.Value;
            else parms[4].Value = DBNull.Value;

            if (idTipoVinculo.HasValue) parms[5].Value = idTipoVinculo.Value;
            else parms[5].Value = DBNull.Value;

            parms[6].Value = pagina;
            parms[7].Value = registrosPorPagina;

            var result = new List<Vaga>();

            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, spVagasFilial, parms))
            {
                ...
                while (dr.Read() && SetInstanceNonDispose(dr, objVaga))
                {
                    result.Add(objVaga);
                    objVaga = new Vaga();
                }
        ...
        /// <remarks>Gieyson Stelmak</remarks>
        public static DataTable ListarVagasFilial(int idFilial, int paginaCorrente, int tamanhoPagina, bool? anunciadas,
            int? idUsuarioFilialPerfil, int? idUsuarioFilialPerfilConfidencial, List<int> listIdFuncoes, List<string> ListNomeFuncoes,
            out int totalRegistros, bool? flgVagasCampanha, List<string> ListIdUsuarioFilialPerfilFiltro)
        {

            DataTable dt = null;
            //  try
            //  {
            #region [Consulta Solr]
            ///Removido a consulta das minahs vagas no solr, por não indexar vaga que ainda não foi auditada.
            //dt = PesquisaVaga.ListaMinhasVagas(paginaCorrente, tamanhoPagina, idFilial, flgVagasCampanha, anunciadas, ListIdUsuarioFilialPerfilFiltro, ListNomeFuncoes, out totalRegistros);
            ...
            List<int> listaVaga = new List<int>();
            totalRegistros = 0;
            using (SqlConnection conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        using (IDataReader dr = DataAccessLayer.ExecuteReader(trans, CommandType.StoredProcedure, "[BNE].[VAGAS_SALASELECIONADOR]", parms))
                        {
                            if (dr.Read())
                                totalRegistros = Convert.ToInt32(dr[0]);
                            ...
                            dt.Columns.Add("Num_Cvs_Recebidos");
                            dt.Columns.Add("Num_Cvs_Recebidos_Nao_Lidos");
                            foreach (DataRow linha in dt.Rows)
                                listaVaga.Add((int)linha["Idf_Vaga"]);

                            if (listaVaga.Count > 0)
                            {
                                var ResultCandidatos = BNE.Solr.VagaCandidato.EfetuarRequisicao(BLL.Custom.Solr.BuscaVagaCandidato.MontaUrlSolrMinhasVagas(listaVaga));
                                foreach (DataRow row in dt.Rows)
                                {
                                    row["Num_Cvs_Recebidos_Nao_Lidos"] = ResultCandidatos.response.docs.Where(x => x.Idf_Vaga == (int)row["Idf_Vaga"] && x.Dta_Visualizacao <= DateTime.Now.AddYears(-100)).Count();
                                    row["Num_Cvs_Recebidos"] = ResultCandidatos.response.docs.Where(x => x.Idf_Vaga == (int)row["Idf_Vaga"]).Count();
                                    List<VagaPergunta> listaP = VagaPergunta.RecuperarListaPerguntas((int)row["Idf_Vaga"], trans);
        ...
        public static void InativarVaga(int idVaga, int? idfUsuarioFilialPerfil, Enumeradores.VagaLog? Processo)
        {
            ...
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                ...
                using (var trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        trans.Commit();
                        //Remover as candidaturas do solr de referente a vaga bloqueada.
                        //BLL.Custom.Rabbit.IndexarCandidatura.Indexar(null, idVaga);
                    }
                    catch (Exception ex)
                    {
                        EL.GerenciadorException.GravarExcecao(ex, "Falha ao inativar vaga no banco: " + idVaga);
                        trans.Rollback();
                    }
                }
                conn.Close();
            ...
            {
                EL.GerenciadorException.GravarExcecao(ex, "Falha ao inativar vaga no Solr: " + idVaga);
            }
        ...
        /// <remarks>Gieyson Stelmak</remarks>
        public static bool SetInstanceNonDispose(IDataReader dr, Vaga objVaga)
        {
        ...
        #region IndexarVagaSolr
        /// <summary>
        ///     Indexa uma Vaga no solr
        /// </summary>
        /// <param name="Idf_Vaga"></param>
        public static void IndexarVagaSolr(int IdVaga)
        {
            var urlSLOR = Parametro.RecuperaValorParametro(Enumeradores.Parametro.UrlSolrAtualizaVaga);
        ...
        public void CalcularAberturaEncerramento()
        {
            ...
            //Task 47138 - Retirada do anuncio imediato de vagas.
        ...
        #region Publicar
        /// <summary>
        /// Vaga nova = true - dispara e-mail de cadastro de vaga no final da auditoria.
        /// </summary>
        /// <param name="vagaNova"></param>
        public void Publicar(bool vagaNova = false)
        {
            var parametros = new ParametroExecucaoCollection
            {
                {"idVaga", "Vaga", IdVaga.ToString(CultureInfo.InvariantCulture), CodigoVaga},
                {"novoCadastro", "novoCadastro", vagaNova.ToString(), vagaNova.ToString() }
            };
        ...
        public static string MontarLinkVagaSMS(int idVaga, Curriculo objCurriculo, string Utms)
        {
            try
            {
                ...
                try
                {
                    linkVaga = Helper.EncurtarUrl(linkVaga);
                }
                catch (Exception)
                {
                    linkVaga = string.Format("http://bne.vc/{0}", idVaga);
                }
        ...
        public static void RemoverVagasFilial(int idFilial, bool status)
        {
            ...
            using (var dr = DataAccessLayer.ExecuteReader(CommandType.Text, spVagasInativasFilial, parms))
            {
                while (dr.Read())
                {
            ...
            else//Remover
            {
                ...
                //Remover as vagas da filial do solr.
                Vaga.ExcluirVagasFilialSolr(idFilial);
            }
        }
        #endregion

        #region [ExcluirVagasFilialSolr]
        public static void ExcluirVagasFilialSolr(int idfFilial)
        {
            var urlSLOR = Parametro.RecuperaValorParametro(Enumeradores.Parametro.UrlRemoverVagasFilial);
            urlSLOR = urlSLOR.Replace("{1}", idfFilial.ToString());
        ...
        #region [EnviarEmailCadastroVaga]
        public void EnviarEmailCadastroVaga(List<DTO.Curriculo> curriculos)
        {
            using (var conexao = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                conexao.Open();
                using (var trans = conexao.BeginTransaction())
                {
                    try
                    {
                        string curriculosRecomendadosOuLinkParaVaga;
                        if (curriculos.Count > 0)
                        {
                            var sbcurriculos = new StringBuilder();
                            foreach (var curriculo in curriculos)
                            {
                                sbcurriculos.Append(CartaEmail.GetById((int)Enumeradores.CartaEmail.CardCurriculo).Montar(new
                                {
                                    nome = curriculo.NomeCompleto,
                                    funcao = curriculo.NomeFuncaoPretendida,
                                    idade = curriculo.Idade,
                                    escolaridade = curriculo.UltimaFormacaoCompleta,
                                    bairro = curriculo.Bairro,
                                    cidade = curriculo.NomeCidade,
                                    estado = curriculo.SiglaEstado
                                }).Conteudo);
                            }

                            string linkVerCurriculos;

                            if (FlagVagaRapida)
                            {
                                linkVerCurriculos = $"http://{Helper.RecuperarURLAmbiente()}/campanha/vagarapida?e={Helper.ToBase64(EmailVaga)}";
                            }
                            else
                            {
                                if (UsuarioFilialPerfil.PessoaFisica == null)
                                {
                                    //TODO Gieyson autocandidatura metodo para trazer apenas cpf e data de nascimento de quem cadastrou a vaga
                                    UsuarioFilialPerfil.CompleteObject(trans);
                                    UsuarioFilialPerfil.PessoaFisica.CompleteObject(trans);
                                }
                                linkVerCurriculos = $"http://{Helper.RecuperarURLAmbiente()}/logar/{LoginAutomatico.GerarHashAcessoLogin(UsuarioFilialPerfil.PessoaFisica.CPF, UsuarioFilialPerfil.PessoaFisica.DataNascimento, "/sala-selecionador-vagas-anunciadas")}";
                            }

                            curriculosRecomendadosOuLinkParaVaga = CartaEmail.GetById((int)Enumeradores.CartaEmail.ConfirmacaoCadastroVagaRecomencadoes).Montar(new
                            {
                                curriculos = sbcurriculos.ToString(),
                                vercurriculos = linkVerCurriculos
                            }).Conteudo;
                        ...
                        else
                        {
                            ...
                            curriculosRecomendadosOuLinkParaVaga = CartaEmail.GetById((int)Enumeradores.CartaEmail.ConfirmacaoCadastroVagaLinkParaVaga).Montar(new
                            {
                                linkvaga = $"http://{Helper.RecuperarURLAmbiente()}/logar/{LoginAutomatico.GerarHashAcessoLogin(UsuarioFilialPerfil.PessoaFisica.CPF, UsuarioFilialPerfil.PessoaFisica.DataNascimento, $"/vagas-de-emprego/vaga/visualizarvagaempresa?identificador={IdVaga}")}",
                            }).Conteudo;
                        }

                        if (Cidade.Estado == null)
                        {
                            Cidade.CompleteObject(trans);
                        }

                        if (string.IsNullOrWhiteSpace(Funcao.DescricaoFuncao))
                        {
                            Funcao.CompleteObject(trans);
                        }

                        var cartaEmail = CartaEmail.GetById((int)Enumeradores.CartaEmail.ConfirmacaoCadastroVaga).Montar(new
                        {
                            funcao = VagaCurso.ListaCursoParaCartaEmail(Funcao.DescricaoFuncao, this.IdVaga, trans),
                            cidade = Cidade.NomeCidade,
                            estado = Cidade.Estado.SiglaEstado,
                            infoadicional = curriculosRecomendadosOuLinkParaVaga
                        });

                        EmailSenderFactory
                            .Create(TipoEnviadorEmail.Fila)
                            .Enviar(cartaEmail.Assunto, cartaEmail.Conteudo, Enumeradores.CartaEmail.CadastroVaga, Parametro.RecuperaValorParametro(Enumeradores.Parametro.EmailMensagens), EmailVaga, trans);

                        trans.Commit();
                    }
                    catch (Exception ex)
                    {
                        EL.GerenciadorException.GravarExcecao(ex, "Erro ao enviar carta de anuncio de vaga ");
                        trans.Rollback();
                    }
        ...
        public SolrResponse<CurriculoCompleto.Response> RealizarAutoCandidatura()
        {
            int quantidadeCandidatos = 5;
            var curriculos = CurriculoCompleto.EfetuarRequisicao(BuscaCurriculo.MontarQueryAutoCandidatura(0, quantidadeCandidatos, this));

            if (curriculos.response.numFound > 0)
            {
                using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
                {
                    conn.Open();
                    using (var trans = conn.BeginTransaction())
                    {
                        try
                        {
                            foreach (var curriculo in curriculos.response.docs)
                            {
                                VagaCandidato.CandidatarAutomaticamente(new Curriculo(curriculo.Idf_Curriculo), this, trans);
                            }
                            ...
                            foreach (var curriculo in curriculos.response.docs)
                            {
                                DomainEventsHandler.Handle(new OnNovaCandidatura(curriculo.Idf_Curriculo, this.IdVaga));
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        /// <summary>
        /// Deve realizar a candidatura automatica para a vaga e enviar email de confirmacao de cadastro
        /// </summary>
        public void FinalizarPublicacaoNovaVaga()
        {
            var curriculos = RealizarAutoCandidatura();
            if (curriculos != null)
            {
                EnviarEmailCadastroVaga(curriculos.response.docs.Select(c => Curriculo.MapearCurriculo(c, true)).ToList());
            }
        }
        ...
        public int MigrationId
        {
            ...
            get { return this._idVaga; }
        }


        TipoVinculo _tipoVinculo;
        public TipoVinculo MigrationTipoVinculo
        {
            set
            {
                this._tipoVinculo = value;
            }
            get { return this._tipoVinculo; }
        }
        ...
        public static IEnumerable<VagaComPoucaCandidatura> RecuperarVagasParaComPoucasCandidaturas(int dias, int quantidadeMinimaCurriculos)
        {
            var parms = new List<SqlParameter>
            {
                new SqlParameter {ParameterName = "@Dias", SqlDbType = SqlDbType.Int, Size=4, Value = dias},
                new SqlParameter {ParameterName = "@QuantidadeMinimaCurriculos", SqlDbType = SqlDbType.Int, Size=4, Value = quantidadeMinimaCurriculos}
            };

            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, Spvagascompoucacandidaturas, parms))
            {
                while (dr.Read())
                {
                    var vaga = LoadObject(Convert.ToInt32(dr["Idf_Vaga"]));
                    var CPF = Convert.ToDecimal(dr["Num_CPF"]);
                    var dataNascimento = Convert.ToDateTime(dr["Dta_Nascimento"]);

                    yield return new VagaComPoucaCandidatura
                    {
                        IdVaga = vaga.IdVaga,
                        LinkVerCurriculos = vaga.MontarUrlVisualizarBancoCurriculosParaVaga(CPF, dataNascimento),
                        QuantidadeInscritos = Convert.ToInt32(dr["Qtd_Inscritos"]),
                        QuantidadeBancoCurriculo = vaga.RetornarQuantidadeBancoCurriculos(),
                        Funcao = dr["Des_Funcao"].ToString(),
                        Email = dr["Eml_Vaga"].ToString(),
        ...
        public int RetornarQuantidadeBancoCurriculos()
        {
            var resultado = CurriculoIdentificador.EfetuarRequisicao(BLL.Custom.BuscaCurriculo.MontarQuery(this, false));
            try
            {
        ...
        public string MontarUrlVisualizarBancoCurriculosParaVaga(decimal cpf, DateTime dataNascimento)
        {
            return $"http://{Helper.RecuperarURLAmbiente()}/logar/{LoginAutomatico.GerarHashAcessoLogin(cpf, dataNascimento, $"~/{Rota.RecuperarURLRota(RouteCollection.PesquisaCurriculosBancoCurriculos).Replace("{vagabancocurriculo}", Helper.ToBase64(IdVaga.ToString()))}")}";
        }

        public bool PertenceAoUsuario(UsuarioFilialPerfil usuarioFilialPerfil)
        {
            var parms = new List<SqlParameter>
            {
                new SqlParameter {ParameterName = "@Idf_Vaga", SqlDbType = SqlDbType.Int, Size = 4, Value = _idVaga},
                new SqlParameter {ParameterName = "@Idf_Usuario_Filial_Perfil", SqlDbType = SqlDbType.Int, Size = 4, Value = usuarioFilialPerfil.IdUsuarioFilialPerfil}
            };

            return (int)DataAccessLayer.ExecuteScalar(CommandType.Text, Spvalidarvagapertenceaousuario, parms) > 0;
        }
    }
}