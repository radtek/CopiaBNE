...
namespace BNE.BLL
{
	public partial class VagaCandidato // Tabela: BNE_Vaga_Candidato
	{
		...
		private StatusCandidatura _statusCandidatura;
		private OrigemCandidatura _origemCandidatura;
        private DateTime? _dataVisualizacao;
        private UsuarioFilialPerfil _usuarioFilialPerfilVisualizacao;
		...
		public StatusCandidatura StatusCandidatura
		{
			get
			{
				return this._statusCandidatura;
			}
			set
			{
				this._statusCandidatura = value;
				this._modified = true;
			}
		}
		#endregion 

		#region OrigemCandidatura
		/// <summary>
		/// Campo opcional.
		/// </summary>
		public OrigemCandidatura OrigemCandidatura
		{
			get
			{
				return this._origemCandidatura;
			}
			set
			{
				this._origemCandidatura = value;
				this._modified = true;
			}
		}
		#endregion 
        ...
        #region UsuarioFilialPerfilVisualizacao
        /// <summary>
        /// Campo opcional.
        /// </summary>
        public UsuarioFilialPerfil UsuarioFilialPerfilVisualizacao
        {
            get
            {
                return this._usuarioFilialPerfilVisualizacao;
            }
            set
            {
                this._usuarioFilialPerfilVisualizacao = value;
                this._modified = true;
            }
        }
        #endregion
		...
		#region Consultas
        private const string SPINSERT = "INSERT INTO BNE_Vaga_Candidato (Idf_Curriculo, Idf_Vaga, Dta_Cadastro, Flg_Inativo, Idf_Status_Curriculo_Vaga, Flg_Auto_Candidatura, Idf_Status_Candidatura, Idf_Origem_Candidatura, Dta_Visualizacao,Idf_Usuario_Filial_Perfil_Visualizacao) VALUES (@Idf_Curriculo, @Idf_Vaga, GETDATE(), @Flg_Inativo, @Idf_Status_Curriculo_Vaga, @Flg_Auto_Candidatura, @Idf_Status_Candidatura, @Idf_Origem_Candidatura, @Dta_Visualizacao, @Idf_Usuario_Filial_Perfil_Visualizacao);SET @Idf_Vaga_Candidato = SCOPE_IDENTITY();";
        private const string SPUPDATE = "UPDATE BNE_Vaga_Candidato SET Idf_Curriculo = @Idf_Curriculo, Idf_Vaga = @Idf_Vaga, Flg_Inativo = @Flg_Inativo, Idf_Status_Curriculo_Vaga = @Idf_Status_Curriculo_Vaga, Flg_Auto_Candidatura = @Flg_Auto_Candidatura, Idf_Status_Candidatura = @Idf_Status_Candidatura, Idf_Origem_Candidatura = @Idf_Origem_Candidatura, Dta_Visualizacao = @Dta_Visualizacao, Idf_Usuario_Filial_Perfil_Visualizacao = @Idf_Usuario_Filial_Perfil_Visualizacao WHERE Idf_Vaga_Candidato = @Idf_Vaga_Candidato";
        private const string SPDELETE = "DELETE FROM BNE_Vaga_Candidato WHERE Idf_Vaga_Candidato = @Idf_Vaga_Candidato";
		private const string SPSELECTID = "SELECT * FROM BNE_Vaga_Candidato WITH(NOLOCK) WHERE Idf_Vaga_Candidato = @Idf_Vaga_Candidato";
		#endregion
		...
		private List<SqlParameter> GetParameters()
		{
            ...
            parms.Add(new SqlParameter("@Idf_Usuario_Filial_Perfil_Visualizacao", SqlDbType.Int, 4));
		...
		private void SetParameters(List<SqlParameter> parms)
		{
			...
			if (this._statusCurriculoVaga != null)
		...
		public static IDataReader LoadDataReader(string colunaOrdenacao, string direcaoOrdenacao, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
		{
            ...
            string SPSELECTPAG = "SELECT ROW_NUMBER() OVER (ORDER BY " + colunaOrdenacao + " " + direcaoOrdenacao + ") AS RowID, Vag.Idf_Vaga_Candidato, Vag.Idf_Curriculo, Vag.Idf_Vaga, Vag.Dta_Cadastro, Vag.Flg_Inativo, Vag.Idf_Status_Curriculo_Vaga, Vag.Flg_Auto_Candidatura, Vag.Idf_Status_Candidatura, Vag.Idf_Origem_Candidatura, Vag.Dta_Visualizacao  FROM BNE_Vaga_Candidato Vag";
		...
		/// <returns>Verdadeiro ou falso informando se a operação foi executada com sucesso.</returns>
		/// <remarks>Gieyson Stelmak</remarks>
		private static bool SetInstance(IDataReader dr, VagaCandidato objVagaCandidato, bool dispose = true)
		{
			try
			{
				if (dr.Read())
				{
					...
					if (dr["Idf_Status_Curriculo_Vaga"] != DBNull.Value)
					...
					if (dr["Idf_Status_Candidatura"] != DBNull.Value)
						objVagaCandidato._statusCandidatura = new StatusCandidatura(Convert.ToInt16(dr["Idf_Status_Candidatura"]));
					if (dr["Idf_Origem_Candidatura"] != DBNull.Value)
						objVagaCandidato._origemCandidatura = new OrigemCandidatura(Convert.ToInt32(dr["Idf_Origem_Candidatura"]));

                    if (dr["Dta_Visualizacao"] != DBNull.Value)
                        objVagaCandidato._dataVisualizacao = Convert.ToDateTime(dr["Dta_Visualizacao"]);
                    else
                        objVagaCandidato._dataVisualizacao = null;

                    if (dr["Idf_Usuario_Filial_Perfil_Visualizacao"] != DBNull.Value)
                        objVagaCandidato._usuarioFilialPerfilVisualizacao = new UsuarioFilialPerfil(Convert.ToInt32(dr["Idf_Usuario_Filial_Perfil_Visualizacao"]));
