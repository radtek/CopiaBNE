...
namespace BNE.BLL
{
    public partial class BoletoBancario
    {
        ...
        public static BoletoNet.Boleto ProcessarBoletoNovo(BLL.Pagamento objPagamento, SqlTransaction trans, bool isCobranca = false)
        {
            ...
            Boolean mesmoBoleto = false;
            if (objBoletoBancario != null)
                mesmoBoleto = objBoletoBancario.Pagamento.IdPagamento != objPagamento.IdPagamento;


           if (objBoletoBancario == null || mesmoBoleto)
            {
            ...
            //    conn.Open();
              ...
              //  {
                    using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, spRecuperarUltimioBoletoNaoPago, parms))
                    {
                        if (dr.Read())
                        {
                            objPagamento.Add(new Pagamento(Convert.ToInt32(dr["Idf_Pagamento"])));
                        }

                    }


                 string html = GerarLayoutBoletoHTML(CriarBoletos(objPagamento, false));
