...
namespace BNE.BLL
{
    public partial class Funcao // Tabela: TAB_Funcao
    {
        ...
        private static List<FuncaoCache> ListarFuncoesCACHE()
        {
            ...
            AND F.Idf_Area_BNE IS NOT NULL 
            ";
        ...
        #region FuncaoSinonimoSugest
        public class FuncaoSinonimoSugest{
            public int IdFuncao { get; set; }
            public string Funcao { get; set; }
            public int QuantidadePesquisa { get; set; }
...
AND (@Idf_Categoria IS NULL OR F.Idf_Funcao_Categoria = @IDF_CATEGORIA)";
        ...
        /// <returns>Verdadeiro ou falso informando se a operação foi executada com sucesso.</returns>
        /// <remarks>Vinicius Maciel</remarks>
        public static bool SetInstance_NotDispose(IDataReader dr, Funcao objFuncao)
        {
            try
            {
                objFuncao._idFuncao = Convert.ToInt32(dr["Idf_Funcao"]);
        ...
        #region [Sugeste Função Sinonimo]
        /// <summary>
        /// lista de sugestão da função passada mostrando qual as funções sinonimos mais pesquisadas. ajudando o usuario a colocar a função mais adquada
        /// </summary>
        /// <param name="IdFuncao"></param>
        /// <returns></returns>
        public static List<FuncaoSinonimoSugest> SugesteFuncaoSinonimo(int IdFuncao)
        {
            string cachekey = String.Format("SugesteFuncaoSinonimo:{0}", IdFuncao);
            if (MemoryCache.Default[cachekey] != null)
                return (List<FuncaoSinonimoSugest>)MemoryCache.Default[cachekey];

            var listSugest = new List<FuncaoSinonimoSugest>();

            List<FuncaoSinonimoSugest> list = new List<FuncaoSinonimoSugest>();
            var objDataTable = new DataTable();
            List<SqlParameter> parametros = new List<SqlParameter>
            {
                new SqlParameter { ParameterName = "@Idf_Funcao", SqlDbType = SqlDbType.Int, Value = IdFuncao }
            };
            int porcentagem = Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.PorcentagemSugestFuncaoSinonimo));
            using (var dr = DataAccessLayer.ExecuteReader(CommandType.StoredProcedure, "bne.sp_sugestao_funcao_sinonimo", parametros))
            {
                while(dr.Read())
                {
                    FuncaoSinonimoSugest obj = new FuncaoSinonimoSugest();
                    obj.Funcao = dr["Des_Funcao"].ToString();
                    obj.IdFuncao = Convert.ToInt32(dr["Idf_Funcao"]);
                    obj.QuantidadePesquisa = Convert.ToInt32(dr["Qtd_Pesquisas"]);
                    list.Add(obj);
                }
            }
            ...
            //calcula a porcentagem que a função foi pesquisada para apagerecer no sugest ex: 30% - se a função pesquisada tem 100 pesquisas
            //só vai entrar na pesquisa as que possuirem acima de 130.
            int qtdMinima = ((from item in list where item.IdFuncao.Equals(IdFuncao) select item.QuantidadePesquisa).FirstOrDefault() * (100 +porcentagem)) /100;
            int qtdPesquisada = (from item in list where item.IdFuncao.Equals(IdFuncao) select item.QuantidadePesquisa).FirstOrDefault();
                foreach (var item in list)
            {
                if (item.QuantidadePesquisa >= qtdMinima && !item.IdFuncao.Equals(IdFuncao))
                {
                    item.porcentagem = ((item.QuantidadePesquisa * 100) / qtdPesquisada)-100;
                    if (item.porcentagem > porcentagem)
                    {
                        listSugest.Add(item);
                        MemoryCache.Default.Add(cachekey, listSugest, policy);
                        return listSugest;
                    }
                }
            }
