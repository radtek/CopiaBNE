	...
	public partial class PesquisaVaga // Tabela: TAB_Pesquisa_Vaga
	{
		...
		private void SetParameters(List<SqlParameter> parms)
		{
			...
			if (!String.IsNullOrEmpty(this._descricaoCodVaga))
                parms[11].Value = Convert.ToDecimal(Regex.Replace(this._descricaoCodVaga, Parametro.RecuperaValorParametro(Enumeradores.Parametro.RegexCodVaga), "")).ToString();
			else
			...
			if (this._flagDeficiencia.HasValue)
				parms[18].Value = this._flagDeficiencia;
			else
		...
		/// <returns>Verdadeiro ou falso informando se a operação foi executada com sucesso.</returns>
		/// <remarks>Mailson</remarks>
		private static bool SetInstance(IDataReader dr, PesquisaVaga objPesquisaVaga, bool dispose = true)
		{
			try
			{
				if (dr.Read())
				{
                    ...
                    if (dr["Flg_Pesquisa_Avancada"] != DBNull.Value)
					...
					if (dr["Flg_Deficiencia"] != DBNull.Value)
						objPesquisaVaga._flagDeficiencia = Convert.ToBoolean(dr["Flg_Deficiencia"]);
