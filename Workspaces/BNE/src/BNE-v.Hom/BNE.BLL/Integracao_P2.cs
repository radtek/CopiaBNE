...
namespace BNE.BLL
{
    public partial class Integracao // Tabela: plataforma.BNE_Integracao
    {
        ...
        #region Sprecuperarintegracoesrecisao
        private const string Sprecuperarintegracoesrecisao = @"
        	 SELECT * FROM plataforma.BNE_Integracao WHERE Dta_Integracao IS NULL
                AND ( Idf_Integracao_Situacao = 1 OR Idf_Integracao_Situacao = 3 ) AND Flg_Admitido = 0
                AND dta_saida_prevista < getdate()
        ";
        #endregion

        #region Sprecuperarintegracoesadmissao
        private const string Sprecuperarintegracoesadmissao = @"
        	 SELECT * FROM plataforma.BNE_Integracao WHERE Dta_Integracao IS NULL AND ( Idf_Integracao_Situacao = 1 OR Idf_Integracao_Situacao = 3 ) AND Flg_Admitido = 1
        ";
        #endregion

        #region RecuperarIntegracoesRecuperarIntegracoesRecisao
        /// <summary>
        /// Recupera as integracoes não integradas de demissão
        /// </summary>
        /// <returns>Uma datatable com os emails não enviados</returns>
        public static List<Integracao> RecuperarIntegracoesRecisao()
        {
            var lista = new List<Integracao>();
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, Sprecuperarintegracoesrecisao, null))
            {
                while (dr.Read())
                {
        ...
        #region RecuperarIntegracoesAdmissao
        /// <summary>
        /// Recupera as integracoes não integradas de admissao
        /// </summary>
        /// <returns>Uma datatable com os emails não enviados</returns>
        public static List<Integracao> RecuperarIntegracoesAdmissao()
        {
            var lista = new List<Integracao>();
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, Sprecuperarintegracoesadmissao, null))
            {
                while (dr.Read())
                {
                    var objIntegracao = new Integracao();
