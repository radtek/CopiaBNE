...
{
    internal partial class InativarCurriculo : BaseService
    {
        #region Construtor
        public InativarCurriculo()
        {
            InitializeComponent();
        }
        #endregion
        ...
        private static readonly string HoraExecucao = Settings.Default.InativarCurriculoHoraExecucao;
        private static readonly int DelayExecucao = Settings.Default.InativarCurriculoDelayMinutos;
        #endregion
        ...
        {
            var objTs = new ThreadStart(IniciarInativarCurriculo);
        ...
        private void AjustarThread(DateTime horaAtual, bool primeiraExecucao)
        {
            if (primeiraExecucao)
            {
                ...
                EventLogWriter.LogEvent(
                    string.Format(
                        "Serviço Atualizar Plano - O Serviço está aguardando {0} para sua iniciar sua execução.",
            ...
            else
            {
                ...
                if ((int) delay.TotalMilliseconds - (int) tempoTotalExecucao.TotalMilliseconds > 0)
                {
                    EventLogWriter.LogEvent(
                        string.Format(
                            "Serviço Atualizar Plano - O Serviço está aguardando {0} para sua próxima execução, pois levou {1} para concluir a excução atual.",
                    ...
                    EventLogWriter.LogEvent(
                        string.Format(
                            "Serviço Atualizar Plano - O Serviço vai reiniciar agora {0} pois sua última execução foi às {1}, seu tempo de execução foi de {2} e seu delay é de {3}.",
        ...
        #region IniciarInativarCurriculo
        public void IniciarInativarCurriculo()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    Curriculo.InativarCurriculosDesatualizados();
