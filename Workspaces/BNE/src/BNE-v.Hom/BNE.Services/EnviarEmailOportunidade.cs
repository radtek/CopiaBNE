        ...
        public static void EnviaEmailOportunidade(DataTable dtVaga)
        {
            ...
            foreach (DataRow row in dtVaga.Rows)
            {
                ...
                if (results.Rows.Count > 0)
                {
                    ...
                    //Funcao estagio
                    var funcao = VagaCurso.ListaCursoParaCartaEmail(row["des_funcao"].ToString(), Convert.ToInt32(row["idf_Vaga"]));

                    objcartaOportunidade.DescricaoAssunto = objcartaOportunidade.DescricaoAssunto.Replace("{Funcao}", funcao)
                    ...
                    carta = carta.Replace("{Funcao}", funcao);
