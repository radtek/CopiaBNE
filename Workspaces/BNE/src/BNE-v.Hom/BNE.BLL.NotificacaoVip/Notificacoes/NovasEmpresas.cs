...
{
    internal class NovasEmpresas : INotificacaoCV
    {
        #region Notificar
        public bool Notificar(DTO.Curriculo curriculo, short idNotificacao, int idCiclo)
        {
            try
            {
                ...
                var numEmpresasNovas = Curriculo.CarregarQtdEmpresasNovas(curriculo.idCurriculo);
                var qtdEmpresasNovas = numEmpresasNovas > 0 ? numEmpresasNovas.ToString() : "várias";
                ...
                var objCarta = CartaEmail.RecuperarCarta(BLL.Enumeradores.CartaEmail.NovasEmpresasNaCidade);
                var link = LoginAutomatico.GerarHashAcessoLogin(curriculo.numeroCPF, curriculo.dataNascimento, "/" + Rota.RecuperarURLRota(BNE.BLL.Enumeradores.RouteCollection.EscolherEmpresa));
                
                assunto = objCarta.Assunto.Replace("{QtdEmpresasNovas}", qtdEmpresasNovas.ToString());
                assunto = assunto.Replace("várias", "Várias");

                mensagem = objCarta.Conteudo.Replace("{Primeiro_Nome}", curriculo.nome);
                mensagem = mensagem.Replace("{link}", link);
                mensagem = mensagem.Replace("{QtdEmpresasNovas}", qtdEmpresasNovas.ToString());
                mensagem = mensagem.Replace("{cidade}", curriculo.cidade + "/" + curriculo.siglaEstado);
                ...
                {
                    var desMensagemSMS = CartaSMS.RecuperaValorConteudo(BLL.Enumeradores.CartaSMS.VipNovasEmpresas);
                    desMensagemSMS = desMensagemSMS.Replace("{Nome}", curriculo.nome);
                    desMensagemSMS = desMensagemSMS.Replace("{QtdEmpresasNovas}", qtdEmpresasNovas.ToString());
            ...
            {
                EL.GerenciadorException.GravarExcecao(ex, "Falha na notificação de novas empresas para o CV " + curriculo.idCurriculo);
                return false;
            }
        }
        #endregion
