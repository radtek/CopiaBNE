...
{
    internal class InformativoGenerico : INotificacaoCV
    {
        public bool Notificar(DTO.Curriculo curriculo, short idNotificacao, int idCiclo)
        {
            try
            {
                ...
                InformativoVip informativoParaEnvio = InformativoVip.RetornarProximoInformativoCVPorTipo(curriculo.idCurriculo, (int)Enumeradores.TipoInformativoVip.Generico);
                ...
                if (informativoParaEnvio != null)
                {
                    ...
                    if(!string.IsNullOrEmpty(informativoParaEnvio.DescricaoAssuntoInformativoVip) && !string.IsNullOrEmpty(informativoParaEnvio.ValorInformativoVip))
                    {
                    ...
                    if (curriculo.numeroCelular.HasValue && !string.IsNullOrEmpty(informativoParaEnvio.DescricaoMensagemSMS))
                    {
            ...
            {
                EL.GerenciadorException.GravarExcecao(ex, "Falha na notificação generica para o CV " + curriculo.idCurriculo);
                return false;
            }
        }
    }
