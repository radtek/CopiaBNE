...
public class HttpCombiner : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        if (request.Url.Host.ToLowerInvariant().IndexOf("localhost") != -1 ||
            request.Url.Host.ToLowerInvariant().IndexOf("127.0.0.1") != -1)
        {
            CACHE_DURATION = TimeSpan.FromMilliseconds(1);
        }
        else
        {
            CACHE_DURATION = TimeSpan.FromDays(10);
        }
