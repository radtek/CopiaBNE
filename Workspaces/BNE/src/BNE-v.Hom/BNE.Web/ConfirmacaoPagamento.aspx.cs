...
namespace BNE.Web
{
    public partial class ConfirmacaoPagamento : BasePage
    {
        ...
        private void GerarBoleto(PlanoAdquirido objPlanoAdquirido)
        {
            ...
            {
                boletos = BoletoBancario.GerarBoletosNovo(objPlanoAdquirido);
            }
            ...
            if (boletos.Count > 0)
            {
                BoletoBancario.RetornarBoleto(BoletoBancario.GerarLayoutBoletoHTML(boletos),out pdfArray,out imgArray,out pdfURL,out imgURL);
                PDFArray = pdfArray;
                PDFURL = pdfURL;
                ImageUrl = imgURL;
            ...
            if (boletos != null && boletos.Count > 1  && pdfArray != null)
            {           
