...
{
    public partial class PaymentMobile : BasePagePagamento
    {
        ...
        //public Boolean ChavePlanoGratis
        //{
        //    get;set;
        //}
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                PrimeiraGratis = Convert.ToBoolean(Request.QueryString["primeira"] != null);
        ...
        private void ValidarCriacaoDoPlano()
        {
            ...
            if (objPlanoAdquirido != null)
            {
                ...
                base.PrimeiraGratis.Value = Request.QueryString["primeira"] != null;
                    ...
                    if (objPlano.FlagRecorrente)
                    {
                        if (objPlano.IdPlano == 620)
                            ltValorPlanoTopo.Text = Request.QueryString["primeira"] == null ? string.Format(
                                    ...
                                    ltValorPlano.Text) :String.Format(Parametro.RecuperaValorParametro(Enumeradores.Parametro.Campanha1Gratis), ltValorPlano.Text);

                        else if (objPlano.IdPlano == 644)
                            ltValorPlanoTopo.Text =
                                string.Format(
                                    Parametro.RecuperaValorParametro(
                                        Enumeradores.Parametro.CampanhaRecorrenciaEstagiario), ltValorPlano.Text);
                        else if (objPlano.IdPlano == 672)
                            ltValorPlanoTopo.Text =
                                string.Format(
                                    Parametro.RecuperaValorParametro(Enumeradores.Parametro.CampanhaBlackFriday), ltValorPlano.Text);
                        else if (objPlano.IdPlano == 676)
                            ltValorPlanoTopo.Text =
                                string.Format(Parametro.RecuperaValorParametro(Enumeradores.Parametro.SegundoMesFree),
                                    ltValorPlano.Text);
                        hRecorrente.Value = "true";
                    }
                    else
                    {
                        ltValorPlanoTopo.Text = string.Format(Parametro.RecuperaValorParametro(Enumeradores.Parametro.CampanhaEmDobroBNE), ltValorPlano.Text);
            ...
            try
            {
                ...
                if (PagamentoMobilePF.ProcessoCompra(ref objPlanoAdquirido, Enumeradores.TipoPagamento.CartaoCredito))
                {
                    ...
                    if (Pagamento.CarregarPagamentoPrimeiraParcelaEmAbertoPorPlanoAdquirido(objPlanoAdquirido.IdPlanoAdquirido, out objPagamento))
                    {
                        ...
                        var retornoTransacao = Transacao.ValidarPagamentoCartaoCreditoPrimeiraGratis(ref objPagamento, objPlanoAdquirido.IdPlanoAdquirido,
                            PageHelper.RecuperarIP(), numeroCartao, mesValidade, anoValidade, codigoSeguranca, nomeCartaoCredito, null, 
                            Enumeradores.GerenciadoraTransacao.Cielo, out erro, out resultadoTransacaoResposta);
                ...
                if (PagamentoMobilePF.ProcessoCompra(ref objPlanoAdquirido, Enumeradores.TipoPagamento.BoletoBancario))
                {
                    ...
                    else
                    {
                        ...
                        if (string.IsNullOrEmpty(objPagamento.DescricaoDescricao))
                            return new { text = String.Join("", System.Text.RegularExpressions.Regex.Split(objBoleto.CodigoBarra.LinhaDigitavel, @"[^\d]")), status = "OK" };
