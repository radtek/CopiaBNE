...
namespace BNE.Web.UserControls.Forms.SalaAdministrador
{
    public partial class EdicaoCV : BaseUserControl
    {
        ...
        protected void gvEdicaoCV_ItemCommand(object source, GridCommandEventArgs e)
        {
            ...
            {
                ucBloquearCandidato.InicializarBloquear(Convert.ToInt32(e.CommandArgument), null, ((Label)e.Item.FindControl("lblNomeCandidato")).Text, ((Label)e.Item.FindControl("lblEmail")).Text);
            ...
            else if (e.CommandName.Equals("QuemMeViu"))
            {
                ...
                Redirect(GetRouteUrl(BLL.Enumeradores.RouteCollection.AdministradorQuemMeViu.ToString(), new { Idf_Curriculo = idCurriculo }));
            }
            else if (e.CommandName.Equals("CancelamentoVIP"))
            {
                int idCurriculo = Convert.ToInt32(gvEdicaoCV.MasterTableView.DataKeyValues[e.Item.ItemIndex]["Idf_Curriculo"]);
                ucAtendimentoVip.MostrarModal(idCurriculo, base.IdUsuarioFilialPerfilLogadoUsuarioInterno.Value);
            }
            else if (e.CommandName.Equals("InformacoesCurriculo"))
            {
                string idCurriculo = gvEdicaoCV.MasterTableView.DataKeyValues[e.Item.ItemIndex]["Idf_Curriculo"].ToString();
                Redirect(GetRouteUrl(BLL.Enumeradores.RouteCollection.CurriculoInformacao.ToString(), new { Idf_Curriculo = idCurriculo }));
        ...
        #region Inicializar
        public void Inicializar(string cpf)
        {
            PageIndex = 1;
            gvEdicaoCV.PageSize = 6;
            if (!string.IsNullOrEmpty(cpf)){
                tbxFiltroBusca.Text = cpf;
                CarregarGrid();
            }
        }
        #endregion
