    ...
    public partial class DetalhesPlano : BaseUserControl
    {
        ...
        protected void gvParcelas_ItemCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            else if (e.CommandName.Equals("Visualizar"))
            {
                try
                {
                    ...
                    hlPDFCobranca.NavigateUrl = BoletoBancario.RetornarBoleto(objPagamento, Enumeradores.FormatoBoleto.PDF, true);
                    hlPDFNormal.NavigateUrl = BoletoBancario.RetornarBoleto(objPagamento, Enumeradores.FormatoBoleto.PDF);
        ...
        protected void gvParcelasAdicional_ItemCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            else if (e.CommandName.Equals("Visualizar"))
            {
                try
                {
                    ...
                    hlPDFCobranca.NavigateUrl = BoletoBancario.RetornarBoleto(objPagamento, Enumeradores.FormatoBoleto.IMAGEM);
                    hlPDFNormal.NavigateUrl = BoletoBancario.RetornarBoleto(objPagamento, Enumeradores.FormatoBoleto.PDF);
        ...
        private bool Salvar()
        {
            try
            {
                ...
                if (Convert.ToInt32(rcbSituacao.SelectedValue) == (int)Enumeradores.PagamentoSituacao.EmAberto 
                                && (!objPagamento.DataVencimento.Equals(txtDataVencimento.ValorDatetime) || !objPagamento.ValorPagamento.Equals(txtValor.Valor.Value)) || !objPagamento.FlagJuros.Equals(chkJuros.Checked))
                    modificacao = true;
                ...
                objPagamento.ValorJuros = valorJuros;
                objPagamento.ValorPagamento = txtValor.Valor.Value + valorJuros;
                ...
                if (objPagamento.AdicionalPlano == null)
                {
                    ...
                    objPagamento.PlanoParcela.ValorParcela = txtValor.Valor.Value + valorJuros;
                    objPagamento.PlanoParcela.EmailEnvioBoleto = txtEmailBoleto.Valor;
                    if (chkNotaAntecipada.Checked)
                    {
        ...
        private void PreencherCampos(string parcela)
        {
            ...
            if (objPagamento.PlanoParcela != null)
            {
                ...
                txtQtdeSMSLiberada.Enabled = !chkLiberarSMS.Checked;
                txtEmailBoleto.Valor = PlanoAdquiridoDetalhes.RetornaEmailNotas(objPagamento.IdPagamento);//string.IsNullOrEmpty(objPagamento.PlanoParcela.EmailEnvioBoleto) ? planoAdiquiridoDetalhes.EmailEnvioBoleto : objPagamento.PlanoParcela.EmailEnvioBoleto;
