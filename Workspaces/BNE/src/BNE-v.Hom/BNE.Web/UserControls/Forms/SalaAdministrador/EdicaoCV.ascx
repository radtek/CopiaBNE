...
<%@ Register Src="../../Modais/EditarDadosPessoais.ascx" TagName="EditarDadosPessoais"
    TagPrefix="uc5" %>
<%@ Register  Src="../../Modais/ucAtendimentoVip.ascx" TagName="AtendimentoVip" 
    TagPrefix="uc6" %>
...
<asp:UpdatePanel ID="upGvEdicaoCV" runat="server" UpdateMode="Conditional">
    <ContentTemplate>
        <div class="painel_padrao_sala_adm">
            <p>
                Você pode pesquisar por Código, CPF, nome, e-mail ou telefone: 
            </p>
                    ...
                    <Columns>
                        <telerik:GridTemplateColumn HeaderText="Nome do Candidato" ItemStyle-CssClass="nome_candidato">
                            <ItemTemplate>
                                ...
                                <asp:Label ID="lblEmail" Visible="false" runat="server" Text='<%# Eval("Eml_Pessoa") %>'></asp:Label>
                                <asp:Label ID="lblIDPlano" Visible="false" runat="server" Text='<%# Eval("Idf_Plano_Adquirido") %>'></asp:Label>
                            </ItemTemplate>
                        </telerik:GridTemplateColumn>
                       <%-- <telerik:GridTemplateColumn HeaderText="E-mail" ItemStyle-CssClass="">
                            <ItemTemplate>
                                <asp:Label ID="lblEmail" runat="server" Style='<%# ( Eval("Idf_Email_Situacao_Bloqueio") != DBNull.Value  && Convert.ToInt32(Eval("Idf_Email_Situacao_Bloqueio")) == 5 ? "color:red;" : "" )  %>' Text='<%# Eval("Eml_pessoa") %>'></asp:Label>
                            </ItemTemplate>
                        </telerik:GridTemplateColumn>--%>
                        ...
                        <telerik:GridTemplateColumn HeaderText="Ações" ItemStyle-CssClass="col_action_arq"  >
                            <ItemTemplate>
                                ...
                                <asp:LinkButton ID="btiVIP" Visible='<%# Convert.ToBoolean(Eval("Flg_VIP")) %>' runat="server" ToolTip="Cancelamento VIP" AlternateText=" Cancelamento VIP" CausesValidation="false"
                                    CommandName="CancelamentoVIP" Text="VIP"  />
                                <asp:LinkButton ID="btiInformacoesCurriculo" runat="server" ToolTip="Informações do Currículo" 
                                    CommandName="InformacoesCurriculo" Text="S.A"></asp:LinkButton>
                            </ItemTemplate>
                        </telerik:GridTemplateColumn>
                    </Columns>
                    <NoRecordsTemplate>
...
<uc6:AtendimentoVip  ID="ucAtendimentoVip" runat="server" ></uc6:AtendimentoVip>
