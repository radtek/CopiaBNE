...
namespace AdminLTE_Application.Controllers
{
    ...
    public class EmpresaSemPlanoController : Controller
    {
        ...
        // GET: EmpresaSemPlano
        public ActionResult Index(string sortOrder, string currentFilter, string SearchString, int? page, string opcaoPesquisa, int? PageSize, string currentFilterSituacao, string reservation)
        {
            ...
            ViewBag.dtUltimoAtendimentoSort = "Ultimo Atendimento Vendedor desc";
            ...
            ViewBag.qtdFuncionarios = "Num Funcionario desc";
            ...
            var empresas = from s in db.VW_EMPRESA_SEM_PLANO
                           select s;
            switch (sortOrder)
            {
                ...
                case "Ultima Ação Cliente asc":
                    ...
                    ViewBag.dtUltimaAcaoClienteSort = sortOrder;
                    break;
                case "Ultimo Atendimento desc":
                    empresas = empresas.OrderByDescending(s => s.Data_ultimo_ATC_empresa);
                    ViewBag.dtUltimoAtendimentoSort = sortOrder;
                    break;
                case "Ultimo Atendimento asc":
             ...
             if (String.IsNullOrEmpty(SearchString) && !String.IsNullOrEmpty(currentFilterSituacao) && !String.Equals(currentFilterSituacao,"X"))
            {
             ...
             if (!string.IsNullOrEmpty(reservation))
             {
                 DateTime datainicio;
                 DateTime datafim;
                 datainicio = DateTime.ParseExact(reservation.Substring(0, 10), "MM/dd/yyyy", new CultureInfo("pt-BR"));
                 datafim = DateTime.ParseExact(reservation.Substring(13, 10), "MM/dd/yyyy", new CultureInfo("pt-BR"));
