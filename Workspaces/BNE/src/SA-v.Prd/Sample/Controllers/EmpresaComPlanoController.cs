...
namespace AdminLTE_Application.Controllers
{
    ...
    public class EmpresaComPlanoController : Controller
    {
        ...
        // GET: EmpresaSemPlano
        public ActionResult Index(string button, GridEmpresaComPlano model)
        {
            ...
            using (var context = new Model())
            {
                ...
                var empresas = from s in context.VW_EMPRESA_COM_PLANO
                               select s;
                ...
                if (model.FlSituacaoEmpresa.HasValue && model.FlSituacaoEmpresa.Value > 0)
                    empresas = empresas.Where(s => s.Idf_Situacao_Filial.Equals(model.FlSituacaoEmpresa.Value));

                if (!string.IsNullOrEmpty(model.FlPlano))
                    empresas = empresas.Where(s => s.Des_Plano.Contains(model.FlPlano));
                ...
                switch (model.Ordenacao)
                {
                    ...
                    case "Boleto_Vencimento asc":
                        ...
                        break;
                    case "Plano desc":
                        empresas = empresas.OrderByDescending(s => s.Des_Plano);
                        break;
                    case "Plano asc":
                        empresas = empresas.OrderBy(s => s.Des_Plano);
                        break;
                    default:
                        empresas = empresas.OrderByDescending(s => s.Data_Ultima_Acao_Site);
                        model.Ordenacao = "Ultimo_Atendimento desc";
                        break;
                }
                #endregion
                ...
                using (var bd = new Model())
                {
                    ...
                    foreach (var item in model.Empresas.Where(x => !x.qtd_prazo_boleto.HasValue))//tipo boleto bancaario
                    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Num_CPF,Num_CNPJ,Dta_Cadastro,Des_Atendimento,Data_Ultimo_Atendimento,Data_Ultima_Acao_Site,Data_ultimo_ATC_empresa,Idf_Atendimento,Raz_Social,Sig_Estado,Des_Area_BNE,Nme_Cidade,Dta_Fim_Plano,Total_Acao,Dta_Retorno, Des_Plano")] VW_EMPRESA_COM_PLANO VW_EMPRESA_COM_PLANO)
        {
            if (ModelState.IsValid)
            {
