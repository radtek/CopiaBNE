...
namespace Sample.Controllers
{
    ...
    public class TransferirController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
                ...
                var cpfadm = Convert.ToDecimal(userWithClaims.Claims.FirstOrDefault(c => c.Type.Equals("cpf")).Value);

                using (var context = new Model())
                {
                    var cpf = new SqlParameter("@cpf", cpfpara);
                    var cnpjt = new SqlParameter("@cnpj", cnpj);
                    var cpfADM = new SqlParameter("@cpf_adm", cpfadm);

                    var result = context.Database
                        .ExecuteSqlCommand("dbo.transfereEmpresa @cnpj, @cpf, @cpf_adm", cnpjt, cpf, cpfADM);
