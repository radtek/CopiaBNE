var AzulzinhoController = angular.module('AzulzinhoController', []);

AzulzinhoController.controller('CadastroCVController', ['$scope', '$http', '$location', '$route', '$rootScope', '$filter', '$anchorScroll', 'localStorageService', 'authService',
    ...
    $scope.contarVagasPerfilCandidato = function () {
        ...
        //"Vaga/GetVagasAzulzinho?idFuncao=0&filtro=&start=0&rows=18"
        $http.get(urlAPI + "Vaga/GetVagasAzulzinho?filtro=" + encodeURIComponent(store.curriculo.cidade) + "&start=0&rows=10000&idFuncao=" + $rootScope.idFuncaoPretendida)

        //$http.get(urlAPI + "Vaga?filtro=&start=0&rows=100&idFuncao=" + $rootScope.idFuncaoPretendida)
            .success(function (data, status, headers, config) {
    ...
    $scope.IniciarEscolherCV = function () {
        ...
        console.log('idiomas candidato na session =>', sessionStorage.idiomasCandidato);
    }
   ...
   //angular x-editable
    $scope.showEstadoCivil = function () {
        ...
        return (store.curriculo.idEstadoCivil && selected.length) ? selected[0].text : '';
    };
    ...
    $scope.salvarCadastroPasso1 = function () {
        ...
        $http.post(urlAPI + "Curriculo/SalvarCV?passoCadastro=1", sessionStorage.curriculo)
            .success(function (data, status, headers, config) {
                ...
                console.log('CV no Passo 1 =>', sessionStorage.curriculo);
    ...
    $scope.salvarCadastroPasso2 = function () {
        ...
        $http.post(urlAPI + "Curriculo/SalvarCV?passoCadastro=2", sessionStorage.curriculo)
            ...
            .error(function (data, status, headers, config) {
                console.log('erro passo 2 ', data);
            });

        console.log('CV no Passo 2 =>', sessionStorage.curriculo);
    };
    ...
    $scope.onStream = function (stream, videoElem) {
        //console.log('onstream',stream, videoElem);
    ...
    //store.curriculo.cursos = angular.fromJson(sessionStorage.Cursos);
    //console.log('cursos =>', store.cursos);
}]);

AzulzinhoController.controller('HomeController', ['$scope', '$http', '$location', '$rootScope', 'authService', function ($scope, $http, $location, $rootScope, $authService) {
...
AzulzinhoController.controller('ImprimirCVController', ['$scope', '$http', '$location', function ($scope, $http, $location) {
