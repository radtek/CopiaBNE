    ...
    internal partial class IntegracaoWebfopag : BaseService
    {
                    ...
                    try
                    {
                       EventLogWriter.LogEvent(string.Format("Iniciou agora {0}.", DateTime.Now),
                        ...
                        foreach (var objIntegracao in listaIntegrao)
                        {
                            ...
                            {
                                GerenciadorException.GravarExcecao(ex, "Integracao Recisao - idf_integracao " + objIntegracao.IdIntegracao);
                                objIntegracao.IntegracaoSituacao =
                        ...
                        foreach (var objIntegracao in listaIntegraoAdmissao)
                        {
                            ...
                            {
                                GerenciadorException.GravarExcecao(ex, "Integracao Admissao - idf_integracao " + objIntegracao.IdIntegracao);
                                objIntegracao.IntegracaoSituacao =
                                    new BLL.IntegracaoSituacao((int)IntegracaoSituacao.ComErro);
                                objIntegracao.Save();
                            }
        ...
        private void IntegrarAdmissao(Integracao objIntegracao)
        {
            ...
            if (objIntegracao.NumeroCPF.HasValue)
            {
                ...
                //caso o curriculo seja vip não deve bloquear o curriculo
                ...
                var atualizaCvSolr = objCurriculo.FlagVIP ? true : false;
                objCurriculo.SalvarIntegracao(objIntegracao, objPessoaFisicaComplemento, objPessoaFisicaVeiculo,
                    objExperienciaProfissional, false, atualizaCvSolr);
                ...
                if (excluirCvSolr)
                {
                    ...
                    Curriculo.ExcluirCVSolr(objCurriculo.IdCurriculo, objCurriculo.PessoaFisica.EmailPessoa);
                }
        ...
        private void CarregarPessoaFisica(Integracao objIntegracao, ref PessoaFisica objPessoaFisica)
        {
            ...
            if (!string.IsNullOrEmpty(objIntegracao.NumeroCelular))
            {//pra ter certeza que não sobreescrever o telefone da pessoa por null
