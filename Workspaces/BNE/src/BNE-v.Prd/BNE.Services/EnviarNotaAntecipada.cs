...
namespace BNE.Services
{
    partial class EnviarNotaAntecipada : ServiceBase, IJob
    {
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                var enviados = Pagamento.EnvioNotaAntecipadaSemPagamento();
                GravaLogText($"Envidas {enviados} notas. {DateTime.Now}");
                Thread.Sleep(new TimeSpan(0, 60, 0));//enviar as notas que ainda não tinham sido geradas.
                enviados = Pagamento.EnvioNotaAntecipadaSemPagamento();
                GravaLogText($"Envidas {enviados} notas. {DateTime.Now}");

            }
            catch (Exception ex)
            {
                EL.GerenciadorException.GravarExcecao(ex, "serviço EnviarNotaAntecipada");
            }
        ...
        protected override void OnStart(string[] args)
        {
            ...
            var trigger = TriggerBuilder.Create()
                .StartNow()
                .WithSchedule(CronScheduleBuilder.AtHourAndMinuteOnGivenDaysOfWeek(6, 0, dias))
                .Build();
        ...
        {
            using (StreamWriter sw = File.AppendText(AppDomain.CurrentDomain.BaseDirectory + "Nota_Antecipada.txt"))
            {
