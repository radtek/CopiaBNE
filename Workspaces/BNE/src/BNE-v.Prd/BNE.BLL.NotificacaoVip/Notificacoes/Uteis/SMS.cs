    ...
    internal class SMS
    {
        ...
        public void Enviar()
        {
            using (var objWsTanque = new BNETanqueService.AppClient())
            {
                ...
                //Tratativa de cópia de SMS enviados para o Marcelo a pedido do Tortato e Gustavo Brasil
                if (_idCV == 1747084)
                {
                    listaSMS.Add(new BNETanqueService.Mensagem
                    {
                        ci = _idCV.ToString(),
                        np = "Tortato",
                        nc = 41999624495,
                        dm = "Copia SMS Marcelo -> " + _mensagem
                    });
