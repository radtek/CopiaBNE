...
namespace BNE.BLL.Notificacao
{
    public partial class AlertaCurriculos // Tabela: alerta.Tab_Alerta_Curriculos
    {
        ...
        private const string spExisteAlerta = @"select top 1 idf_curriculo from alerta.tab_alerta_curriculos with(nolock)
        ...
        public static void Atualizar(IEnumerable<int> curriculos)
        {
            var parms = new List<SqlParameter>
            {
                new SqlParameter {ParameterName = "@idfs_curriculos", SqlDbType = SqlDbType.VarChar, Size = -1, Value = string.Join(",", curriculos)},
        ...
        #region [ExisteAlerta]
        public static bool ExisteAlerta(int idCurriculo)
        {
