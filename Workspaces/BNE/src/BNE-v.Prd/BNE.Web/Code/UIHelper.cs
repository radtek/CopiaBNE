...
namespace BNE.Web.Code
{
    public static class UIHelper
    {
        ...
        public static void CarregarRadComboBox(RadComboBox rcb, IDictionary<string, string> dataSource)
        {
            CarregarRadComboBox(rcb, dataSource, null);
        }
        public static void CarregarRadComboBox(RadComboBox rcb, IDictionary<string, string> dataSource, RadComboBoxItem rcbiIndexZero)
        {
        ...
        {
            return Helper.FormatarCidade(nomeCidade, siglaEstado);
        }
        #endregion
        ...
        {
            return Helper.FormatarTelefone(ddd, telefone);
        }
        #endregion
        ...
        public static bool RecuperarEnderecoCompleto(string numCEP, out string descricaoLogradouro, out string descricaoBairro, out string nomeCidade, out string siglaEstado)
        {
            ...
            try
            {
                var objCep = new Cep
                {
                    sCep = numCEP.Replace("-", string.Empty).Replace(".", string.Empty).Trim()
                };

                var qtdeCepEncontrados = Cep.ContaConsulta(objCep);

                if(!qtdeCepEncontrados.Equals(0))
                {
        ...
        {
            return Helper.RecuperarURLAmbiente();
        }
        #endregion
        ...
        /// <returns></returns>
        public static string RetornarUrlFoto(decimal numeroCPF, Handlers.PessoaFisicaFoto.OrigemFoto origemFoto)
        {
        ...
        public static string TratarEmail(string email, out string At)
        {
            int cort = email.IndexOf("@");
            string mail = email.Substring(0, cort);
            At = email.Substring(cort+1, email.Length - cort-1 );
            return mail;
        }
