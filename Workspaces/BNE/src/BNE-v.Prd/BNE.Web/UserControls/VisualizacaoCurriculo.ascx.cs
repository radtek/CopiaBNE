...
namespace BNE.Web.UserControls
{
    public partial class VisualizacaoCurriculo : BaseUserControl
    {
        ...
        private void PreencherCampos(bool mostrarDadosCompletos)
        {
            ...
            var objCurriculoDTO = Curriculo.CarregarCurriculoSolr(IdCurriculoVisualizacaoCurriculo) ?? Curriculo.CarregarCurriculoDTO(IdCurriculoVisualizacaoCurriculo, Curriculo.DadosCurriculo.Tudo);
            ...
            if (objCurriculoDTO != null && !objCurriculoDTO.Inativo)
            {
                ...
                carregarRespostasPergunta(objCurriculoDTO);
        ...
        private void VerDados()
        {
            if (base.IdPessoaFisicaLogada.HasValue)
            {
                if (base.IdFilial.HasValue)
                {
                    ...
                    else if (objFilial.SituacaoFilial.IdSituacaoFilial.Equals((int)BLL.Enumeradores.SituacaoFilial.ForaDoHorarioComercial))
                    {
                        ucEmpresaBloqueada.MostrarModalForaHorario();
                        return;
                    }
        ...
        private void CarregarDadosPessoais(BLL.DTO.Curriculo objCurriculo, bool mostrarDadosCompletos)
        {
            if (mostrarDadosCompletos)
            {
                ...
                if (objCurriculo.Flg_WhatsApp.HasValue && objCurriculo.Flg_WhatsApp.Value)
                    divWhats.Visible = true;
        ...
        #region [carregarRespostasPergunta]
        private void carregarRespostasPergunta(BLL.DTO.Curriculo objCurriculo)
        {
            if (IdVaga.HasValue)
            {
                //consulta no solr as respostas no vaga Candidato.
              var vagaCand =  BuscaCurriculo.MontarQueryCandidatoVaga(IdVaga.Value, objCurriculo.IdCurriculo);
                DataTable dt = new DataTable();
                dt.Columns.Add(new DataColumn("Pergunta", Type.GetType("System.String")));
                dt.Columns.Add(new DataColumn("Resposta", Type.GetType("System.String")));

                foreach (var item in vagaCand.response.docs)
                {
                    if(item.Des_Vaga_Pergunta !=null && item.Des_Vaga_Pergunta.Length > 0)
                    {
                        for (int i = 0; i < item.Des_Vaga_Pergunta.Length; i++)
                        {
                            var dr = dt.NewRow();

                            dr["Pergunta"] = item.Des_Vaga_Pergunta[i];
                            dr["Resposta"] = string.IsNullOrEmpty(item.Des_Resposta[i]) ? (item.Flg_Resposta[i] ? "Sim" : "Não") : item.Des_Resposta[i];
                            dt.Rows.Add(dr);
                ...
                if(dt.Rows.Count > 0)
                {
                    pnlRespostas.Visible = true;
                    UIHelper.CarregarRepeater(rpRespostas, dt);
                    upResposta.Update();
                }
            }
        }
