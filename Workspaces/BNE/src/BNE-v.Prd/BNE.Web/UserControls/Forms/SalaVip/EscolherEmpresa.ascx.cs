...
namespace BNE.Web.UserControls.Forms.SalaVip
{
    public partial class EscolherEmpresa : BaseUserControl
    {
        ...
        void ucVerDadosEmpresa_Candidatar()
        {
            try
            {
                Curriculo objCurriculo = new Curriculo(base.IdCurriculo.Value);
                ...
                //Enviar para o STC da empresa
                try
                {
                    int? idOrigemFilial = OrigemFilial.RecuperarIdOrigemPorFilial(new Filial(ucVerDadosEmpresa.IdFilial));
                    if (idOrigemFilial.HasValue && !CurriculoOrigem.ExisteCurriculoNaOrigem(objCurriculo, new Origem(idOrigemFilial.Value)))
                    {
                        //Empresa tem STC.
                ...
                {
                    EL.GerenciadorException.GravarExcecao(ex, String.Format("Erro ao gravar origem do curriculo, na tela de escolha de empresa id filial {0} idcv {1}", ucVerDadosEmpresa.IdFilial, base.IdCurriculo.Value));
                }
        ...
        {
            //Verifica se existe parametro cidade para fazer a busca
            if (Request.QueryString["Cidade"] != null)
                txtCidade.Text = Convert.ToString(Request.QueryString["Cidade"]);
        ...
        private void CarregarGridEscolherEmpresa()
        {
            ...
            {
                int totalRegistros;
                UIHelper.CarregarRadGrid(gvEscolherEmpresa, Filial.ListarFilialPorRazaoRamoCidade(txtCidade.Text, txtRamoAtividade.Text, txtNomeEmpresa.Text, base.IdCurriculo.Value, PageIndex, gvEscolherEmpresa.PageSize, out totalRegistros), totalRegistros);
            }
            else
            {
                base.ExibirLogin();
