...
function SugestaoFuncao(args) {
    SugesteFuncaoPretendida(args);
}
...
function SalvarDadosContato() {
    ...
    if ($('#ddlAt').find('option:selected').text() != "") {
        email = email +'@'+ $('#ddlAt').find('option:selected').text();
    }
    else {
        email = email + '@' + $('#txtAT').val();
    }
    var pretensao = employer.controles.recuperarValorControle('txtPretensaoSalarial');

    var termosDeUso = $("#cbPoliticaPrivacidade").is(':checked');
    var whatsApp = $("#cbWhats").is(':checked');
    var idEscolaridade = employer.controles.recuperarValor('ddlEscolaridade');
    var escolaridadest = $('#ddlEscolaridade').find('option:selected').text();

    var dados = "{'cpf':'" + cpf + "','dataNascimento':'" + dataNascimento + "','nome':'" + nome + "','sexo':'" + sexo + "','funcao1':'" + funcao1
        + "','ano1':'" + ano1 + "','mes1':'" + mes1 + "','funcao2':'" + funcao2 + "','ano2':'" + ano2 + "','mes2':'" + mes2
        + "','funcao3':'" + funcao3 + "','ano3':'" + ano3 + "','mes3':'" + mes3 + "','dddCelular':'"
        + dddCelular + "' ,'celular':'" + celular + "','email':'" + email + "','nomeCidade':'"
        + nomeCidade + "','pretensao':'" + pretensao + "','termosDeUso':'" + termosDeUso + "','whatsApp':'" + whatsApp + "','escolaridade':'" + idEscolaridade + "','escolaridadest':'" + escolaridadest+"'}";
...
function SugesteFuncaoPretendida(item) {
    if (item != null) {
        var txtfunc = item.id[item.id.length - 1];
        var sugestoes = "";
        $.ajax({
            type: "POST",
            url: "/ajax.aspx/SugestaoFuncao",
            data: "{'funcao': '" + item.value + "'}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            success: function (data, response) {
                ...
                if (data.d == null || data.d.length <= 0) {
                    employer.util.findControl('divSugestFuncaoSinonimo' + txtfunc).css('display', 'none');
                    return;
                }
                ...
                employer.util.findControl('divSugestFuncaoSinonimo' + txtfunc).css('display', 'flex');
                document.getElementById('spanPorcentagem' + txtfunc).innerHTML = data.d[0].porcentagem;

                for (var i = 0; i < data.d.length; i++) {
                    sugestoes += '<span onclick="TrocaFuncao(this);" id="spanFuncaoSugestao' + i + txtfunc + '">' + data.d[i].Funcao + '</span>';
                }

                document.getElementById('spanFuncaoSugestao' + txtfunc).innerHTML = sugestoes;
            },
...
function CloseSugest(campo) {
    employer.util.findControl('divSugestFuncaoSinonimo' + campo).css('display', 'none');

}

function TrocaFuncao(args) {
    var campo = args.id[args.id.length - 1];
    console.log(args.id[args.id.length - 1]);
    $('input[name*=txtFuncaoPretendida' + campo + ']').val(args.innerHTML);
    CloseSugest(campo);
}

function checarPoliticaPrivacidade(source, args) {
    if ($('#cbPoliticaPrivacidade').is(':checked')) {
        args.IsValid = true;
    }
    else {
        args.IsValid = false;

        var element = $("#politica-privacidade");
        $('html, body').animate({ scrollTop: $(element).offset().top - 270 }, 300, 'linear', function () {
            jQuery(element).animate({ backgroundColor: 'red' }, 'fast', 'linear', function () {
