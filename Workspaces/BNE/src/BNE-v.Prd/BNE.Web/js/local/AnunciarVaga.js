...
function redirectToWebEstagios() {
    var href = 'http://employerestagios.com.br/?utm_source=bne&utm_campaign=gridvagas&utm_term=bne_Anunciar_vaga';
...
function AjustarPretensaoSalarial() {
    ...
    function validarSalarioMinimo(valorDe, valorAte) {
        ...
        var ehAprendiz = $('#cphConteudo_hfAprendiz').val() === "1" ? true : false;

        var valordoMinimo = $('#cphConteudo_hdfValorSalarioMinimo').val().replace(',', '.');


        if (ehEstagio) {
            valordoMinimo = $('input[name*=hdfValorBolsamInimo]')[0].value;
        }
        else if (ehAprendiz) {
            valordoMinimo = $('input[name*=hdfValorBolsaMinimoAprendiz]')[0].value;
        }
...
var abrirCompartilhamentoFacebook = function (url) {
    ...
    setTimeout(function () {
        window.document.body.scrollTop = document.getElementById('vaga_pub_success').offsetTop;
    }, 10);
}
