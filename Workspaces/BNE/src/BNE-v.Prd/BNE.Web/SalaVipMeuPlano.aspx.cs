...
namespace BNE.Web
{
    public partial class SalaVipMeuPlano : BasePagePagamento
    {
        ...
        private void PreencheCampos(PlanoAdquirido objPlanoAdquiridoLiberado, bool mostraLinkRenovar)
        {
            if (objPlanoAdquiridoLiberado != null)
            {
                ...
                if (objPlanoAdquiridoLiberado.FlagRecorrente)
                    pnlPlanoValidade.Visible = false;
                else
                ...
                AjustarTituloTela($"Meu Plano - {objPlanoAdquiridoLiberado.Plano.DescricaoPlano}", "salavip_meuplano_tpplano__nmplano");
                ...
                int dias = DateTime.Now.Subtract(objPlanoAdquiridoLiberado.DataInicioPlano).Days;
                hdfDiasVip.Value = dias.ToString();
                if (dias <= 30)
                {
                    spanExtrato.Visible = spanDdl.Visible = false;
                    spanPerido.Visible = true;
                }
                else
                {
                    spanExtrato.Visible = spanDdl.Visible = true;
                    spanPerido.Visible = false;
                    dias = 30;
                }
                CarregarExtrato(dias);
                upMeuPlano.Update();
            }
        }
        ...
        //        divErroCancelamentoAssinatura.Visible = true;
            ...
            for (int i =0; i< cblMotivoCancelar.Items.Count; i++){
                if (cblMotivoCancelar.Items[i].Selected)
                {
                    ...
                    objPlanoMotivoBloqueio.FlgInativo = true;// ativar quando for cancelado pelo adm
        ...
        private void CarregarExtrato(int dias)
        {
            BLL.DTO.ExtratoVIP objExtrato = PlanoAdquirido.ExtratovipDias(base.IdCurriculo.Value, dias);
            lblVisualizacoesCurriculo_mes.InnerText = objExtrato.VisualizacoesCurriculo.ToString();
            lblJornalRecebido_mes.InnerHtml = objExtrato.VagasPeloJornal > 9999 ? "+9999" : objExtrato.VagasPeloJornal.ToString();
            lblVagasCandidatadas_mes.InnerText = objExtrato.VagasCandidatadas.ToString();
            lblVagasVisualizadas_mes.InnerText = objExtrato.VagasVisualizadas.ToString();
            lblApareceuNasBuscas_mes.InnerText = objExtrato.ApareceuNasBuscas.ToString();
            lblEmpresasBuscaramSeuPerfil_mes.InnerText = objExtrato.EmpresaBuscouSeuPerfil.ToString();
            lblVagasNaoVisualizadas_mes.InnerText = objExtrato.VagasNaoVisualizadas.ToString();
            lblBuscasPeloSeuPerfil_mes.InnerText = objExtrato.BuscaSeuPerfil.ToString();
            lblVagasNoSeuPerfil_mes.InnerText = objExtrato.VagasNoPerfil.ToString();
        ...
        protected void ddlPeriodo_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (ddlPeriodo.SelectedValue.Equals("0"))
                    CarregarExtrato(30);
                else
                    CarregarExtrato(Convert.ToInt32(hdfDiasVip.Value));
            }
            catch (Exception)
            {
                CarregarExtrato(30);
