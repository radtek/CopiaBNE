...
namespace BNE.BLL
{
    public partial class VagaPergunta // Tabela: BNE_Vaga_Pergunta
    {
        ...
        private const string Spselectperguntas = "SELECT Idf_Vaga_Pergunta, Des_Vaga_Pergunta, Flg_Resposta, Idf_Tipo_Resposta, Idf_Vaga, Dta_Cadastro FROM BNE_Vaga_Pergunta WITH(NOLOCK) WHERE Idf_vaga = @Idf_Vaga AND Flg_Inativo = 0 ORDER BY Idf_Vaga_Pergunta";
        #endregion
        ...
        #region Construtores
        public VagaPergunta(int idVagaPergunta, string desVagaPergunta, bool? flagResposta, TipoResposta tipoResposta, DateTime dataCadastro, Vaga vaga)
        {
        ...
        public static List<VagaPergunta> RecuperarListaPerguntas(int idVaga, SqlTransaction trans = null)
        {
            var parms = new List<SqlParameter>
                {
            ...
            while (dr.Read())
            {
                ...
                listVagaPergunta.Add(new VagaPergunta(Convert.ToInt32(dr["Idf_Vaga_Pergunta"]), dr["Des_Vaga_Pergunta"].ToString(), flagResposta, new TipoResposta(Convert.ToInt32(dr["Idf_Tipo_Resposta"])), Convert.ToDateTime(dr["Dta_Cadastro"]), new Vaga(Convert.ToInt32(dr["Idf_Vaga"]))));
            }
        ...
        public int MigrationId
        {
            set
            {
                this._idVagaPergunta = value;
            }
            get { return this._idVagaPergunta; }
        }
        #endregion
    }
}