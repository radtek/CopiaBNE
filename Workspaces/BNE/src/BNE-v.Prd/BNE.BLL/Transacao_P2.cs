...
using Phone = Uol.PagSeguro.Domain.Phone;
using Transaction = Uol.PagSeguro.Domain.Transaction;
...
namespace BNE.BLL
{
    public partial class Transacao // Tabela: BNE_Transacao
    {
        ...
        public static readonly string DefaultApiKeyPagarMe = ConfigurationManager.AppSettings["DefaultApiKeyPagarMe"];
        public static readonly string DefaultEncryptionKeyPagarMe = ConfigurationManager.AppSettings["DefaultEncryptionKeyPagarMe"];
        public static readonly string UrlRetorno = ConfigurationManager.AppSettings["urlRetorno"];
        ...
        public string NomeCartaoCredito
        {
            get
            {
                return !string.IsNullOrEmpty(_nomeCartaoCredito)
                 ? Crypto.Decrypt(this._nomeCartaoCredito)
                 : string.Empty;
            }
            set
            {
                ...
                this._nomeCartaoCredito = !string.IsNullOrEmpty(value) ? Crypto.Encrypt(value) : string.Empty;
                this._numeroVersaoCriptografia = Crypto.Version;
                this._modified = true;
            }
        }
        ...
        WHERE
		       ...
		       AND T.Idf_Status_Transacao IN (3,8,9) -- 9 => Realizada 3 => Capturado
        ...
        private const string SP_SELECT_IDF_PAGAMENTO = "SELECT * FROM BNE_Transacao WITH(NOLOCK) WHERE Idf_Pagamento = @Idf_Pagamento";
        #endregion
                ...
                if (!objPagamento.JaPago(trans))
                {
                            ...
                            ((objPlanoAdquirido != null && objPlanoAdquirido.FlagRecorrente) || (objPlanoAdquirido.Plano.CompleteObject() && objPlanoAdquirido.Plano.FlagRecorrente)))
                    {
                        ...
                        else
                        {
                            ...
                            Curriculo.CarregarPorPessoaFisica(trans, objPagamento.UsuarioFilialPerfil.PessoaFisica.IdPessoaFisica, out objCurriculo);
                            CurriculoObservacao.SalvarCRM(
                                "Plano Recorrente " + objPlanoAdquirido.IdPlanoAdquirido +
                                " Renovação, via Leitura de arquivo", objCurriculo,
                                "ControleParcelas -> Renovacao Recorrencia", trans);
                        }
            ...
            try
            {
                if (dr.Read())
                {
                    ...
                    if (dr["Nome_Cartao_Credito"] != DBNull.Value)
                        objTransacao._nomeCartaoCredito = dr["Nome_Cartao_Credito"].ToString();
        ...
        public static Transacao CriarNovaTransacao(Transacao objTransacao, ref Pagamento objPagamento, SqlTransaction trans)
        {
            return CriarTransacaoCartaoCredito(
                ...
                objTransacao.NomeCartaoCredito,
                (Enumeradores.GerenciadoraTransacao)objTransacao.GerenciadoraTransacao,
        ...
        private static Transacao CriarTransacaoCartaoCredito(ref Pagamento objPagamento, int idPlanoAdquirido, string descricaoIP, string numeroCartao, int mesValidadeCartao, int anoValidadeCartao, string numeroDigitoVerificador, Enumeradores.Operadora bandeiraCartao, string nomeCartaoCredito, Enumeradores.GerenciadoraTransacao gerenciadoraTransacao,  string desCodigoToken = null)
        {
            return CriarTransacaoCartaoCredito(ref objPagamento, idPlanoAdquirido, descricaoIP, numeroCartao, mesValidadeCartao, anoValidadeCartao, numeroDigitoVerificador, bandeiraCartao, nomeCartaoCredito, gerenciadoraTransacao,  desCodigoToken,  null);
        }
        private static Transacao CriarTransacaoCartaoCredito(ref Pagamento objPagamento, int idPlanoAdquirido, string descricaoIP, string numeroCartao, int mesValidadeCartao, int anoValidadeCartao, string numeroDigitoVerificador, Enumeradores.Operadora bandeiraCartao, string nomeCartaoCredito, Enumeradores.GerenciadoraTransacao gerenciadoraTransacao,  string desCodigoToken, SqlTransaction trans)
        {
            try
            {
                var objTransacao = new Transacao
                {
                    ...
                    NomeCartaoCredito = nomeCartaoCredito.ToString(), 
                    GerenciadoraTransacao = (int)gerenciadoraTransacao
        ...
        public static bool ValidarPagamentoCartaoCredito(ref Pagamento objPagamento, int idPlanoAdquirido, string descricaoIP, String numeroCartao, int mesValidadeCartao, int anoValidadeCartao, string numeroDigitoVerificador, string nomeCartaoCredito, string desCodToken, BLL.Enumeradores.GerenciadoraTransacao gerenciadoraTransacao,  out string erro, out string  resultadoTransacaoResposta)
        {
            erro =  resultadoTransacaoResposta = string.Empty;
            ...
            {
                //Criando a transação 
                var objTransacao = CriarTransacaoCartaoCredito(ref objPagamento, idPlanoAdquirido, descricaoIP, numeroCartao, mesValidadeCartao, anoValidadeCartao, numeroDigitoVerificador, operadora.Value, nomeCartaoCredito, gerenciadoraTransacao,  desCodToken);

                //Recuperando o retorno
                CartaoCredito.RetornoCartaoCredito objRetorno = null;
                try
                {
                    if (objTransacao.Operadora.IdOperadora == (int)Enumeradores.Operadora.Hipercard)
                    {
                        gerenciadoraTransacao = Enumeradores.GerenciadoraTransacao.PagarMe;
                    }
                    objRetorno = gerenciadoraTransacao == Enumeradores.GerenciadoraTransacao.Cielo ? EfetuarPagamentoCartaoDeCreditoCielo(objTransacao, out resultadoTransacaoResposta) : EfetuarPagamentoCartaoDeCreditoPagarMe(objTransacao,out resultadoTransacaoResposta);
                    
                    if (objRetorno != null && !objRetorno.Aprovado)
                    {
                        //1º COMPRA
                        if (string.IsNullOrEmpty(desCodToken) && UtilizaPagarMe == "True" && gerenciadoraTransacao == Enumeradores.GerenciadoraTransacao.Cielo)
                        {
                            objRetorno = EfetuarPagamentoCartaoDeCreditoPagarMe(objTransacao, out resultadoTransacaoResposta);
                            objTransacao.GerenciadoraTransacao = (int)Enumeradores.GerenciadoraTransacao.PagarMe;
                        }
                    }
                }
                catch (Exception ex)
                {
                    EL.GerenciadorException.GravarExcecao(ex, "Recuperar Retorno Transacao. idTransacao=" + objTransacao.IdTransacao.ToString());
            ...
            {
                EL.GerenciadorException.GravarExcecao(ex, "Criação da Transacao");
                throw;
            }
        }
        #endregion
        ...
        public static bool ValidarPagamentoCartaoCreditoPrimeiraGratis(ref Pagamento objPagamento, int idPlanoAdquirido, string descricaoIP, String numeroCartao, int mesValidadeCartao, int anoValidadeCartao, string numeroDigitoVerificador, string nomeCartaoCredito, string desCodToken, BLL.Enumeradores.GerenciadoraTransacao gerenciadoraTransacao, out string erro,out string resultadoTransacaoResposta)
        {
            erro  = resultadoTransacaoResposta = string.Empty;
            ...
            try
            {
                ...
                if (objRetorno != null)
                {
                    ...
                    if (objRetorno.Aprovado)
                    {
                        ...
                        if (objTransacao.GerenciadoraTransacao == (int)Enumeradores.GerenciadoraTransacao.Cielo)
                        {
                            Transacao.EfetuarCancelamentoCartaoDeCreditoCielo(objPagamento.DescricaoIdentificador);
                        }
                        else
                        {
                            PagarMeOperacoes.CancelarTransacaoPagarMe(objPagamento);
                        }
                    }
                    else
                    {
        ...
        #region EfetuarPagamentoCartaoDeCreditoCielo
              public static CartaoCredito.RetornoCartaoCredito EfetuarPagamentoCartaoDeCreditoCielo(Transacao objTransacao, out string ResultadoSolicitacaoResposta)
        {
            ResultadoSolicitacaoResposta = string.Empty;
            string mid = ConfigurationManager.AppSettings["cielo.customer.id"];
            string key = ConfigurationManager.AppSettings["cielo.customer.key"];
            string url = ConfigurationManager.AppSettings["cielo.webservice.url"];
            ...
            switch (objTransacao.Operadora.IdOperadora)
            {
                ...
                case (int)Enumeradores.BandeiraCartao.MasterCard:
                    ...
                    break;
                case (int)Enumeradores.BandeiraCartao.Dinners:
            ...
            try
            {
                ...
                if (transaction.authorization != null)
                {
                    ...
                    {
                        ResultadoSolicitacaoResposta = "Possível troca de número de cartão e será tentada a recompra." + " Cod: " + transaction.authorization.lr;
                        TransacaoResposta.SalvarResposta(objTransacao, false, ResultadoSolicitacaoResposta, transaction.tid, transaction.authorization.arp, objTransacao.NumeroCartaoCredito.Replace(objTransacao.NumeroCartaoCredito.Substring(6, 6), "******"), null, null, null);
                    ...
                    ResultadoSolicitacaoResposta = transaction.authorization.message.Replace("��", "çã") + " Cod: " + MensagemDeFalha(transaction.authorization.lr);
                    TransacaoResposta.SalvarResposta(objTransacao, (transaction.authorization.code == 6 || transaction.authorization.code == 4 || transaction.authorization.code == 10) ? true : false, ResultadoSolicitacaoResposta, transaction.tid, transaction.authorization.arp, objTransacao.NumeroCartaoCredito.Replace(objTransacao.NumeroCartaoCredito.Substring(6, 6), "******"), null, null, null);
                }

                return new CartaoCredito.RetornoCartaoCredito()
                {
                    Aprovado = (transaction.authorization.code == 6 || transaction.authorization.code == 4 || transaction.authorization.code == 10) ? true : false,
        ...
        public static CartaoCredito.RetornoCartaoCredito EfetuarPagamentoCartaoDeCreditoPagarMe(Transacao objTransacao, out string ResultadoSolicitacaoResposta)
        {  
            //Instancia o servico
           objTransacao.PlanoAdquirido.CompleteObject();
            ResultadoSolicitacaoResposta = string.Empty;
           try
           {
                ...
                var transaction = PagarMeOperacoes.Charge(objTransacao, UrlRetorno);
                transaction.Save();
               
                if (transaction != null)
                {
                    ResultadoSolicitacaoResposta = transaction.Status.ToString() + " Cod: " + transaction.AcquirerResponseCode;
                    TransacaoResposta.SalvarResposta(objTransacao, transaction.Status == TransactionStatus.Paid, ResultadoSolicitacaoResposta, transaction.Nsu.ToString(), transaction.AuthorizationCode, objTransacao.NumeroCartaoCredito.Replace(objTransacao.NumeroCartaoCredito.Substring(6, 6), "******"), null, null, null);
                }

                return new CartaoCredito.RetornoCartaoCredito()
                {
                    Aprovado = transaction.Status == TransactionStatus.Paid,
                    DescricaoTransacao = transaction.Tid.ToString(),
                    DescricaoAutorizacao = transaction.Nsu.ToString(),
        ...
        public static string CriarTransacaoPagSeguro(Pagamento objPagamento, PlanoAdquirido objPlanoAdquirido, int idPessoaFisica, string descricaoIP, string urlRedirecionamento)
        {
            ...
            try
            {
                ...
                payment.Items.Add(new Uol.PagSeguro.Domain.Item(objPlano.IdPlano.ToString(), objPlano.DescricaoPlano, 1, objPagamento.ValorPagamento));
        ...
        public static Transacao CarregaTransacaoPorPagamento(int idf_Pagamento, SqlTransaction trans = null)
        {
            var parms = new List<SqlParameter>
                {
                    new SqlParameter { ParameterName = "@Idf_Pagamento", SqlDbType = SqlDbType.Int, Size = 4, Value = idf_Pagamento }
                };

            SqlDataReader dr;

            using (dr = DataAccessLayer.ExecuteReader(trans, CommandType.Text, SP_SELECT_IDF_PAGAMENTO, parms))
            {
                Transacao objTransacao = new Transacao();
                if (SetInstance(dr, objTransacao))
                    return objTransacao;
