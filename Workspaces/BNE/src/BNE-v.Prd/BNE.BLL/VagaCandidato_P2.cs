...
namespace BNE.BLL
{
    public partial class VagaCandidato // Tabela: BNE_Vaga_Candidato
    {
        ...
        #region [spEnviarCandidatura]
        private const string spEnviarCandidatura = @"select  pf.Nme_Pessoa, pf.Num_CPF, pf.Dta_Nascimento, vag.Des_Atribuicoes,
                                 cid.Nme_Cidade, cid.Sig_Estado, vag.Des_Funcao, vag.vlr_salario_de, vag.vlr_salario_para , pf.Eml_Pessoa  from bne.bne_vaga_candidato vc with(nolock)
...
ORDER BY V.Idf_Vaga DESC
	   ...
	   pf.num_cpf, pf.dta_nascimento, ufp.idf_usuario_Filial_perfil,
        ...
        #region [SpvagacandidatoporcurriculoJaEnviei]
        private const string SpvagacandidatoporcurriculoJaEnviei = @"DECLARE @FirstRec INT
        DECLARE @LastRec INT
        DECLARE @iSelect VARCHAR(8000)
        DECLARE @iSelectCount VARCHAR(8000)
        DECLARE @iSelectPag VARCHAR(8000)
        ...
        SELECT	ROW_NUMBER() OVER (ORDER BY VC.Dta_Cadastro DESC) AS RowID,
                ...
                V.Dta_Prazo,
                VC.Flg_Auto_Candidatura,
                lec.Dta_Processamento_Candidatura,
                lec.Dta_Analise_Cv,
                lec.Dta_Envio_CV,
				vc.dta_Visualizacao
          FROM    BNE.BNE_Vaga_Candidato VC WITH(NOLOCK)
		        ...
		        JOIN plataforma.TAB_Funcao FUN WITH(NOLOCK) ON V.Idf_Funcao = FUN.Idf_Funcao
		        JOIN BNE.TAB_Filial F WITH(NOLOCK) ON V.Idf_Filial = F.Idf_Filial
				join BNE.BNE_Log_Envio_Candidatura lec with(nolock) on lec.idf_vaga_candidato = vc.idf_vaga_candidato
           LEFT JOIN BNE.BNE_Vaga_Candidato VC2 ON VC2.Idf_Curriculo = C.Idf_Curriculo AND C.Flg_VIP = 0
				 AND VC2.Flg_Auto_Candidatura = 1 AND VC2.Idf_Vaga = VC.Idf_Vaga
        WHERE    VC.Idf_Curriculo = ' + CONVERT(VARCHAR, @Idf_Curriculo) + '
                ...
                using (var trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (podeCandidatar) //Se pode candidatar (é vip ou tem saldo)
                        {
                            ...
                            #region [Enviar E-mail da Candidatura - Track: 1]
                            EnviarCandidatura(trans);
                            #endregion
            ...
            out int? quantidadeCandidaturasGratis)
        {
            ...
            if (!CarregarPorVagaCurriculo(objVaga.IdVaga, objCurriculo.IdCurriculo, out objVagaCandidato))
            {
                objVagaCandidato = new VagaCandidato { Curriculo = objCurriculo, Vaga = objVaga, FlagInativo = false, FlagAutoCandidatura = false };
        ...
        private static void AcaoAbrirCurriculo(object obj)
        {
            try
            {
                ...
                if (acao.IdVaga.HasValue)
                {
                    ...
                    if (acao.IdVagaCandidato <= 0)
                    {//atualizar data log envio candidatura
                        VagaCandidato objVagaCandidato = new VagaCandidato();
                        if (VagaCandidato.CarregarPorVagaCurriculo(acao.IdVaga.Value, acao.IdCurriculo, out objVagaCandidato))
                        {
                            LogEnvioCandidatura.AtulizarDatasEtapasCandidatura(objVagaCandidato.IdVagaCandidato, objVagaCandidato.DataVisualizacao);
                        }
                    }
        ...
        public class AcaoAbrirCV
        {
            ...
            public int IdVagaCandidato { get; set; }
        ...
        #region CarregarVagaCandidatadaPorCurriculo
        public static DataTable CarregarVagaCandidatadaPorCurriculoJaEnviei(int idCurriculo, int paginaCorrente, int tamanhoPagina, out int totalRegistros)
        {
            var parms = new List<SqlParameter>
            {
                new SqlParameter("@Idf_Curriculo", SqlDbType.Int, 4),
            ...
            try
            {
                using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, SpvagacandidatoporcurriculoJaEnviei, parms))
                {
                    if (dr.Read())
                        totalRegistros = Convert.ToInt32(dr[0]);
        ...
        #region [Setar Etapas da candidatura]
        public void SetarEtapasCandidatura(SqlTransaction trans = null)
        {
            var random = new Random();
            int minimo = Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.MinutosRandomEtapaCandidaturaMinimo));
            int maximo = Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.MinutosRandomEtapaCandidaturaMaximo));

            LogEnvioCandidatura obj = new LogEnvioCandidatura();
            obj.Curriculo = this.Curriculo;
            obj.Vaga = this.Vaga;
            obj.VagaCandidato = this;

            //*********** USAR NA CONSOLE PARA ATAULIZAR A BASE****************

            //if (this.DataVisualizacao.HasValue)
            //{
            //    obj.DataEnvioCV = this.DataVisualizacao.Value;
            //    int tentativa = 0;
            //    do
            //    {
            //        obj.DataProcessamentoCandidatura = this.DataCadastro.AddMinutes(random.Next(minimo, maximo));
            //        obj.DataAnaliseCV = obj.DataProcessamentoCandidatura.Value.AddMinutes(random.Next(minimo, maximo));
            //        tentativa++;
            //        Console.WriteLine($"Tentavia set as datas: {tentativa}");
            //    }
            //    while (obj.DataProcessamentoCandidatura > obj.DataAnaliseCV && obj.DataAnaliseCV > this.DataVisualizacao.Value);
            ...
            obj.DataProcessamentoCandidatura = this.DataCadastro.AddMinutes(random.Next(minimo, maximo));
            obj.DataAnaliseCV = obj.DataProcessamentoCandidatura.Value.AddMinutes(random.Next(minimo, maximo));
            //obj.DataEnvioCV = obj.DataAnaliseCV.Value.AddMinutes(random.Next(minimo, maximo));
            // }
            if (trans != null)
                obj.Save(trans);
            else
        ...
        #region [Enviar Candidatura]
        /// <summary>
        /// Enviar Email de confirmação de candidatura.
        /// </summary>
        private void EnviarCandidatura(SqlTransaction trans)
        {
            try
            {
                var carta = CartaEmail.LoadObject((int)Enumeradores.CartaEmail.CandEtapaProcessamento);
                var emailRemetente = Parametro.RecuperaValorParametro(Enumeradores.Parametro.ContaPadraoEnvioEmail);

                List<SqlParameter> parametros = new List<SqlParameter>()
            {
                new SqlParameter{ParameterName = "@Idf_Vaga_Candidato", SqlDbType = SqlDbType.Int, Value = this._idVagaCandidato},

            };
                using (var dr = DataAccessLayer.ExecuteReader(trans, CommandType.Text, spEnviarCandidatura, parametros))
                {
                    if (dr.Read())
                    {
                        ...
                        decimal? salarioDe = null;
                        decimal? salarioPara = null;

                        if(dr["vlr_salario_de"] != DBNull.Value)
                            salarioDe = Convert.ToDecimal(dr["vlr_salario_de"]);
                        if (dr["vlr_salario_para"] != DBNull.Value)
                            salarioPara = Convert.ToDecimal(dr["vlr_salario_para"]);


                        carta.ValorCartaEmail = carta.ValorCartaEmail.Replace("{LinkJaEnviei}", link).Replace("{Funcao}", dr["Des_Funcao"].ToString())
                            ...
                            .Replace("{Salario}", Helper.RetornarDesricaoSalario(salarioDe, salarioPara))
                            .Replace("{Nome}", Helper.RetornarPrimeiroNome(dr["Nme_Pessoa"].ToString()));

                        carta.DescricaoAssunto = carta.DescricaoAssunto.Replace("{Funcao}", dr["Des_Funcao"].ToString());

                        if (!string.IsNullOrEmpty(dr["Eml_Pessoa"].ToString()))
                        {
                            EmailSenderFactory.Create(TipoEnviadorEmail.Fila)
                                         .Enviar(carta.DescricaoAssunto, carta.ValorCartaEmail, BLL.Enumeradores.CartaEmail.CandEtapaProcessamento, emailRemetente, dr["Eml_Pessoa"].ToString());
            ...
            {
                EL.GerenciadorException.GravarExcecao(ex, "erro ao enviar e-mail da candidatura");
            }
