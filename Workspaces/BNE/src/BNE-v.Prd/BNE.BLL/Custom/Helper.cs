...
namespace BNE.BLL.Custom
{
    public static class Helper
    {
        ...
        {
            valorSalarioDe = valorSalarioDe.HasValue ? valorSalarioDe : 0;
            valorSalarioAte = valorSalarioAte.HasValue ? valorSalarioAte : 0;

            if (valorSalarioDe.Value.Equals(0) && valorSalarioAte.Value.Equals(0))
                return "Salário a combinar";

            if (valorSalarioDe.Value.Equals(valorSalarioAte.Value))
                return string.Format("Salário de R$ {0}", ((decimal)valorSalarioDe).ToString("N2"));

            if (valorSalarioDe.Value.Equals(0) || valorSalarioAte.Value.Equals(0))
            {
                return string.Format("Salário de R$ {0}", (valorSalarioDe.Value > 0 ? ((decimal)valorSalarioDe).ToString("N2") : ((decimal)valorSalarioAte).ToString("N2")));
            }
        ...
        public static bool ValidarCartaoCredito(string numeroCartao, out Enumeradores.Operadora? operadoraCartao)
        {
            ...
            if (isValid)
            {
                ...
                Regex prefixRegExpDinners = new Regex("^(30[6-9]|36|38)");
                Regex prefixHipercard = new Regex("^38[0-9]|^60[0-9]");
                Regex prefixElo = new Regex("^(63|438935|504175|451416|5067|4576|4011|506699|6504|6550)");
                Regex prefixAura = new Regex("^(50)");
                Regex prefixDiscover = new Regex("^(6011|622(12[6-9]|1[3-9][0-9]|[2-8][0-9]{2}|9[0-1][0-9]|92[0-5]|64[4-9])|65)");
                ...
                else if (prefixRegExpVisa.IsMatch(cardNumbersOnly))
                {
                    operadoraCartao = Enumeradores.Operadora.Visa;
                }
                else if (prefixRegExpAmex.IsMatch(cardNumbersOnly))
                {
                    operadoraCartao = Enumeradores.Operadora.Amex;
                }
                else if (prefixRegExpDinners.IsMatch(cardNumbersOnly))
                {
                    operadoraCartao = Enumeradores.Operadora.Dinners;
                }
                else if (prefixHipercard.IsMatch(cardNumbersOnly))
                {
                    operadoraCartao = Enumeradores.Operadora.Hipercard;
                }
                else if (prefixElo.IsMatch(cardNumbersOnly))
                {
                    operadoraCartao = Enumeradores.Operadora.Elo;
                }
                else if (prefixAura.IsMatch(cardNumbersOnly))
                {
                ...
                switch (operadoraCartao)
                {
                    ...
                    case BNE.BLL.Enumeradores.Operadora.Amex:
                        ...
                        break;
                    case BNE.BLL.Enumeradores.Operadora.Hipercard:
                        isValid = (cardNumberLength == 16);
                        break;
        ...
        public static string CalcularTempoEmprego(string DtaAdmissao, string DtaDemissao)
        {
            ...
            try
            {
                ...
                DateTime dtaAdmissao = (!string.IsNullOrEmpty(DtaAdmissao) ? Convert.ToDateTime(DtaAdmissao, System.Globalization.CultureInfo.GetCultureInfo("hi-IN").DateTimeFormat) : DateTime.Now);
                DateTime dtaDemissao = (!string.IsNullOrEmpty(DtaDemissao) ? Convert.ToDateTime(DtaDemissao, System.Globalization.CultureInfo.GetCultureInfo("hi-IN").DateTimeFormat) : DateTime.Now);
                ...
                retorno = string.Format("{0}{1}{2}", strAno, strSeparadorMes, strMes);

            }
            catch (Exception ex)
            {
                EL.GerenciadorException.GravarExcecao(ex,
                    "CalcularTempoEmprego-> DtaAdmissao:" + DtaAdmissao + "/  DtaDemissao:" + DtaDemissao);
            }
        ...
        public static string EncurtarUrl(string url)
        {
            ...
            try
            {
                using (var client = new WebClient())
                {
                    ...
                    dynamic urlDeserialize = JsonConvert.DeserializeObject(result);

                    if (urlDeserialize.id.Value != null)
                        urlShort = urlDeserialize.id.Value;
                    else
                        EL.GerenciadorException.GravarExcecao(new InvalidOperationException(result.ToString()));
                }
        ...
        public static decimal? DetectarSalario(string strVlrSalario)
        {
            try
            {
                decimal Salario; 
                strVlrSalario = Regex.Replace(strVlrSalario, @"(R(\s|)\$|r(\s|)\$)|\(|\)|\+|\-", "");

                //Regex para remover caracteres invalidos no inicio
                strVlrSalario = Regex.Replace(strVlrSalario, @"^(\.{1,10}|\,{1,10})(\s{1,10}|)(\.{1,10}|\,{1,10})", "");

                //Regex para remover caracteres invalidos no fim
                strVlrSalario = Regex.Replace(strVlrSalario, @"(\.{1,10}|\,{1,10})(\s{1,10}|)(|\.{1,10}|\,{1,10}).?$", "");

                //Regex para remover possiveis caracaters não numéricos
                strVlrSalario = Regex.Replace(strVlrSalario, @"([a-z]|[áàâãéèêíìîóòõô]).*?", "", RegexOptions.IgnoreCase);

                strVlrSalario = strVlrSalario.Trim();

                CultureInfo providere = new CultureInfo("pt-BR");

                if (strVlrSalario != null && strVlrSalario != "")
                {
