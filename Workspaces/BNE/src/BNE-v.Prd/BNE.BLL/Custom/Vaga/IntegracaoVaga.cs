    ...
    public class IntegracaoVaga
    {
        ...
        protected bool ProcessaVagaExistente()
        {
            try
            {
                ...
                var tipoVinculoSalvos = VagaTipoVinculo.ListarTipoVinculoPorVaga(_VagaIntegracaoBD.Vaga.IdVaga);
                if (_VagaIntegracao.TiposVinculo != null)
                {
        ...
        public Funcao TratarFuncaoVaga(string desFuncao)
        {
            try
            {
                ...
                //Tratamento Função Vendedor
                var regex = new Regex(@"vendedor[ae]\(?s?\)?", RegexOptions.IgnoreCase);
                if (regex.IsMatch(desFuncao))
                    desFuncao = "Vendedor";
