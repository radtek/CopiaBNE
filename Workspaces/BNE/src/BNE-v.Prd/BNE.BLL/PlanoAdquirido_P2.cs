...
namespace BNE.BLL
{
    public partial class PlanoAdquirido // Tabela: BNE_Plano_Adquirido
    {
        ...
        private const string SpVerificaPlanoQuantidade = @"

SELECT DISTINCT pq.Idf_Plano_Quantidade  FROM BNE.BNE_Plano_Adquirido pa INNER JOIN BNE.BNE_Plano_Parcela pp ON pp.Idf_Plano_Adquirido = pa.Idf_Plano_Adquirido
INNER JOIN BNE.BNE_Pagamento pg ON pg.Idf_Plano_Parcela = pp.Idf_Plano_Parcela
INNER JOIN BNE.BNE_Plano_Quantidade pq ON pq.Idf_Plano_Adquirido = pa.Idf_Plano_Adquirido
INNER JOIN BNE.BNE_Plano p ON p.Idf_Plano = pa.Idf_Plano
WHERE 1=1
AND pa.Idf_Plano_Situacao = 1
AND pq.Flg_Inativo = 1";


        private const string SpPlanosPJCartaoNaoRecorrente = @"
SELECT DISTINCT pa.Idf_Filial, PA.Dta_Fim_Plano,
pa.Idf_Plano_Situacao,
	PA.Idf_Plano_Adquirido, 
	pa.Qtd_Parcela AS 'QtdeParcelasPA', 
	p.Qtd_Parcela AS 'QtdeParcelasP', 
	transacoesCobradas.totalCobrancas AS 'QtdeParcelasPagas' 
FROM BNE.BNE_Plano_Adquirido PA WITH(NOLOCK)
	...
	INNER JOIN BNE.BNE_Pagamento PG WITH(NOLOCK) ON PG.Idf_Plano_Parcela = PP.Idf_Plano_Parcela
	OUTER APPLY
		(
			SELECT COUNT(1) totalCobrancas
			FROM BNE.BNE_Plano_Parcela p2 WITH (NOLOCK)
				INNER JOIN BNE.BNE_Pagamento pg2 WITH (NOLOCK)
...
WHERE 
	PA.Dta_Fim_Plano < GETDATE() - 1 
	AND PA.Idf_Plano_Situacao = 1
	AND PA.Flg_Recorrente = 0
	AND PG.Idf_Tipo_Pagamento = 1
	AND pa.Idf_Filial IS NOT null
";


        private const string SpPlanosPJCartao = @"
SELECT DISTINCT pa.Idf_Filial, PA.Dta_Fim_Plano,
pa.Idf_Plano_Situacao,
	PA.Idf_Plano_Adquirido, 
	pa.Qtd_Parcela AS 'QtdeParcelasPA', 
...
WHERE 
	PA.Dta_Fim_Plano < GETDATE() - 1 
	AND PA.Idf_Plano_Situacao = 1
	AND PA.Flg_Recorrente = 1
	AND PG.Idf_Tipo_Pagamento = 1
	AND pa.Idf_Filial IS NOT null
";

        private const string SPRetornaPlanosComDataCancelamento =
            @"
SELECT Idf_Plano_Adquirido, Dta_Fim_Plano
FROM BNE.BNE_Plano_Adquirido WITH(NOLOCK)
WHERE Idf_Plano_Situacao = 1
      AND Dta_Cancelamento IS NOT NULL
      AND Dta_Fim_Plano < GETDATE() - 1;";


        private const string SpAtualizarPlanoPJBoletosNaoRecorrente = @"
SELECT DISTINCT 
	pa.Dta_Fim_Plano, 
	PA.Idf_Plano_Adquirido, 
	pa.Qtd_Parcela AS 'QtdeParcelasPA', 
...
WHERE 
	PA.Dta_Fim_Plano < GETDATE()  
	AND PA.Idf_Plano_Situacao = 1
	AND PG.Idf_Tipo_Pagamento = 2
";

        private const string SpRetonaPlanoVencidoComBoleto1 = @"
...
FROM BNE.BNE_Plano_Adquirido pa WITH (NOLOCK)
    INNER JOIN BNE.BNE_Plano_Parcela pp WITH (NOLOCK)
        ON pp.Idf_Plano_Adquirido = pa.Idf_Plano_Adquirido
    INNER JOIN BNE.BNE_Pagamento pg WITH (NOLOCK)
        ON pg.Idf_Plano_Parcela = pp.Idf_Plano_Parcela
    INNER JOIN BNE.BNE_Plano p WITH (NOLOCK)
...
WHERE pa.Idf_Filial IS NOT NULL
	  ...
	  (	
		  (
			   pg.Idf_Tipo_Pagamento = 1 AND 	
              pa.Flg_Recorrente = 1
              AND (CONVERT(VARCHAR, pg.Dta_Vencimento + 14, 112) = CONVERT(VARCHAR, GETDATE(), 112) OR CONVERT(VARCHAR, PA.Dta_Fim_Plano + 14, 112) = CONVERT(VARCHAR, GETDATE(), 112))
          )
          OR
          (
				pg.Idf_Tipo_Pagamento = 1 ANd
              pa.Flg_Recorrente = 0
              AND (CONVERT(VARCHAR, pg.Dta_Vencimento, 112) = CONVERT(VARCHAR, GETDATE(), 112) OR CONVERT(VARCHAR, PA.Dta_Fim_Plano, 112) = CONVERT(VARCHAR, GETDATE(), 112))
          )
      )
...
SELECT pa.Idf_Plano_Adquirido ,
        ...
        #region []
        private const string SpRetonaPlanoVencidoFinalziadoComBoleto = @"
 SELECT 
	pg.Idf_Tipo_Pagamento,
    transacoesCobradas.totalCobrancas,
    pg.Idf_Pagamento_Situacao,
...
FROM BNE.BNE_Plano_Adquirido pa WITH (NOLOCK)
    ...
    INNER JOIN BNE.BNE_Plano p WITH (NOLOCK)
        ON p.Idf_Plano = pa.Idf_Plano
		join bne.tab_filial f with(nolock) on f.idf_Filial
		= pa.idf_Filial
		join bne.bne_tipo_pagamento tp with(nolock) on tp.idf_tipo_pagamento 
		= pg.idf_tipo_pagamento
    OUTER APPLY
(
    SELECT COUNT(1) totalCobrancas
...
WHERE pa.Idf_Filial IS NOT NULL
      and pg.Idf_Tipo_Pagamento = 2 --boleto
      AND
      ( 
	  ...
	  ((
          ...
          (
              pa.Flg_Recorrente = 0
              AND (CONVERT(VARCHAR, pg.Dta_Vencimento + 3 , 112) = CONVERT(VARCHAR, getdate(), 112) OR CONVERT(VARCHAR, PA.Dta_Fim_Plano, 112) = CONVERT(VARCHAR, getdate(), 112))
          ))
	  )
      ...
      AND pa.Idf_Filial IS NOT NULL
	  and pg.idf_pagamento_situacao <> 3 -- cancelado
	group by pg.Idf_Tipo_Pagamento,
    transacoesCobradas.totalCobrancas,
    pg.Idf_Pagamento_Situacao,
    pa.Idf_Plano_Adquirido,
	tp.des_tipo_pagamaneto,
...
";
        ...
        public static void AtualizarPlanosPJCartao()
        {
            try
            {
                ...
                using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, SpPlanosPJCartaoNaoRecorrente, null))
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        planoAdquiridoCancelameto.QuantidadeParcelasPlanoAdquirido = dr["QtdeParcelasPA"] != DBNull.Value ? Convert.ToInt32(dr["QtdeParcelasPA"]) : 0;
                        planoAdquiridoCancelameto.QuantidadeParcelasPlano = dr["QtdeParcelasP"] != DBNull.Value ? Convert.ToInt32(dr["QtdeParcelasP"]) : 0;
                        planoAdquiridoCancelameto.TotalParcelasPagas = dr["QtdeParcelasPagas"] != DBNull.Value ? Convert.ToInt32(dr["QtdeParcelasPagas"]) : 0;
                        ...
                        if (planoAdquiridoCancelameto.TotalParcelasPagas > 0)
                        {
                            planoAdquirido.PlanoSituacao = planoAdquiridoCancelameto.TotalParcelasPagas >= totalParcela ? new PlanoSituacao((int)Enumeradores.PlanoSituacao.Encerrado) : new PlanoSituacao((int)Enumeradores.PlanoSituacao.Bloqueado);
                        }
                        else
                        {
        ...
        public static void AtualizarPlanosPJCartaoRecorrente() {
            try
            {
                List<PlanoAdquirido> listaPlanosAdquirido = new List<PlanoAdquirido>();
                ...
                var planoAdquirido = new PlanoAdquirido();
                using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, SpPlanosPJCartao, null))
                {
                    List<PlanoAdquirido> planosAdquirido = new List<PlanoAdquirido>();
                    while (dr.Read())
                    {
        ...
        public static void VerificaPlanoQuantidade()
        {
            List<PlanoQuantidade> listaPlanosQuantidade = new List<PlanoQuantidade>();
            var p_quantidade = new PlanoQuantidade();

            //var planoQuantidade = new PlanoQuantidade();
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, SpVerificaPlanoQuantidade, null))
            {
                
                while (dr.Read())
                {
                    //var planoQuantidadeInativados = new PlanoAdiquiridoVencidos();

                    var plano_quantidade = PlanoQuantidade.LoadObject(dr["Idf_Plano_Quantidade"] != DBNull.Value ? Convert.ToInt32(dr["Idf_Plano_Quantidade"]) : 0);

                    //planoAdquirido = PlanoAdquirido.LoadObject(planoAdquiridoCancelameto.IdPlanoAdquirido);

                    plano_quantidade.FlagInativo = false;

                    listaPlanosQuantidade.Add(plano_quantidade);
                }
                foreach (var plano in listaPlanosQuantidade)
                {
                    plano.Save();
        ...
        public static void AtualizarPlanosComDataEncerramento()
        {
            try
            {
                List<PlanoAdquirido> listaPlanosAdquirido = new List<PlanoAdquirido>();
                ...
                var planoAdquirido = new PlanoAdquirido();
                using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, SPRetornaPlanosComDataCancelamento, null))
                {
                    List<PlanoAdquirido> planosAdquirido = new List<PlanoAdquirido>();
                    while (dr.Read())
                    {
        ...
        public static void AtualizarPlanoPJBoletosNaoRecorrente()
        {
            try
            {

                /*Finalizar Plano em Aberto de Boletos não recorrentes */
                ...
                var planoAdquirido = new PlanoAdquirido();
                using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, SpAtualizarPlanoPJBoletosNaoRecorrente, null))
                {
                    List<PlanoAdquirido> planosAdquirido = new List<PlanoAdquirido>();
                    while (dr.Read())
                    {
                        ...
                        if (planoAdquiridoCancelameto.TotalParcelasPagas > 0)
                        {
                            ...
                            if (planoAdquiridoCancelameto.TotalParcelasPagas >= totalParcela)
                            {
                                planoAdquirido.PlanoSituacao = new PlanoSituacao((int)Enumeradores.PlanoSituacao.Encerrado);
                                planosAdquirido.Add(planoAdquirido);
                            }
                            ...
                            {
                                var dataEncerramento = planoAdquiridoCancelameto.DataFimPlano.AddDays(3);

                                if (DateTime.Now >= dataEncerramento) {
                                    planoAdquirido.PlanoSituacao = new PlanoSituacao((int)Enumeradores.PlanoSituacao.Bloqueado);
                                    planosAdquirido.Add(planoAdquirido);
                                }   
                            }
                        }
        ...
        //public static void AtualizarPlanoPJ1()
        //{
        //    try
        //    {
        //        List<PlanoAdquirido> listaPlanosAdquirido = new List<PlanoAdquirido>();
        ...
        //        {
        //            new SqlParameter{ParameterName="@Dta_Vencimento", SqlDbType = SqlDbType.DateTime, Value = (DateTime.Now.AddDays(-Convert.ToInt32(Parametro.RecuperaValorParametro(Enumeradores.Parametro.DiaBloquearPagamentoAbertoVencido)))) }
        //        };
        //        var planoAdquirido = new PlanoAdquirido();
        //        using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, SpAtualizarPlanoPJBoletosNaoRecorrente, parametros))
        //        {
        //            List<PlanoAdquirido> planosAdquirido = new List<PlanoAdquirido>();
        //            while (dr.Read())
        //            {
