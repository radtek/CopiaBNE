...
namespace BNE.BLL
{
    public partial class Escolaridade // Tabela: TAB_Escolaridade
    {
        ...
        SET @sql = @sql + ' 
                    AND F.Flg_Inativo = 0
                    AND F.Idf_Pessoa_Fisica = ' + CONVERT(VARCHAR, @Idf_Pessoa_Fisica) + '
                    AND E.Idf_Escolaridade IN ( 4, 5, 6, 7, 10, 11, 12, 13 ) -- EFI EFC EMI EMC ETI ETC ESI ESC
        ...
        /// </summary>
        /// <returns></returns>
        public static IDataReader ListarDR()
        {
            return DataAccessLayer.ExecuteReader(CommandType.Text, Selectnivel, null);
        }
        #endregion
