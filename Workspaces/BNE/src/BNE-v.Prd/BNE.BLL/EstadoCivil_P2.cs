...
namespace BNE.BLL
{
    public partial class EstadoCivil // Tabela: TAB_Estado_Civil
    {
        ...
        private const string SpPorDescricao = @"
        SELECT * 
        FROM plataforma.TAB_Estado_Civil 
        WITH(NOLOCK)
        WHERE Des_Estado_Civil = @Des_Estado_Civil ";
        #endregion
        ...
        /// </summary>
        /// <param name="descricao">Descrição do estado civil.</param>
        /// <returns>Instância de EstadoCivil.</returns>
        /// <remarks>Gieyson Stelmak</remarks>
        public static EstadoCivil LoadObject(string descricao)
        {
            if (!string.IsNullOrEmpty(descricao))
            {
                ...
                if (HabilitaCache)
                {
                    var estadoCivil = EstadosCivis.FirstOrDefault(s => s.Descricao == descricao);

                    if (estadoCivil != null)
                        return new EstadoCivil
                        {
                            IdEstadoCivil = estadoCivil.Identificador,
                            DescricaoEstadoCivil = estadoCivil.Descricao
                        };
                ...
                var retorno = new EstadoCivil();

                var parms = new List<SqlParameter>();
                parms.Add(new SqlParameter
                {
                    ParameterName = "@Des_Estado_Civil",
                    SqlDbType = SqlDbType.VarChar,
                    Size = 80,
                    Value = descricao
                });

                using (var dr = DataAccessLayer.ExecuteReader(CommandType.Text, SpPorDescricao, parms))
                    if (!SetInstance(dr, retorno))
                        return null;
