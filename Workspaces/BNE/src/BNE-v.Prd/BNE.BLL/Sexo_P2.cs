...
namespace BNE.BLL
{
    public partial class Sexo : ICloneable // Tabela: TAB_Sexo
    {
        ...
        private static List<SexoCache> Sexos
        {
            get
            {
                return Cache.GetItem(ObjectKey, ListarSexosCACHE);
            }
        }
        #endregion
        ...
        private static List<SexoCache> ListarSexosCACHE()
        {
            ...
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, spselecttodossexos, null))
            {
                while (dr.Read())
                {
                    lista.Add(new SexoCache { Identificador = Convert.ToInt32(dr["Idf_Sexo"]), Descricao = dr["Des_Sexo"].ToString(), SiglaSexo = Convert.ToChar(dr["Sig_Sexo"]) });
                }
                if (!dr.IsClosed)
                    dr.Close();
            }
