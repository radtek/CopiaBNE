    ...
    public partial class PlanoParcela // Tabela: BNE_Plano_Parcela
    {
        ...
        SET @iSelect = '
            SELECT	
                    ...
                    pg.Vlr_juros,
                    (CASE WHEN PP.Eml_Envio_Boleto IS NULL OR PP.Eml_Envio_Boleto = '''' THEN PD.Eml_Envio_Boleto ELSE PP.Eml_Envio_Boleto END) as Eml_Envio_Boleto,    
            ...
            FROM    BNE.BNE_Plano PL WITH(NOLOCK)
                    ...
                    LEFT JOIN BNE.BNE_Plano_Adquirido_Detalhes PD WITH(NOLOCK) ON PP.Idf_Plano_Adquirido = PD.Idf_Plano_Adquirido
                    LEFT JOIN BNE.BNE_Pagamento_Situacao PS WITH(NOLOCK) ON PG.Idf_Pagamento_Situacao = PS.Idf_Pagamento_Situacao
                    LEFT JOIN BNE.BNE_Tipo_Pagamento TP WITH(NOLOCK) ON TP.Idf_Tipo_Pagamento = PG.Idf_Tipo_Pagamento
        ...
        private const string SPTotalParcelasPagas = @" 
            SELECT COUNT(pp.Idf_Plano_Parcela)
             FROM   BNE.BNE_Plano_Parcela pp WITH ( NOLOCK )
                    JOIN BNE.BNE_Pagamento pg WITH ( NOLOCK ) ON pp.Idf_Plano_Parcela = pg.Idf_Plano_Parcela
             WHERE  pp.Idf_Plano_Adquirido = @Idf_Plano_Adquirido
                    AND pp.Idf_Plano_Parcela_Situacao = 2;";
        #endregion
        ...
        AND pp.Idf_Plano_Parcela_Situacao <> 3";
        #endregion
...
WHERE   1 = 1
        ...
        public static bool EmitirNF(Pagamento objPagamento, UsuarioFilialPerfil objUsuarioLogado)
        {
            ...
            if (objPlanoAdquirido.ParaPessoaJuridica())
            {
                if (Filial.RecuperarInformacoesIntegracaoFinanceiro(objPagamento.IdPagamento, out dataPagamento, out valorPagamento, out desIdentificador, out dataInicioPlano, out dataFimPLano, out numCNPJ, out razaoSocial, out cep, out rua, out numEndereco, out complemento, out bairro, out cidade, out uf, out nomeFantasia, out idfCnaePrincipal, out emailContato, out ddd, out telefone, out nomeContato, out numBanco, out filialGestora))
                {
                    ...
                    var tipoPagamento = (Enumeradores.TipoPagamento)objPagamento.TipoPagamento.IdTipoPagamento;
                    BoletoBancario objBoletoBancario;
                   if(BoletoBancario.CarregarPeloNossoNumero(objPagamento.DescricaoIdentificador, out objBoletoBancario))
                    {//boleto do pagarMe tem que emitir a nota para a conta do cartão de  credito
                        tipoPagamento = objBoletoBancario.UrlBoleto != null ? Enumeradores.TipoPagamento.CartaoCredito : (Enumeradores.TipoPagamento)objPagamento.TipoPagamento.IdTipoPagamento;
                    }
                    ContaBancaria objContaBancaria = ContaBancaria.RecuperarContaBancariaDeTipoPagamento(tipoPagamento, (Enumeradores.Banco)numBanco);
            ...
            else
            {
                if (PessoaFisica.RecuperarInformacoesIntegracaoFinanceiro(objPagamento.IdPagamento, out dataPagamento, out valorPagamento, out desIdentificador, out dataInicioPlano, out dataFimPLano, out numCPF, out nomeContato, out cep, out rua, out numEndereco, out complemento, out bairro, out cidade, out uf, out emailContato, out ddd, out telefone, out numBanco, out filialGestora))
                {
                    ...
                    var idTipoContrato = objPlanoAdquirido.Plano.TipoContrato.IdTipoContrato;
                    ...
                    try
                    {
                      ...
                      //baixaNota = objWsIntegracao.IntegrarFaturamentoFinanceiro(wsIntegracaoWFat.SistemaOrigem.BNE,
                      //      dataPagamento == default(DateTime) ? new DateTime?() : new DateTime?(dataPagamento),
                      //      valorPagamento - objPagamento.ValorJuros, tipoTransacaoWfat, descricaoIdentificador,
        ...
        public static bool SetInstance_NonDispose(IDataReader dr, PlanoParcela objPlanoParcela)
        {
            ...
            if (dr["Dta_Emissao_Nota_Antecipada"] != DBNull.Value)
                objPlanoParcela.DataEmissaoNotaAntecipada = Convert.ToDateTime(dr["Dta_Emissao_Nota_Antecipada"]);
        ...
        public static PlanoParcela CriarParcelaRecorrenciaPeloPlanoAdquirido(BLL.PlanoAdquirido objPlanoAdquirido, PlanoDesconto objPlanoDesconto, SqlTransaction trans = null)
        {
            ...
            {
                PlanoAdquirido = objPlanoAdquirido,
                ValorParcela = objPlanoAdquirido.Plano.PlanoTipo.IdPlanoTipo == (int)BLL.Enumeradores.PlanoTipo.PessoaJuridica ? objPlanoAdquirido.ValorBase : (objPlanoDesconto != null ? objPlanoAdquirido.Plano.ValorBase - objPlanoDesconto.ValorDesconto.Value : objPlanoAdquirido.Plano.ValorBase ),
                //Se for cortesia gerar parcela com a situação pago
        ...
        public static int QuantidadeParcelasPagas(PlanoAdquirido objPlanoAdquirido)
        {
            var parms = new List<SqlParameter>
                {
                    new SqlParameter { ParameterName = "@Idf_Plano_Adquirido", SqlDbType = SqlDbType.Int, Value = objPlanoAdquirido.IdPlanoAdquirido }
            ...
            return Convert.ToInt32(DataAccessLayer.ExecuteScalar(CommandType.Text, SPTotalParcelasPagas, parms));
        }

        public static PlanoParcela CriarParcelaRecorrenciaPeloPlanoAdquiridoSegundoMesGratis(BLL.PlanoAdquirido objPlanoAdquirido, SqlTransaction trans = null)
        {
            PlanoParcela objPlanoParcelaUltima = PlanoParcela.CarregarUltimaParcelaPorSituacaoDaParcela(objPlanoAdquirido.IdPlanoAdquirido, new BLL.PagamentoSituacao((int)BNE.BLL.Enumeradores.PagamentoSituacao.Pago), trans);
            ...
            var objPlanoParcela = new PlanoParcela
            {
                ...
                ValorParcela = 0,
                //Se for cortesia gerar parcela com a situação pago
                PlanoParcelaSituacao = objPlanoAdquirido.ValorBase > 0 ? new BLL.PlanoParcelaSituacao((int)BNE.BLL.Enumeradores.PlanoParcelaSituacao.EmAberto) : new BNE.BLL.PlanoParcelaSituacao((int)BNE.BLL.Enumeradores.PlanoParcelaSituacao.Pago),
                NumeroDesconto = objPlanoParcelaUltima.NumeroDesconto,
                FlagInativo = false,
                QuantidadeSMSTotal = objPlanoParcelaUltima.QuantidadeSMSTotal,
                DataEmissaoNotaAntecipada = objPlanoParcelaUltima.DataEmissaoNotaAntecipada,
        ...
        public static bool ValidacoesSegundaGratis(PlanoAdquirido objPlanoAdquirido)
        {
          
            int quantidadeParcelasPagas = QuantidadeParcelasPagas(objPlanoAdquirido);
            return quantidadeParcelasPagas == 1;
