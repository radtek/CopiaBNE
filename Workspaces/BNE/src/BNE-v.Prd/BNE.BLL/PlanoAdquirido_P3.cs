...
WHERE
        ...
        public void SalvarPlanoAdquiridoParcelaPagamento(UsuarioFilialPerfil objUsuarioFilialPerfil, Filial objFilial, Plano objPlano,
            PlanoAdquiridoDetalhes objPlanoAdquiridoDetalhes, DateTime? dataEnvioBoleto, DateTime? dataVencimentoBoleto,
            DateTime? dataEmissaoNFAntecipada, Boolean gerarParcela)
        {
            using (var conn = new SqlConnection(DataAccessLayer.CONN_STRING))
            {
                conn.Open();
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        else //Ajustando parcelas a vencer
                        {
                            ...
                            foreach (PlanoParcela parcelas in todasParcelas)
                            {
                                if (parcelas.PlanoParcelaSituacao.IdPlanoParcelaSituacao == (int)Enumeradores.PlanoParcelaSituacao.EmAberto)
                                {
                                    if (parcelas.ValorParcela != this.ValorBase)
                                        ...
                                        parcelas.DataEmissaoNotaAntecipada = dataEmissaoNFAntecipada;
                                        ...
                                        parcelas.DataEmissaoNotaAntecipada = dataEmissaoNFAntecipada.Value;
                            ...
                            foreach (var objPlanoParcela in parcelasEmAberto)
                            {
                                ...
                                if (objPagamentoAberto != null)
                                {
                                    ...
                                    objPagamento.DescricaoDescricao = objPagamentoAberto.DescricaoDescricao;
                        ...
                        {
                            BoletoBancario.CriarBoletosPagarMe(listaPagamentos);
                        }
                            ...
                            if (pagamentos.Count > 0)
                            {
                                ...
                                if (dataEnvioBoleto.HasValue)
                                {
                                    if (objPlanoAdquiridoDetalhes != null && !string.IsNullOrWhiteSpace(objPlanoAdquiridoDetalhes.EmailEnvioBoleto))
                                    {
                                        ...
                                        byte[] anexos = BoletoBancario.CriarAnexoParaEnviarPorEmail(BoletoBancario.GerarLayoutBoletoHTMLPagarMe(BoletoBancario.CriarBoletosPagarMe(pagamentos)), Enumeradores.FormatoBoleto.PDF);
        ...
        public bool Liberar(bool ajustarDatas, SqlTransaction trans = null)
        {
            ...
            {
                CompleteObject();
                if (string.IsNullOrEmpty(Plano.DescricaoPlano))
                    Plano.CompleteObject();
            }
            else
            {
                    ...
                    try
                    {
                        ...
                        foreach (var objPlanoParcela in PlanoParcela.ListaParcelasPorPlanoAdquirido(this, trans).Where(p => p.PlanoParcelaSituacao.IdPlanoParcelaSituacao == (int)Enumeradores.PlanoParcelaSituacao.EmAberto).OrderBy(o => o.IdPlanoParcela))
                        {
                            ...
                            if (objPagamentoAberto != null)
                            {
                                ...
                                var dataVencimentoBoletoCompare = dataVencimentoBoleto.AddMonths(count).AddDays(Feriado.RetornarDiaUtilVencimento(dataVencimentoBoleto.AddMonths(count).Date));
                                if (!objPagamentoAberto.DataVencimento.Value.Date.Equals(dataVencimentoBoletoCompare))
                                {//criar novo boleto se a data de vencimento for alterada.
                                    objPagamentoAberto.DataVencimento = dataVencimentoBoleto.AddMonths(count);
                                    PagarMeOperacoes.GerarBoleto(objPagamentoAberto, trans, true);
                                }
                                else
                                {
                                    objPagamentoAberto.DataVencimento = dataVencimentoBoleto.AddMonths(count);
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            //Ativar o plano quantidade pos quando é bloqueado é inativado.
                            PlanoQuantidade objPlanoQuantidade;

                            if (PlanoQuantidade.CarregarPorPlanoAdquirido(this, out objPlanoQuantidade, trans))
                            {
                                objPlanoQuantidade.FlagInativo = false;
                                objPlanoQuantidade.Save(trans);
                            }
