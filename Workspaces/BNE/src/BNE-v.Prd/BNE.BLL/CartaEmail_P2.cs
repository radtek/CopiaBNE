...
namespace BNE.BLL
{
    public partial class CartaEmail // Tabela: BNE_Carta_Email
    {
        ...
        private static List<CartaEmail> Cartas => Cache.GetItem("BNE_Carta_Email", ListarCartas);
        ...
        private static Dictionary<Enumeradores.CartaEmail, DTO.CartaEmail> ListarCartas(List<Enumeradores.CartaEmail> idsCartas)
        {
            ...
            using (var dr = DataAccessLayer.ExecuteReader(CommandType.Text, query, parms))
            {
                while (dr.Read())
                {
        ...
        private static List<CartaEmail> ListarCartas()
        {
            var retorno = new List<CartaEmail>();
            string query = "SELECT * FROM BNE_Carta_Email WITH(NOLOCK)";
            using (var dr = DataAccessLayer.ExecuteReader(CommandType.Text, query, null))
            {
                var objCartaEmail = new CartaEmail();
                while (SetInstance(dr, objCartaEmail, false))
                {
                    retorno.Add(objCartaEmail);
                    objCartaEmail = new CartaEmail();
                }
            }
        ...
        {
            var valoresConteudos = ListarCartas(new List<Enumeradores.CartaEmail> { carta });
            return valoresConteudos[carta];
        }

        public DTO.CartaEmail Montar(object parametros)
        {
            var cartaEmail = Montar(this);

            //var substitution = new Dictionary<string, string>();
            //if (parametros != null)
            //{
            //    foreach (var subValue in parametros)
            //    {
            //        substitution.Add(subValue.Name.ToString(), subValue.Value.ToString());
            //    }
            //}

            cartaEmail.Assunto = cartaEmail.Assunto.FormatWith(parametros, MissingKeyBehaviour.Ignore);
            cartaEmail.Conteudo = cartaEmail.Conteudo.FormatWith(parametros, MissingKeyBehaviour.Ignore);

            return cartaEmail;
        }

        private DTO.CartaEmail Montar(CartaEmail objCartaEmail, string subject = null, string content = null)
        {
            var subjectContent = subject ?? objCartaEmail.DescricaoAssunto;
            var bodyContent = content ?? objCartaEmail.ValorCartaEmail;

            if (objCartaEmail.Template != null)
            {
                var parentTemplate = GetById(objCartaEmail.Template.IdCartaEmail);

                subjectContent = parentTemplate.DescricaoAssunto.FormatWith(new { conteudo = subjectContent }, MissingKeyBehaviour.Ignore);
                bodyContent = parentTemplate.ValorCartaEmail.FormatWith(new { conteudo = bodyContent }, MissingKeyBehaviour.Ignore);

                return Montar(parentTemplate, subjectContent, bodyContent);
            }

            return new DTO.CartaEmail { Assunto = subjectContent.Replace("{conteudo}", string.Empty), Conteudo = bodyContent.Replace("{conteudo}", string.Empty) };
        }

        public static CartaEmail GetById(int cartaEmail)
        {
            return Cartas.First(c => c.IdCartaEmail == cartaEmail);
        }
