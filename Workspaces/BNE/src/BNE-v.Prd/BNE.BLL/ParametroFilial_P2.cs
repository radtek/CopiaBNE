...
namespace BNE.BLL
{
    public partial class ParametroFilial : ICloneable // Tabela: BNE_Pagamento
    {
        ...
        public static Dictionary<Enumeradores.Parametro, string> ListarParametros(Filial objFilial, List<Enumeradores.Parametro> idsParametros)
        {
            var parms = new List<SqlParameter>();

            var itensParametros = idsParametros.Distinct().ToDictionary(p => p, p => string.Empty); //Criando um dicionário vazio

            string query = "select Idf_Parametro, Vlr_Parametro FROM TAB_Parametro_Filial WITH (NOLOCK) where Idf_Filial = @Idf_Filial AND Idf_Parametro in ( ";
            ...
            parms.Add(new SqlParameter { ParameterName = "@Idf_Filial", SqlDbType = SqlDbType.Int, Size = 4, Value = objFilial.IdFilial });
            ...
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, query, parms))
            {
                while (dr.Read())
                {
                    ...
                    itensParametros[parametro] = Convert.ToString(dr["Vlr_Parametro"]);
                }
                if (!dr.IsClosed)
                    dr.Close();
            }
