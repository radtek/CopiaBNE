...
namespace BNE.BLL
{
    public partial class VagaTipoVinculo // Tabela: BNE_Vaga_Tipo_Vinculo
    {
        ...
        SELECT  vtv.*, tv.Des_Tipo_Vinculo, tv.Cod_Grau_Tipo_Vinculo, tv.Idf_Tipo_Vinculo
        FROM    BNE_Vaga_Tipo_Vinculo vtv WITH(NOLOCK)
        JOIN BNE_Tipo_Vinculo tv WITH(NOLOCK) ON vtv.Idf_Tipo_Vinculo = tv.Idf_Tipo_Vinculo
        ...
        public static List<VagaTipoVinculo> ListarTipoVinculoPorVaga(int idVaga, SqlTransaction trans = null)
        {
            ...
            {
                var objVagaTipoVinculo = new VagaTipoVinculo();
                while (SetInstance(dr, objVagaTipoVinculo, false))
                {
                    listVagaTipoVinculo.Add(objVagaTipoVinculo);
                    objVagaTipoVinculo = new VagaTipoVinculo();
                }
