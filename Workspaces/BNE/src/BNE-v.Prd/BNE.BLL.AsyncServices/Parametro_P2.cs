...
namespace BNE.BLL.AsyncServices
{
    public partial class Parametro // Tabela: TAB_Parametro
    {
        ...
        private static Dictionary<Enumeradores.Parametro, string> ListarParametrosCACHE()
        {
            ...
            string query = "select Idf_Parametro, Vlr_Parametro from TAB_Parametro WITH (NOLOCK) where Idf_Parametro in ( ";
        ...
        public static Dictionary<Enumeradores.Parametro, string> ListarParametros(List<Enumeradores.Parametro> idsParametros)
        {
            ...
            {
                EL.GerenciadorException.GravarExcecao(ex, "Erro ao buscar parametros.");
                throw;
            }
        }
        #endregion
        ...
        /// </summary>
        public static void SalvarParametros(Dictionary<Enumeradores.Parametro, string> dicionarioParametros)
        {
            var parms = new List<SqlParameter>();

            string query = String.Empty;
            int i = 1;
            foreach (KeyValuePair<Enumeradores.Parametro, string> kvp in dicionarioParametros)
            {
                ...
                string nomeCampo = String.Format("UPDATE TAB_Parametro SET Vlr_Parametro = {0} WHERE Idf_Parametro = {1};", nomeParametroValor, nomeParametro);
