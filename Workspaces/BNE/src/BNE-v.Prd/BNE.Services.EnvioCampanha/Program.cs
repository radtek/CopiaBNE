...
namespace BNE.Services.EnvioCampanha
{
    internal class Program
    {
            ...
            try
            {
                HostFactory.Run(c =>
                {
                    c.ScheduleQuartzJobAsService(q =>
                    {
                        ...
                        q.AddTrigger(() => TriggerBuilder.Create().StartNow().WithSchedule(SimpleScheduleBuilder.RepeatSecondlyForever(RepeatSecondlyForever)).Build());
        ...
        private static readonly int RepeatSecondlyForever = Convert.ToInt32(ConfigurationManager.AppSettings["SimpleScheduleBuilder.RepeatSecondlyForever"]);
