    ...
    public class IntegracaoController : ApiController
    {
        ...
        public HttpResponseMessage ExportaCandidato([FromBody] ExportaCandidatoParam param)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    integrador.ExportarCandidato(param, out result);
        ...
        /// </summary>
        /// <param name="commmand">Dados do usuário</param>
        /// <response code="200">Sucesso</response>
        /// <response code="401">Dados inválidos</response>
        /// <response code="500">Ocorreu um erro ao tentar realizar a operação</response>
        [HttpPut]
        [ResponseType(typeof(bool))]
        public HttpResponseMessage AtualizaUsuario([FromBody]AtualizaUsuario commmand)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    return Request.CreateResponse(integrador.AtualizarUsuario(commmand));
                }
                catch (Exception ex)
                {
        ...
        [HttpPost]
        public HttpResponseMessage ExportarCandidatoSINE([FromUri]decimal? cpf)
        {
            try
            {
                if (!cpf.HasValue)
                ...
                var result = Integrador.ExportarCurriculoSINE(cpf.Value);

                return Request.CreateResponse(HttpStatusCode.OK, new { Id = result });
            }
            catch (Exception e)
            {
