...
namespace BNE.Services.Plugins.PluginsEntrada
{
    ...
    public class Campanha : InputPlugin
    {
        ...
        private MensagemPlugin DeRecrutamento(CampanhaRecrutamento objCampanha, Dictionary<BLL.AsyncServices.Enumeradores.Parametro, string> dicionarioParametros)
        {
            ...
            loteSMS.idCampanha = (int)BLL.Mensagem.Enumeradores.CampanhaTanque.CampanhaRecrutamento;

            var objVendedor = objCampanha.Vaga.Filial.Vendedor();

            var templateSMSCampanha = new CampanhaTanque().GetTextoCampanha(BLL.Mensagem.Enumeradores.CampanhaTanque.CampanhaRecrutamento);
            ...
            if (quantidadeCurriculosEnviados > campanhaRecrutamentoTetoParaEnvio)
            {
                ...
                if (quantidadeCandidaturas < objCampanha.QuantidadeRetorno)
                {
                    ...
                    #region [Enviar E-mail para o Vendedor informando que a campanha atingiu o teto de disparo.]

                    //Funcao estagio
                    var funcao = VagaCurso.ListaCursoParaCartaEmail(objCampanha.Vaga.DescricaoFuncao, objCampanha.Vaga.IdVaga);
                    ...
                    if (objVendedor != null && !string.IsNullOrWhiteSpace(objVendedor.EmailVendedor))
                        listaEmail.Add(MontarEmail(assuntoAtingiuLimite, mensagemAtingiuLimite, emailRemetente, objVendedor.EmailVendedor));
                    else // Task 48432 - Empresa não tem vendedor enviar para o parametro
            ...
            {
                //Definindo lote e recuperando os currículos
                ...
                {
                    #region [Enviar E-mail para o Vendedor informando que a campanha não tem o número suficiente de Cvs.]
                    int quantidadeCandidaturas = objCampanha.Vaga.QuantidadeCandidaturas();
                    //Funcao estagio
                    var funcao = VagaCurso.ListaCursoParaCartaEmail(objCampanha.Vaga.DescricaoFuncao, objCampanha.Vaga.IdVaga);
                    ...
                    if (objVendedor != null && !string.IsNullOrWhiteSpace(objVendedor.EmailVendedor))
                        listaEmail.Add(MontarEmail(assunto, mensagem, emailRemetente, objVendedor.EmailVendedor));
                    else // Task 48432 - Empresa não tem vendedor enviar para o parametro
        ...
        private MensagemPlugin DePesquisaCurriculo(CampanhaRecrutamento objCampanha, Dictionary<BLL.AsyncServices.Enumeradores.Parametro, string> dicionarioParametros)
        {
            ...
            foreach (var objFuncao in funcoesPesquisa)
            {
                var loteSMS = new MensagemPlugin.MensagemSMSTanque
                {
                    ...
                    idCampanha = (int)BLL.Mensagem.Enumeradores.CampanhaTanque.PesquisaCV
                };
                var templateSMSCampanha = new CampanhaTanque().GetTextoCampanha(BLL.Mensagem.Enumeradores.CampanhaTanque.PesquisaCV);
