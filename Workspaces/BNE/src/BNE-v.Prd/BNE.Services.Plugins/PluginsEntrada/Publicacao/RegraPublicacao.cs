...
namespace BNE.Services.Plugins.PluginsEntrada.Publicacao
{
    internal class RegraPublicacao
    {
        ...
        public int IdfRegraPublicacao { get; set; }
        ...
        #region SpRecuperarRegrasTermosVagas
        private const string SpRecuperarRegrasTermosVagas = @"
        SELECT  Des_Palavra_Publicacao, 
                Des_Palavra_Substituicao,
                Idf_Acao_Publicacao,
                Des_Campo_Publicacao,
        ...
        WHERE   ( PP.Flg_Inativo = 0 OR PP.Flg_Inativo IS NULL )
                AND RP.Flg_Inativo = 0
                AND RCP.Flg_Inativo = 0
                AND CP.Idf_Tipo_Publicacao = 1
                AND Idf_Acao_Publicacao = 2
        ";
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public static List<RegraPublicacao> RecuperarRegrasPublicacao(TipoPublicacao tipoPublicacao, Vaga objVaga = null)
        {
            ...
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, Sprecuperarregrapublicacao, parms))
            {
                while (dr.Read())
                {
                    lista.Add(new RegraPublicacao
                    {
                        ...
                        IdfRegraPublicacao = Convert.ToInt32(dr["Idf_Regra_Publicacao"])
                    });
                }
            }
            if(objVaga != null)
            {// Task 61062 Algumas regras de publicação não se aplicam a certas empresas.
                List<BLL.Enumeradores.Parametro> listparametros = new List<BLL.Enumeradores.Parametro>();
                listparametros.Add(BLL.Enumeradores.Parametro.RegraPublicacaoAuditoriaVaga);
                var dici = ParametroFilial.ListarParametros(objVaga.Filial, listparametros);
                ...
                if(dici != null)
                {
                    ...
                    foreach (var item in dici)
                    {
                        ...
                        var remover = value;
                        lista.RemoveAll(x => x.IdfRegraPublicacao == remover);
        ...
        public static List<RegraPublicacao> RecuperarRegrasTermosVagas()
        {
            var lista = new List<RegraPublicacao>();
            using (IDataReader dr = DataAccessLayer.ExecuteReader(CommandType.Text, SpRecuperarRegrasTermosVagas, null))
            {
                while (dr.Read())
                {
                    lista.Add(new RegraPublicacao
